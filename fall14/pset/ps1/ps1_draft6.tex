...
ensuring that you can identify an unknown stone of weight 40.  Using the first approach, the set of 
stones we pick is {1, 2, 4, 8, 16, 32}, which is equivalent to the set $\{ 2^{0}, 2^{1}, 2^{2}, 2^{3}, 
2^{4}, 2^{5} \}$.  Using these set of stones, we can ``make" every weight from 1 to 40 (in fact, from 1 to 63): 
to make a particular weight, we look at the binary representation of the weight, and use the stones 
that correspond to the binary representation.  For example, to make 13, whose binary represetnation is 1101, we should 
