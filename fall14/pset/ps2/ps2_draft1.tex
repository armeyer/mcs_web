...
nonempty set of nonnegative integers that has no least element.  The induction should lead to a contradiction.)
...
value.  We will show that, in this solution, $a, b, c, \text{and } d$ must all be even.  However, we can then obtain 
another solution over the positive integers with a smaller $a$ by dividing $a, b, c, \text{and } d$ in half.  This 
is a contradiction, and so no solution exists.  All that remains is to show that $a, b, c, \text{and } d$ must all 
be even.  The left side of Moitra's equation is divisible by 3, so $d^4$ is a multiple of 3, so $d$ must be a multiple of 3.  Substituting 
$d=2d^{'}$ into Moitra's equation gives: 
