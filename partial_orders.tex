...
\end{equation}
holds for all $a,b \in A$, we have completely captured the weak partial
order $R$ by the subset relation on the corresponding sets.  A similar
...
\begin{notesproblem}
Prove the iff assertion~\eqref{rgb}.
\end{notesproblem}
...
\begin{notesproblem}\label{tir}
Prove that a binary relation is a strict partial order iff it is
