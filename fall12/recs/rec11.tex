...
\begin{scope}[>=stealth,->,thick]
   ...
   \draw[->] (v2) edge (v3);
   \draw[->] (v3) edge (v1);
  \draw (v1) .. controls (0.6,1) and (0.3,0.8) .. (v3) ;
  \draw (v3) .. controls (1,-0.3)  .. (v2);
\end{scope}
\end{tikzpicture}
\hspace{5cm}
\begin{tikzpicture}
\node (lb) at (1, -1) { (b) };
    \begin{scope}[every node/.style={circle,draw}]
    \node[draw] (v3) at (2,0) { 3 };
	\node[draw] (v2) at (2,2) { 2};
    \node (v1) at (0,2) { 1 };
    \node (v4) at (0,0) { 4};
\end{scope}
\begin{scope}[>=stealth,->,thick]
...
%\item Show that the PageRank of any webpage in a webgraph $G$ that is strongly connected is strictly positive.
\end{enumerate}
