...
%\begin{center}\includegraphics[width=12cm]{truthTable.pdf}\end{center}

\begin{center}
{\huge
\begin{tabular}{|c|c|c|c|c|c|c|c|}
\hline
 A \quad & B \quad & C \quad & \quad\quad & \quad\quad & \quad\quad & \quad\quad & \quad\quad \\ \hline
 &  &  &  &  &  &  & \\ \hline
 &  &  &  &  &  &  & \\ \hline
 &  &  &  &  &  &  & \\ \hline
 &  &  &  &  &  &  & \\ \hline
...
In the game of Squares and Circles, the players (you and your computer) start with a sequence of shapes: some circles and some squares. Players take turns making moves. On each move, a player chooses any two shapes from the sequence. These two are replaced with a single one according to the following rule:
...
\solution[\vspace{4in}]{We know $\dfrac{x}{x\ln{x}} = \dfrac{1}{\ln{x}} \to 0 < \infty$. Hence $x$  is $O\left( x\ln{x} \right)$.}

\ppart{3}
$x/ \ln{x}$  is $o  \left( x \right)$
\solution[\vspace{4in}]{In this case we have $\dfrac{x/ \ln{x}}{x} = \dfrac{1}{\ln{x}} \to 0  < \infty$}
\newpage
\ppart{3}
$x^{n+1}$ is $\Omega \left( x^n \right)$
\solution[\vspace{4in}] {Taking the quotient we arrive to  $ \dfrac{x^{n+1}}{x^n} = x \to \infty > 0 $} 

\ppart{3}
$n!$ is $\Theta \left( \dfrac{2\pi n!\ln{n}}{H_n} \right)$ Where $H_n$ is the $n^{\mbox{th}}$ Harmonic number.

\solution[\vspace{4in}]{Taking the division of the two functions we get $\dfrac{n!}{\frac{2\pi n! \ln{n} }{H_n}} = \dfrac{n! H_n}{2\pi n! \ln{n}} = \dfrac{H_n}{2\pi \ln{n}}$. Now we recall $H_n \sim \ln{n}$. Hence the limit  is $1/(2\pi)$ Which is not 0 or $\infty$, so $n!$ is $\Theta\left(  \dfrac{2\pi n! \ln{n}}{H_n} \right)$  }
