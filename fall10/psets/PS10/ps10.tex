...
\end{center}
In the book we proved that if we roll each die once, then die $A$ beats $B$ more often, die $B$ beats die $C$ more often, and die $C$ beats die $A$ more often. Thus, contrary to our intuition, the ``beats'' relation $>$ is not transitive. That is, we have $A > B > C > A$.

We then looked at what happens if we roll each die twice, and add the result. In this problem we will show that the ``beats'' relation reverses in this game, that is, $A < B < C < A$, which is very counterintuitive!
...
\begin{center}
\includegraphics[width=3in]{dieSum2}
\end{center}

There are $81$ leaves and the space is uniform, i.e., each outcome occurs with probability $(1/3)^4 = 1/81$. Let's work out the chances of winning. The sum of the two rolls of the $B$ die is equally likely to be any element of the following multiset:
...
\begin{center}
\includegraphics[width=3in]{dieSum3}
\end{center}
There are $81$ leaves and the space is uniform, i.e., each outcome occurs with probability $(1/3)^4 = 1/81$. Let's work out the chances of winning. The sum of the two rolls of the $C$ die is equally likely to be any element of the following multiset:
$$S_C = \{6, 7, 7, 8, 11, 11, 12, 12, 16\}.$$
...
\solution{We draw the sample space. In the figure, it should be understood that the tree corresponding to $B$ is connected to each leaf of the tree corresponding to $A$. 
\begin{center}
\includegraphics[width=5in]{dieSum4.jpg}
\end{center}
There are $81$ leaves and the space is uniform, i.e., each outcome occurs with probability $(1/3)^4 = 1/81$. Let's work out the chances of winning. The sum of the two rolls of the $B$ die is equally likely to be any element of the following multiset:
$$S_B = \{2, 6, 6, 10, 10, 10, 14, 14, 18\}.$$
The sum of the two rolls of the $A$ die is equally likely to be any element of the following multiset:
$$S_A = \{4, 8, 8, 9, 9, 12, 13, 13, 14\}.$$
We can treat each outcome as a pair $(x,y) \in S_B \times S_A$, where $A$ wins iff $y > x$. If $y = 4$, there is no $x$ for which $y > x$. If $y = 8$, there are $3$ values of $x$, namely $x = 4, 8, 8$, for which $y > x$. Continuing the count in this way, the number of pairs for which $y > x$ is $$1+3+3+3+3+6+6+6+6=37$$
while a similar count shows that there are $42$ pairs for which $x > y$, and there are $2$ ties. Thus, rolling die $B$ twice is more likely to win than rolling die $A$ twice.
}
\eparts 
...
\solution{There is no such strategy.  Let $S_{b,r}$ be a strategy that
  ...
  the first card, but to keep playing.  Then the first card is going to be red with probability $r/(r+b)$ and it's going to be black with probability $b/(r+b)$.  If the first card is black then we now have $b-1$ black cards remaining and $r$ red cards remaining and we should play $S_{b-1,r}$ which by our inductive hypothesis has a probability of $(b-1)/(b+r-1)$ of winning. On the other hand, if the first card is red, we now have $r-1$ red cards and $b$ black cards remaining and we should play $S_{b,r-1}$ which by our inductive hypothesis has a probability of $b/(b+r-1)$ of winning. This gives us a total probability of  \[\Pr(\mbox{win by playing $S_{b,r}$}) = ((b-1)/(b-1+r))(b/(r+b)) +  (b/(b+r-1))(r/(b+r)) = b/(b+r),\] as claimed.  This is summarized in the diagram below.
