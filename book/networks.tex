\chapter{Digraphs \& Communication Networks}
...
\begin{definition}
  ...
  \term{composition}, $S \compose R$, of $R$ and $S$ is defined to be the
...
Problem \ref{CP_transitive_irreflexive_implies_SPO}).
\end{proof}
...
\hyperdef{communication}{networks}{\section{Communication Networks}}
...
inside parallel computers.  In this section, we'll look at some of the
...
$\pi(i)$.  A \emph{routing}, $P$, that \term{solves} a routing problem,
...
\iffalse and that there are no additional delays at switches,\fi
...
\iffalse Following this approach, we can build arbitrarily large networks
just by adding in more building blocks.  \fi
...
``root switch''), 2 below it, 4 below those, and so forth.  By the formula~\eqref{}
for geometric sums, the total number of switches is $2 N - 1$, which is nearly the
...
\mfigure{!}{3.5in}{figures/butterfly2}
...
\begin{figure}[h]
\begin{center}
\mfigure{!}{4in}{figures/butterfly-base}
...
\begin{figure}[h]
\begin{center}
\mfigure{!}{4in}{figures/butterfly-recursive}
...
There is a simple argument that shows that the congestion of the
butterfly network is about $\sqrt{N}$, more precisely, the congestion
is $\sqrt{N}$ if $N$ is an even power of 2 and $\sqrt{N/2}$ if $N$ is
an odd power of 2 (see Problem\ref{PS_butterfly_congestion}).
...
amounts to recursively growing Bene\u{s} nets by adding both inputs
