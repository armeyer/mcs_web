\hyperdef{while}{programs}{\section{Reasoning About \textbf{While} Programs}}
...
these notes we'll illustrate what it means to have a clean mathematical
...
The programs we'll study are called ``\while\ programs.''  We can define
them as a recursive data type:
\begin{definition}\label{whiledef} \mbox{}

\textbf{base cases:}
\begin{itemize}

\item $x \assigned e$ is a \while\ program, called an \emph{assignment
    statement}, where $x$ is a variable and $e$ is an expression.

\item $\halt$ is a \while\ program.
...
\term{expressions} we'll mean any of the familiar integer valued
expressions involving integer constants and operations such as addition,
multiplication, exponentiation, quotient or remainder.  As \emph{tests},
...
stored data and it doesn't return a value.  The data consists of integers
stored as the values of variables, namely environments:
\begin{definition}
...
It's standard in programming language theory to write $\mean{e}\rho$ as
shorthand for $\mean{e}(\rho)$, that is, applying the \emph{meaning},
$\mean{e}$, of $e$ to $\rho$.  (A variable is a special case of an
expression, so $\mean{x}\rho \eqdef \rho(x)$.)

Executing a program causes a succession of changes to the environment
which may continue until the program halts.  Actually the only command
which immediately alters the environment is an assignment command.
Namely, the effect of the command
\[
x \assigned e
\]
on an environment is that the value assigned to the variable $x$ is
changed to the value of $e$ in the original environment.  We can say this
precisely and concisely using the following notation for functions:
\begin{definition}
...
in computability theory such as 6.045, 6.844, or 6.840.
...
environments to environments at halting.  The function $\mean{e}$ is
...
\emph{postcondition}.  So if $P$ is a formula expressing the precondition
...
For example, if $E$ is \while\ program above for the Euclidean algorithm, then the
partial correctness of $E$ can be expressed as
\begin{equation}\label{eucpc}
\after{\paren{a, b \in \naturals\ \QAND\  x \neq 0}}{C}
                   {\paren{x = \gcd(a,b)}}.
\end{equation}
Now we can build up rules for proving these kinds of partial correctness
formulas.  The rules below were formulated in the early 1970's
%%check ref
by Tony Hoare at Univ. Dublin and are known as \term{Hoare Logic}.

The first rule captures the fact that strengthening the preconditions and
weakening the postconditions makes a specification easier to satisfy:
