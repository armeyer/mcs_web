...
\term{equivalent}.  Since the two emphasized columns of truth values
...
A \term{valid} formula is one which is \emph{always} true, no matter
...
A \term{satisfiable} formula is one which can \emph{sometimes} be
...
\end{equation}
is true.
\begin{theorem}\label{thm:distribute-and-or}[\term{Distributive Law} of $\QAND$ over $\QOR$]
\[
A \QAND (B \QOR C)\text{ is equivalent to } (A \QAND B) \QOR (A \QAND C).
\]
\end{theorem}
Theorem~\ref{thm:distribute-and-or} is called a \term{distributive law}
...
\begin{theorem}\label{thm:distribute-or-and}[\term{Distributive Law} of $\QOR$ over $\QAND$]
\[
...
\term{disjunctive normal form} (\term{DNF}).  A DNF formula can often
...
Now we will repeatedly apply The Distributivity of \QAND\ over \QOR
