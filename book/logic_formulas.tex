...
If you haven't thought about it before, it's typical to be bothered by
the fact that implications are considered to be true when their
hypotheses are false.  But implications with false hypotheses never
comes up in ordinary, informal settings, so you shouldn't care much
what truth assignment logicians and mathematicians choose to give
them.

And of course, there are good reasons for choosing to have
implications be true when their hypotheses are false.  An illustrative
example would be a system specification (see
...
system indeed takes the specified actions $A_2$ and $A_5$.  In this
case, the implications $C_2 \QIMP\ A_2$ and $C_5 \QIMP\ A_5$ are both
true.  This means that in this case the system is behaving according
to specification, and accordingly we want the formula~\eqref{qandspec}
to come out true.  This in turn means we want all the other
implications $C_i \QIMP\ A_i$ for $i \neq 2, 5$ to be true, which is
exactly what the rules for implications with false hypotheses gives
us.
...
exponentially ---whether any given proposition of size $n$ is
...
in \term{polynomial time}, that is, that runs number of basic steps
bounded by a polynomial in $s$, where $s$ is the size of an input.  It
turns out that an efficient solution to SAT would immediately imply
...
chaos.  Decrypting coded messages would also become an easy task, so
online financial transactions would be insecure and secret
communications could be read by everyone.  Why this would happen is
explained in Section~\ref{SAT_RSA_sec}.
