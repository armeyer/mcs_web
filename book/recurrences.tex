...
their precedessors.  As a trivial example, this recurrence describes
...
So one typically wants to {\em solve} a recurrence; that is, to find a
...
The recursive solution has three stages, which are described below.
For clarity, the largest disk is shaded in the figures.
\begin{description}
\item{{\bf Stage 1.}}  Move the top $n-1$ disks from the first post to
...
\end{tabular}
\end{center}
\item{{\bf Stage 2.}}  Move the largest disk from the first post to the
...
\end{tabular}
\end{center}
\item{{\bf Stage 3.}}  Move the $n-1$ disks from the second post to
...
purchase burgers and ice cream {\em before} the end of the world.)

There are several methods for solving recurrence equations.  The
simplest is to {\em guess} the solution and then {\em verify} that the
...
proofs, the argument only goes through with a {\em stronger}
...
equations.  But there is one big drawback: you have to {\em guess
...
of looking at a sequence of {\em numbers}, you have to spot a pattern
in a sequence of {\em expressions}, which is sometimes easier.  The
...
rule applicable to the whole of college life--- is {\em chug in
  moderation}.
\begin{align*}
T_n & = 2T_{n-1} + 1 \\
...
effectively {\em proves} that the formula is correct for all $k$.
...
delightful.  Here, we'll cover just {\em one} well-known sorting
...
x^2 = x + 1
\end{align*}
Solving this equation gives {\em two} plausible values for the
parameter $x$:
\begin{align*}
...
\end{equation*}
\item Substitute the boundary conditions into the general soultion.
...
\Theta(n \log n)$ for {\em every} value of $T_1$.  The boundary
condition doesn't matter!

This is the typical situation: {\em the asymptotic solution to a
...
input {\em approximately} in half, sort the halves recursively, and
...
Fortunately, {\em the asymptotic solution to a divide and conquer
...
\label{th:akra-bazzi}
Suppose that the function $T: \mathbb{R} \to\mathbb{R}$ is satisfies
the recurrence
\begin{equation*}
T(x) =
\begin{cases}
\text{is nonnegative and bounded} & \text{for $0 \leq x \leq x_0$} \\
\sum_{i=1}^k T(b_1x + h_i(x)) + g(x) & \text{for x > 0}
\end{cases}
\end{equation*}
where:
\begin{enumerate}
...
This suggests that {\em generating smaller subproblems is far more
...
Fibonacci recurrence is just {\em slightly} smaller than in Towers of
