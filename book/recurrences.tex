...
\arm{Here we use subscripts $T_n$ but function notation $T(n)$ later.
  We should say something about this, or else be consistent.}
...
\section{The Towers of Hanoi}\index{towers of Hanoi}
...
The \idx{recursive solution} has three stages, which are described below.
...
simplest is to \index{guess-and-verify} \emph{guess} the solution and
...
\subsection{Plug and Chug}\index{plug-and-chug}
...
sometimes called ``\idx{expansion}'' or ``iteration''.  As in
...
illustrated with the Towers of Hanoi\index{towers of Hanoi} example.
...
particular, we've collapsed the \idx{geometric sum} to a closed form on the
second line.
...
Let's compare \idx{guess-and-verify} with plug-and-chug.  In the
...
Algorithms textbooks traditionally claim that \idx{sorting} is an
...
Here is how \idx{Merge Sort} works.  The input is a list of $n$
...
maximum number of \idx{comparisons} used in sorting $n$ items?''  This
...
because there is no obvious pattern.  So let's try the \idx{plug-and-chug}
method instead.
...
\section{Linear Recurrences}\index{linear recurrence}
...
recurrence equations.  \idx{Fibonacci numbers} arise in all sorts of
...
input numbers that make \idx{Euclid}'s \idx{GCD algorithm} require the greatest
...
\index{order of linear recurrence}
\index{linear recurrence, order of} of the recurrence is $d$.  Commonly, the
value of the function $f$ is also specified at a few points; these are
called \term{boundary conditions}.  For example, the Fibonacci
...
A charming features of \idx{homogeneous linear recurrence}s is that any
\idx{linear combination} of solutions is another solution.
...
This \idx{closed form} for Fibonacci numbers has some interesting
...
This value is often denoted $\phi$ and called the ``\idx{golden ratio}''.
...
to solve any \idx{homogeneous linear recurrence}; that is, a recurrence of
the form
\begin{align*}
f(n) = a_1 f(n-1) + a_2 f(n-2) + \ldots + a_d f(n - d)
\end{align*}
...
\item If $r$ is a \idx{repeated root} with \idx{multiplicity} $k$ then $r^n,
...
right side of a linear recurrence gives an \term{inhomogenous linear
  recurrence}:
\begin{align*}
...
complete the job is given by a recurrence with a $+n$ term \arm{a $+n$
  term...}  instead of a $+1$.
\begin{align*}
f(1) & = 1, \\
...
\arm{This paragraph revised 7/15/10 to mention gen func Chapter}
...
\arm{Explanation of guessing strategy moved from here into quick
  guide}

\begin{center}
START OF QUICK REFERENCE GUIDE
% This should probably be visually
% distinguished so students can find it quickly.
\end{center}

\begin{center}
\subsection*{Short Guide to Solving Linear Recurrences}\index{linear recurrences, guide to solving}
\end{center}
...
the amazing \term{Akra-Bazzi formula}, namely,
\arm{REDUNDANT:
...
So $p = 1$ does the job. \arm{inserted g(u)} Next, since in this case
...
This is the typical situation: \emph{the \idx{asymptotic solution} to a
...
conditions and integrality all follow from the \term{Akra-Bazzi
...
\arm{``inductively'' on what?...there's no $n$ here.  Maybe cut this remark?}
...
\end{array}
\]
Notice that the recurrence equations for Towers of Hanoi\index{towers
  of Hanoi} and \idx{Merge Sort} are somewhat similar, but the
