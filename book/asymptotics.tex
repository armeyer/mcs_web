...
In our annuity problem, $x=1/(1+p) < 1$, so Theorem~\ref{th:series} applies.
...
\]
$H_n$~is called the $n$th \term{Harmonic number}, and we have just shown
that
\[
B_n = \frac{H_n}{2}.
\]
...
For functions $f,g: \reals \to \reals$, we say $f$ is \term{asymptotically
...
The reason that the \index{$\sim$ (asymptotic equality)}$\sim$ notation is
...
\term{double summations}.  Sometimes it is easy: we can evaluate the inner
...
multiplication procedure, but none is known.\fi
