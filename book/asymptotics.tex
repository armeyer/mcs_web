...
\end{equation*}
for a \term{geometric series}, which means that the ratio of
...
for any geometric series, namely\footnote{To make this equality hold
  for $x = 0$, we adopt the convention that $0^0 ::= 1$.}  \dmj{Do you
  want to point out that the closed-form solution is undefined for $x
  = 1$ even though the original sum is perfectly well-behaved?}
...
formula:\dmj{I knew what you where you were going and I still had to
  stare at the equations to figure out how the pieces fit together.
  Maybe split into two displays: one showing how taking the derivative
of the geometric series yields the desired sum, and the other working
out the derivative of the closed form.}
\begin{align*}
...
\gnote{Figure G4, p.~27 (G-3), ftl-ch9-jul7.}
...
\gnote{Figure G5, p.~28 (G-4), ftl-ch9-jul7.}
...
\gnote{Figure G6, p.~29 (G-5), ftl-ch9-jul7.}
...
\gnote{Geoff: Three subfigures: Figure G9, p.~29 (G-8), ftl-ch9-jul7.}
...
\gnote{Figure G11, p. 35 (G-11), ftl-ch9-jul7.  Note that the top
  block is meant to be unstable (see first paragraph in
...
\gnote{Figure 11.1, p. 39 (708), ftl-ch9-jul7.  Change ``book'' to
  ``block''.}

\graphic[width=.40\textwidth]{bookstack-3}
...
block.  We call this distance the \term{spread} of the stack.  Note
...
\gnote{Figure 11.2, p. 41 (709), ftl-ch9-jul7.  Note extra text in
  manuscript.}

\graphic[width=.4\textwidth]{bookstack-2}
...
Suppose we have a stable stack~$S$ of $n$~blocks with maximum possible
...
\gnote{Figure G14, p. 44 (J-3), ftl-ch9-jul7.}
...
\gnote{Figure G15, p. 45 (J-4), ftl-ch9-jul7.}
...
\gnote{Figure G20, p. 50 (J-9), ftl-ch8-jul7.  Two subfigures.}
...
\gnote{Figure G22, p. 52 (J-11), ftl-ch9-jul7.}
...
$n$~increases, this means that if we are given enough blocks \dmj{move
  ``(in theory anyway)'' to end of sentence?}(in theory anyway), we
...
no longer has a summation inside it).  For example,\footnote{Ok, so
  ...
  straightforward.  Wait till you see the next one!}  \dmj{I reworked
  this a bit for (I hope\@!) improved readability.}
\begin{align*}
...
\begin{theorem}\label{thm:9Q2}\label{thm:stirling}
For all $n \ge 1$,
\begin{equation*}
...
One important thing to notice about Theorem~\ref{thm:9Q2} is that both
...
Equation~\ref{eqn:9Q4} is known as \term{Stirling's Formula} and is
...
out that there is another, ingenious \idx{matrix multiplication}
...
If all this sounds a lot like big-Oh, only in reverse, that is because
...
\dmj{I rewrote this to avoid the dangling participle, but I hope it
  can be improved further.  (``Analogous to little-oh, there is also a
  symbol called little-omega to...''?)}
%
