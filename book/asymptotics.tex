...
The usual formulation of big O spells out the definition of $\limsup$
...
Sometimes people incorrectly use big O in the context of a lower
...
least~$O(n^2)$''?  No!  This statement is meaningless since big O can
only be used for \emph{upper} bounds.  For lower bounds, we use a
different symbol, called ``big%
\index{asymptotic notation!big Omega|textbf} 
Omega.''
...
Likewise, there is also a symbol called little%
\index{asymptotic notation!little omega|textbf} 
omega, analogous to little o, to denote that one function grows 
strictly faster than another function.
