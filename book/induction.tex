...
already proved (equation~\eqref{sum1n}) using the \idx{Well Ordering
Principle}.  It is the kind of statement about all nonnegative
...
\textbf{We'll describe in class some hypotheses that do work.}
...
h_n$, again.  Since $h_1$ and $h_{n+1}$ are the same color as $h_2, \dots,
...
%hide after lecture:
\textbf{See if you can figure it before we take it up in class.}
%end hide
...
which makes them important in computer sciemce, since computer
programs can be understood as defining step-by-step computational
...
dropping below 1,000 feet without the wingflaps and landing gear being
deployed, and the temperature of a nuclear reactor never exceeding the
threshold for a meltdown.
...
\text{transitions} & \eqdef \set{n \movesto n+1 \suchthat 0 \ln n < 99}
                     \union \set{99 \movesto overflow, overflow \movesto overflow}.
\end{align*}

State machines used in digital logic and compilers usually have only a
finite number of states, but machines that model continuing
computations typically have an infinite number of states.  In many
applications, the states, and/or the transitions have labels
indicating input or output values, costs, capacities, or
probabilities, but for our purposes, unlabelled states and transitions
are all we need.  We do name states, as in Figure~\ref{fig:counter},
so we can talk about them, but the names aren't part of the state
machine.

For example, instead of the 99-bounded counter, we could easily define
an ``unbounded'' counter that just keeps counting up without
overflowing similar.  The unbounded counter has an infinite state set,
namely, the nonnegative integers.  This is harder to
draw. \texttt{:-)}

\subsection{Invariance for a Diagonally-Moving Robot}
Suppose we have a robot that moves on an infinite 2-dimensional grid.
The \emph{state} of the robot at any time can be specified by the
coordinates $(x, y)$ of the robot's current position.  The \emph{start
  state} is~$(0, 0)$ since it is given that the robot starts at that
position.  At each step, the robit may to a diagonally adjacent grid
point.  To be precise, robot's transitions are:
\[
...
This once, let's go through this argument again carefully highlighting
where induction comes in.  Namely, define the even-sum property of
states to be:
\[
...
transitions: $(m,n)\movesto (m\pm 1, n\pm 1)$, so after a transition,
the sum of coordinates changes by $\pm 1 + \pm 1$ which is 0, 2, or
-2.  Of course, adding 0, 2 or -2 to an even number gives an even
number.  So by a trivial induction on the number of transitions, we
can prove:
...
\begin{proof}
The proof is induction on the number of steps the robot has made.  The
induction hypothesis is
\[
P(n) \eqdef \text{if $q$ a state reachable in $n$ steps, then Even-sum($q$)}.
\]

\textbf{base case}: $P(0)$ is true since the only state reachable in 0
steps is the start state $(0, 0)$, and $0 + 0$ is even.

\textbf{inductive case} Assume that $P(n)$ is true, and let $r$ be any
state reachable in $n+1$ steps. We need to prove that Even-sum($r$) holds.

Now since $r$ is reachable in $n+1$ steps, there must be a state, $q$,
reachable in $n$ steps such that $q \movesto r$.  Now Even-sum($q$)
holds since $P(n)$ is assumed to be true, so by
...
in $n$ steps, then Even-sum($q$), which implies that every reachable
state has the Even-sum property.
...
The first correctness property is called \term{partial correctness};
...
considerably fewer multiplications called \term{Fast Exponentiation}
...
A useful variant of induction is called \term{strong induction}.
...
But strong induction really isn't any stronger, because any proof
...
nothing like the one for the\idx{Well Ordering Principle}, this
