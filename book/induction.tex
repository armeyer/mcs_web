...
for all nonnegative integers.  Induction plays a central role in discrete
...
---Ordinary and Strong ---and explains why they work and how to use
them in proofs.  It also introduces the Invariant Principle, which is
a version of induction specially adapted for reasoning about
...
\item \textbf{Prove that $P(n)$ implies $P(n+1)$ for every nonnegative
  ...
  for every nonnegative integer $n$, since the goal is to prove the
  that all the implications $P(0) \rightarrow P(1)$, $P(1) \rightarrow P(2)$, $P(2)
  \rightarrow P(3), \dots$, are true.
...
squares.} occupied by a statue of a wealthy potential donor ---whom we
...
\examproblems
\pinput{FP_sums_and_products_of_integers}
...
Carnegie-Mellon Univ.)  Floyd was already famous for work on formal
...
at http://oldwww.acm.org/pubs/membernet/stories/floyd.pdf written by
his closest colleague, Don Knuth.
...
There a few standard properties of derived variables that are handy in
...
does not guarantee that every execution terminates.  That's because an
...
of Definition~\ref{to1_well-order}.  In particular, define
$v:\naturals^2 \to \naturals + \twdone$ as follows
\[
v(x,y) \eqdef y + \frac{x}{x+1}.
\]

Now it's easy to check that $(x,y)\movesto (x',y')$ is a legitimate
robot move iff $v((x',y')) < v((x,y))$.  In particular, $v$ is a
