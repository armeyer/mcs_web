...
cover every possibility.  Since $P(n+1)$ is true in ever case, we can
...
\begin{equation*}
    \meval: \aexp \cross \integers \cross \integers,
\end{equation*}
...
didn't we use it?  Because 

\bigskip
\textcolor{red}{page 444 was missing from my copy of FTL's edits}
\bigskip

the definition of $M$ is ambiguous, while the
(perhaps less natural) definition of $\RM$ is unambiguous.  Does this
ambiguity matter?
Yes, it does.  For example, suppose we defined
\begin{align*}
...
Now by these rules, applied to the first way of constructing~$c$,
