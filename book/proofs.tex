...
not true that the expression is prime \emph{for all} nonnegative integers.
In fact, it's not hard to show that \emph{no} polynomial with integer
coefficients can map all natural numbers into prime numbers, unless it's a
constant (see problem~\ref{PS_prime_polynomial_41}).
...
things are so common that there is a special notation for them.  With this notation,
...
check colorability of several hundred special maps (see
