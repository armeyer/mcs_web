...
showing how generating fucntions can be used to solve certain kinds of
...
automatic way to find the power series coefficients any formula that
is a quotient of polynomials, namely, the method of \idx{partial
...
polynomial has distint nonzero roots, the method rest on
...
\frac{c}{(1-\alpha x)^k}
\]
where $\alpha$ is the reciprocal of the root and for $k \leq m$.
...
F(x)     & = & f_0 & + & f_1 x & + & f_2 x^2 & + & f_3 x^3 & + & \cdots & + & f_n x^n + \cdots.\\
-xF(x)   & = &     & - & f_0 x & - & f_1 x^2 & - & f_2 x^3 & - & \cdots & - & f_{n-1} x^n + \cdots.\\
-x^2F(x) & = &     &   &       & - & f_0 x^2 & - & f_1 x^3 & - & \cdots & - & f_{n-2} x^n + \cdots.\\
\hline
F(x)(1-x-x^2)
         & = & f_0 & + & (f_1-f_0) x
                               & + & (f_2 - f_1 - f_0) x^2
                                             & + &  0  x^3 & + & \cdots & + & 0 x^n + \cdots.\\
         & = &  0  & + & 1    x & + & 0   x^2 = x,
\end{array}
\]
so
...
T(x)     & = & t_0 & + &  t_1 x & + &  t_2 x^2 & + & \cdots & + &     t_n x^n + \cdots.\\
-2xT(x)  & = &     & - & 2t_0 x & - & 2t_1 x^2 & - & \cdots & - & 2t_{n-1} x^n + \cdots.\\
-1/(1-x) & = &  -1 & -  &   1 x & - &    1 x^2 & - & \cdots & - &       1 x^n + \cdots.\\
\hline
T(x)(1-2x) - \frac{1}{1-x}
         & = & t_0 -1 & + & 0 x & + &    0 x^2 & + & \cdots & + &       0 x^n + \cdots.\\
         & = & -1,
\end{array}
\]
so
...
\]
for some constants $c_1,c_2$.  Now multiplying both sides by the right
...
\end{equation}
for some $c_i \in \complexes$ is called a \term{degree $d$
  linear recurrence} with inhomogeneous term $h(n)$.
...
nonnegative integers $\leq d$ and $\alpha$ equals the reciprocal of a
root of the denominator polynomial.  For example, see
Problems~\bref{CP_Fibonacci_and_bunnies}, \bref{MQ_gen_2} and
\bref{CP_towers_of_Sheboygan}.

\section{Formal Power Series}
\TBA{to appear}

\begin{editingnotes}
With this April 19, 2012 revision, all the problems are likely out of
order and need to be moved to other sections.
\end{editingnotes}
