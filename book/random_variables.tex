...
As an example, suppose that you roll two unibased, independent,
...
for an indicator random variable.  Specifically, the \term{Bernoulli
  distribution} has a probability desnity function of the form $f_p:
\set{0, 1} \to [0, 1]$ where
\begin{align*}
...
independent coins that are heads with probabilty $p = 0.75$.  The
...
You may have notced that all of the distributions we have discussed
...
$\mu$ (the mean) and~$\sigma^2$ (the variance).  It's pdef is the
function
\begin{equation*}
...
is exponentially samll in~$n$, which is similar to the behavior of
