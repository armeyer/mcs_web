...
\begin{proof}
Given any nonempty subset, $S$, of $\naturals + \twdone$, take the
floors of each of the elements in $S$.  These floors are all
nonnegative integers, so by the WOP, there is a minimum one; call it
$n_s$.

The integer $n_S$ must be the floor of some number of the form
$n_S+f$, where $f$ is a fraction in $\twdone$.  So let's take all such
fractions $f$, that is, all the $f$'s such that $n_S+f \in S$.  These
$f$'s make up a nonempty set of fractions in $\twdone$, and since
$\twdone$ is well ordered, this nonempty set contains a minimum
element; call it $f_S$.  Now it's easy to verify that $n_S+f_S$ is the
