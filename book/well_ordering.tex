...
We've previously taken for granted the \emph{Prime Factorization Theorem},%
\index{Prime Factorization Theorem|see{Unique Factorization Theorem}}%
\index{Unique Factorization Theorem} 
also known as the \emph{Unique Factorization Theorem} and the \emph{Fundamental Theorem of Arithmetic}, which states that every integer
  greater than one has a unique\footnote{\dots unique up to the order
    in which the prime factors appear} expression as a product of
  prime numbers.  This is another of those familiar mathematical facts
  which are not really obvious.  We'll prove the uniqueness of prime
