...
given that some other event happens.  Questions about
...
\]
\end{rul*}
Of course there is an $n$-event version of the Rule, given in
Problem~\ref{PS_conditional probability_product_rule}, but its form
...
this number is $\binom{n}{k}$, but now the product rule will give us
new derivation of the formula for $\binom{n}{k}$.
...
randomly choosing a given size-$k$ subset, $S$, then the number of
such subsets will be $1/p$.  So what's $p$?  Well, the probability
...
p & = \frac{k}{n} \cdot \frac{k-1}{n-1} \cdot \frac{k-2}{n-2}
  ...
  & = \frac{k!(n-k)!}{n!}.
\end{align*}
So we have again shown that $1/p$, namely, the number  of size-$k$
subsets of $[1,n]$ is
\[
\frac{n!}{k!(n-k)!}.
\]
...
that actually have the condition.  So in terms of overall accuracy,
completely ignoring the test result is a better strategy!
...
are \emph{not} mutually independent.  In fact, it's obvious that they're
not even 3-way independent: if Alice and Bob match, and also
Alice and Carol match, then Bob and Carol will match.
