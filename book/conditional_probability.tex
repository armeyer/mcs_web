...
+ \frac{\pr{(C, A, B)}}{\pr{\set{(C, A, B),(A, A, B),(A, A, C)}}} = \frac{1/9}{1/18 + 1/18 + 1/9}
= \frac{1}{2}.
\]
...
\[
\prcond{\text{[win by switching]}}{\text{[pick A $\QAND$ Carol opens B]}}
= \prcond{(C, A, B)}{\set{(C, A, B),(A, A, B)}}
+ \frac{\pr{(C, A, B)}}{\pr{\set{(C, A, B),(A, A, B)}}} = \frac{1/9}{1/9 + 1/18}
= \frac{1}{2}.
\]
which is exactly what we already deduced from the tree
diagram~\ref{fig:14A2} in the previous chapter.
