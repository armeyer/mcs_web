...
she was abused \emph{and} murdered, is around 

You can see how Strogatz got this number in problem XXXX. But the real
point we wanted to make is that conditional probability is used and
misused all the time, and even experts subject to close scrutiny
can make mistakes.  }
...
entire sample space. For instance, if $E_1, E_2,$ and $E_3$ are disjoint, and if
\pr{E_1} + \pr{E_2} + \pr{E_3} = 1, then
\[
\pr{A} = \prcond{A}{E_1} \cdot \pr{E_1} + \prcond{A}{E_2} \cdot \pr{E_2} + \prcond{A}{E_3} \cdot \pr{E_3}
\]
...
The generalization to $n$ disjoint events is a routine exercise.

\begin{editingnotes}
Again, put $n$ event version in a problem:
\end{editingnotes}
...
\term{Simpson's Paradox}.  But Table~\ref{fig:15D3} shows a set of
