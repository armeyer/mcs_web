...
\]
is our chance of winning---50\%. Mathematically, at least, the logic
in the introduction is sound.  Curioser and curioser!
...
B.'' By misstating our condition, we inadvertently included extraneous
outcomes in our calculation.  With the correct consitioning, we still
win by switching 1/9 of the time in case $(C, A, B)$, but the total
probability is smaller: \pr{(A, A, B)} + \pr{(C, A, B)} = 1/18 + 1/9 =
3/18. The conditional probability would then be:
\[
...
As a simple application, we can use the rule to calculate the number
of size-$k$ subsets of the integers $[1,n]$.  Of course we already
this number is $\binom{n}{k}$, but now the Conditional Probability
Product Rule will give us new derivation of the formula for
$\binom{n}{k}$.
...
\footnote{For illustrative purposes, we are rounding or simplifying
  many of the statistics in this example.
\begin{editingnotes}To solve this same problem
  with more accurate statistics, try \TBA{write problem, insert
  link here}\end{editingnotes}} which seems pretty good for a relatively inexpensive
...
beyond the scope of a mathematical textbook.
\begin{editingnotes}
...
\subsubsection{The Hockey Team in Reverse}
...
of conditional probability.\footnote{Problem~\ref{PS_conditional_space} explains why
