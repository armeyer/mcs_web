...
\emph{given} that some other event [pick A, goat at B] happens.
...
opening of Section~\ref{sec:confuse_Monty} is sound.  Curiouser and
curiouser!
...
common one: \emph{we chose the wrong condition}.  In our
...
subsets of the integers $\Zintv{1}{n}$.  Of course we already know this number
is $\binom{n}{k}$, but now the rule will give us a new derivation of
the formula for $\binom{n}{k}$.

So suppose we choose a size-$k$ subset at random, with all subsets of
$\Zintv{1}{n}$ equally likely to be chosen.  Now if $p$ is the probability of
...
month,'' and likewise with CS and an even-numbered day of the month.
