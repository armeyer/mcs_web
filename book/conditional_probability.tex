...
statistician would assert:\footnote{Confidence is usually used to
  ...
  using his one concept to explain a standard approach to both
...
betting against the extremely unlikely event, which means assuming the
diagnosis is wrong.

In other words, some knowledge of the probability of having TB is
needed in order to figure out how seriously to take a positive
diagnosis, even when the diagnosis is given with what seems like a
high level of confidence.  We can see how the frequency of TB in a
population influences the value of a positive diagnosis by actually
calculating the probability that someone who tests positive has TB.
That is, we want to calculate $\prcond{\TB}{\testplus}$, which we do next.
...
Your doctor should have not have been talking about your particular
situation.  He should just have stuck to the statement that for
\emph{randomly chosen} people, the positive test would be right only
one percent of the time.

But you are not a random person, and whether or not you have TB is a
fact about reality.  The truth about your having TB may be
\emph{unknown}, but that does not mean it has some probability of
being true.  It is either true or false, we just don't know which.
...
Asians/Pacific Islanders than native born Caucasions.  So your
probability of TB would increase dramatically if your family was from
an Asians/Pacific Island.
...
will make different judgments and report differing probabilities.
There is no ``true'' model of who you are, and there is no true
probability of your having TB.
...
The negligible probability that a fair coin will flipping one hundred
consecutive Heads undermines the credibility of the fact that the coin
is fair.  So despite being told the coin is fair, we should
acknowledge at least some tiny possibility of picking an unfair coin.
So let's assume that there are two coins, a fair one and one that
always comes up heads.  One of these coins is randomly chosen biased
so the unfair coin is chosen only with extremely small probability
$2^{-50}$.  The chosen coin is then flipped 100 times.  Let $E$ be the
event of flipping 100 heads and $H$ be the event that the unfair coin
was chosen.
...
virtually certain to be a Head, we had to assume that some \term{prior
  probability} of choosing the fair coin was less than one.  Making
the assumption of some prior probability is known as the
\emph{Bayesian} approach to the problem.

Granting a tiny probability that the unfair coin was being flipped
seems reasonable, so the Bayesian approach provides a good explanation
of our intuitive judgment that the next flip will be a Head.
