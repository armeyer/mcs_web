...
us less ingenious folks, linear recurrences offer a less inspiring but
methodical approach to Gambler's Ruin.
...
\]
Similarly, after the gambler
loses his first bet, he can expect to make another $e_{n-1}$ bets:
\[
...
\expect{\text{number of bets till game ends}} =
\begin{cases}
(\pr{\text{gambler is a winner}}T-n)/(p-q)  &\text{ if } p \neq
1/2,\\
n(T - n) &\text{ if } p = 1/2.
\end{cases}
\end{equation}
...
In the unbiased case ($p=1/2$), Theorem~\ref{expectedbets} can be
rephrased simply as
\begin{equation}\label{capital.profit}
\expect{\text{number of fair bets till game ends}} = \text{initial capital}
...
is $500 \times 100 = 50,000$.  Notice that~\eqref{capital.profit} is a
very simple answer that cries out for an intuitive proof, but we have
not found one.
