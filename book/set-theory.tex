...
the time doubted the relevance of what they called ``\idx{Cantor's
  paradise}'' of ``arbitrary'' infinite sizes.
...
\idx{power set}, $\power(A)$, is ``\idx{strictly bigger}'' than $A$.  That is
...
``\idx{diagonal argument}.''  Diagonal arguments are used to prove many
fundamental results about the limitations of computation, such as the
undecidability of the \idx{Halting Problem} for programs (see
