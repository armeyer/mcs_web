...
The simplest case of rotation is when $\dpth{U} = d+1$.  In that case we
...
Here $X \in \rectr$ with depth $d+2$ is a ``new'' tree ---that is, $X
\notin \subbrn{N}$---with the same label as $S$, and $Y$ with depth
$d+1$ is a new tree with the same label as $N$.  So $\nlbls{(X)} =
\nlbls{(N)}$ and the depths of $X$ and $Y$ are properly balanced
according to the AVL rule.  It is easy to check that $X$ is also a
search tree, which makes it an an AVL tree.  Moreover, $X$ has very
nearly the same subtrees as $N$.  In particular,
\[
