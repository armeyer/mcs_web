...
\subsection{Strings}
We've taken strings of characters for granted as familiar mathematical
objects but defining them as a recursive data type actually can be
helpful.  They make a good first example because we're just giving
recursive definitions of things you already know or are easy to
understand.

\begin{definition}\label{recstring_def}
Let $A$ be a nonempty set called an \term{alphabet}, whose elements
are referred to as \term{symbols}, \term{letters}, or
\term{characters}.  The recursive data type, $\strings{A}$, of strings
over alphabet, $A$, are defined as follows:
\begin{itemize}
\item \textbf{base case}: the empty string, $\emptystring$, is in $\strings{A}$.

\item \textbf{constructor case:} If $s \in \strings{A}$ and $a \in A$, then the pair
       $\ang{a,s} \in \strings{A}$.
\end{itemize}
\end{definition}
If strings were represented in a programming language like Scheme or
Python as \emph{lists} of characters, then $\ang{a,s}$ would be the
list $\text{cons}(a ,s)$.
...
The \term{concatenation} $s\cdot t$ of the strings $s$ and $t$ is the
string consisting of the letters of $s$ followed by the letters of
$t$.  This is a perfectly clear and precise mathematical definition of
concatenation (except maybe for what to do with the empty string).  In
terms of Scheme/Python lists, $s\cdot t$ would be the list
...
\subsection{Strings of Matched Brackets}
