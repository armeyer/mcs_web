...
\inductioncase{Base case}: ($T$ is a leaf).  The only subtree of $T$
is itself, so
\[
\sz{T} = 1 < \infty.
\]
...
depth $d$ and the right branch is $S$ with depth $d+2$.  Then $N$ must
have subtrees as indicated in Figure~\ref{rotate1}.  The rotation
operation re-arranges them into the more balaced tree in
Figure~\ref{rotate2}.
...
\item $\dpth{T_1} = d$,
\item $\dpth{T_6} = d+2$,
\item $\max\setr{\dpht{T_3},\dpth{T_5}} = d$, $\min\setr{\dpht{T_3},\dpth{T_5}} = d-1$,
\item $d \leq \dpth{T_7} \leq d+1$.
\end{itemize}
...
\inductioncase{subcase}: ($T$ is a leaf with one label).  Then
...
\inductioncase{subsubcase}: ($\dpth{S} \leq \dpth{\rightsub{T}}+1$.)
...
\inductioncase{subsubcase}: ($\dpth{S} = \dpth{\rightsub{T}}+2$.)

Then we may assume $T$ has subtrees $T_1,T_2,\dots,T_7$ where
$\nlbl{(T_2)} = \nlbl{(T)}$

\TBA{To be continued\dots}
