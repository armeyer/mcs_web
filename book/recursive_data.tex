...
then the player who moves second can guarantee a win simply by
...
\inductioncase{Base case}: ($G = \loseend$).  The sequence $\loseend$
...
\inductioncase{Base case}: ($G= \loseend$).  Likewise with the outcome being a loss.
...
\subsection{Fundamental Theorem for Win-Lose Games}
...
every possible opponent strategy.
...
their strategy.  Thus, it seems possible that any given strategy for
one player will lose against some opponent strategy that is tailored
to beat it.
...
there is only one possible strategy for each player, namely, finish
with outcome $G$ by doing nothing.
...
Notice that although Theorem~\ref{fundgames} guarantees a winning strategy, its
