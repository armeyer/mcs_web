...
\begin{editingnotes}
Draft, partly subsumed by CP\_VG   ARM 3/4/16
\end{editingnotes}

\section{Games as a Recursive Data Type}

Chess, Checkers, Go, and Nim are examples of \emph{two-person
  terminating games of perfect information}---$\tg$'s for short.
These are games where the move a player can make depends only on the
visible board position, or \emph{state} of the game.  ``Perfect
information'' means that the players know the complete state of the
game at each move.  On the other hand, most card games are \emph{not}
games of perfect information because neither player can see the
other's hand.  ``Terminating'' means that play cannot go on
forever---it must end after a finite number of moves.\footnote{Since
  ...
  the board position \emph{plus} a record of how many times each
  position has been reached.}

We will define $\tg$'s \iffalse in a straightforward way tagged\fi as
a recursive data type and then give a recursive definition of optinal
strategies for game play.  To see how this will work, let's begin with
the game of Nim.
...
player to move wins by taking the whole pile.  On the hand, if there
are two piles withthe same number of stones, then the second player
can guarantee a win simply by mimicking the first player.  For
example, if the first player removes three stones from one pile, then
the second player removes three stones from the other pile.  Take a
moment to \textbf{think through why the mimicking strategy guarantees
  a win}.
