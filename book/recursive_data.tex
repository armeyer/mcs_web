...
The usual way to treat binary strings is as sequences of 0's and 1's.  For
example, the usual definition of the length-4 binary string that is the
binary representation of eleven is the sequence $(1,0,1,1)$ which we would
usually write as ``1011.''  But according to recursive Definition~\ref{recstring_def}
this string would be represented by nested pairs, namely
\[
\ang{1,\ang{0,\ang{1,\ang{1,\emptystring}}}}.
\]
These nested pairs are definitely cumbersome, and may also seem bizarre,
but they actually reflect the way lists of characters would be represented
programming languages like Scheme or Python, where $\ang{a,s}$ would be
the list $\text{cons}(a ,s)$.

We haven't said exactly how the empty string is represented because it
doesn't matter: we just need to be able to recognize it and not confuse it
with any nonempty string.

Continuing the recursive approach, let's define the length of a string.
\begin{definition}
The function $\lnth:\strings{A} \to \naturals$ is defined recursively
...
then
\begin{itemize}
\item $P(r)$ is true for all $r \in R$.
\end{itemize}
}
\begin{problems}
\classproblems
...
It's pretty easy to see that the definition of $M$ is just another way to
define $\RM$, that is $M = \RM$ (see Problem~\ref{M=RM}),
and the definition of $M$ seems more straightforward, why
...
\item $\lefbrk e \sumsym f \rhtbrk \in \aexp$.  The expression $(e \sumsym
  f)$ is called a \emph{sum}.  The \aexp's $e$ and $f$ are called the
