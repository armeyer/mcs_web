...
\newcommand{\nimg}[1]{\ensuremath{\text{Nim}_{\ang{#1}}}}
\newcommand{\fmv}[1]{\ensuremath{\text{Fmv}_{\ang{#1}}}}
...
a win-lose game.  The other games might end in a tie (draw, stalemate,
jigo) rather than a win or loss.  But the results about win-lose games
will apply to games with ties if we simply treat a tie as a loss for
the first player,
...
\end{equation}
describes a length-three path that starts at the top tree (6.8) with
label 6.8.  The selectors are applied in right-to-left order, so we
first go to $\leftsub{(6.8)}$ which is the subtree labelled $\pi$.
Then we take the left branch to the subtree $\leftsub{(\pi)}$ labelled
3, and take a final right branch to end with the subtree
$\rightsub{(3)}$ labelled 3.1.  That is, the subtree at the end of the
...
subtrees will be the same.  In \term{AVL trees} this condition is
