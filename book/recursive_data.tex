...
say how to construct new data elements from previous ones.  Along with each
...
\begin{itemize}
\item \inductioncase{Base case(s)} specifying that some known mathematical elements are
...
bestowed on Robert W Floyd, for, among other things, being discoverer of a
...
longer demanded attention.  It largely disappeared from the computer
...
\inductioncase{Inductive step} ($n\geq 0$):  Now we stare at the gap between
...
\begin{definition}\label{meval-def}
  The \term{evaluation function}, $\text{eval}: \aexp \times \integers \to
...
Structural induction then goes beyond natural number counting by
offering a simple, natural approach to proving things about recursive
data types and recursive computation.  This makes it a technique every
computer scientist should embrace.
