...
Although we will wind up with an some elegant and efficient
algorithms, the aim of this section is to illustrate how to give
simple recursive definitions of a data type and functions on the data
type, along with correctness proofs
...
\end{proof}
\end{corollary}
\footnote{The inequality~\ref{Tleq1LR} is actually an equality,
  see~\ref{sT1sl} below.}
...
of the size of the set of values.  Namely, define a tree to be \term{fully
...
subtree will be the same.  In \term{AVL trees} this condition is
...
makes $N$ a Search tree whose branches are AVL trees and whose labels
are $r$ along with the labels of $T$.  Now if the depth of $S$ is at
most one greater than the depth of $B$, then $N$ will be the desired
AVL tree.
...
or $d+1$ with at least one of the with depth $d+1$.  Let $U$ be the
branch furthest from the middle.  This is illustrated in
Figure~\ref{rotate1}.
...
The simplest case of rotation is when $\dpth{R} = d$.  In that case we
define the rotation of $N$ to be the tree $X$ shown in
Figure~\ref{rotate2}.
...
the same label as $N$.  We leave it to the reader to check that $X$ is
an AVL tree and $\nlbl{X} = \nlbl{N}$.  Moreover, $X$ has very nearly
the same subtrees as $N$.  In particular,
\[
\subbrn{X} = \set{X,Y} \union (subbrn{N} - \set{N,S}).
\]

There are a couple of similar rotations that work in the case that
$\dpth{R} = d+1$.  We'll leave those as exercises.  As a consequence, we have:
...
then
\[
    \nums{\avlrot{(B,r,S)} = \nums{B} \union \set{r} \union \nums{S},
\]
\[
\dpth{\avlrot{(B,r,S)}} = \dpth{B}+1,
\]
...
handled: because sets of values can be any size, but the size of a
recursive tree is always odd.  We handle this by allowing
\emph{leaves} to be labelled with one \emph{or two} values, with both
values obeying the labelling rules for AVL trees.
...
$\avlnsrt{(T,r)}$ will be leaf labelled with the set of two values
$\set{r,\nlbl{(T)}}$.
...
same except for at most $5 \log_2(\sz{(T)})$ subtrees, reflecting the
fact that the time for an insertion, like the time for a search, is
exponentially smaller than the size of the data set.

\begin{theorem}\label{thm:correct-avl-insert}
Suppose $T$ is an AVL tree and $r \in reals$.
Then $\avlnsrt{((T,r))}$ is an AVL tree, and
\[
\nums{\avlnsrt{(T,r)}} = \set{r} \union \nums{T},
\]
\[
\dpth{T} \leq \dpth{\avlnsrt{(T,r)}} \leq \dpth{T}+1,
\]
and there are two size-($5 \log_2(\sz{(T)})$ sets $S_{\text{new}}$,
