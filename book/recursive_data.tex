...
are finite, so
\[
\text{size}(T) \leq 1 + \text{size}(L) + \text{size}(R)
\]
is also finite.
\end{proof}
\end{corollary}

Now it is initially surprising, very useful, and cool fact that the
...
\caption{Tree before rotation.  $T_1,T_3,T_5$ depth $d$, $T_7$ depth $d$ or $d+1$}
