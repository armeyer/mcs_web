...
has ever been recorded; while many \IQ's of over 150 have been
...
\IQ\ is never negative.  We can't get derive smaller fractions using
\emph{only} the facts that the mean of a nonnegative random variable
is 100, because there are nonnegative random variables with mean 100
that achieve these fractions.  For example, if we choose a random
variable equal to 300 with probability 1/3 and 0 with probability 2/3,
then its mean is 100, and the probability of a value of 300 or more
really is 1/3.
...
\term{Chebyshev's Theorem}\footnote{There a Chebyshev Theorems in
...
$\variance{R+S} = \expect{(R+S)^2}$, so we need only prove
...
$S_n$ is binomially distributed.  Equation~\eqref{p1p}, combined with
the fact that $p(1-p)$ is maximized when $p = 1-p$, that is, when
$p=1/2$ (check for yourself!), gives
...
the same birthday will be within 7 of 12.23, that is, the number of
pairs will be between 6 and 19.
...
people.  Like all major web services, Fussbook has a \idx{load
  balancing} problem: it receives lots forum posts that computer
...
Specifically, Fussbook receives 24,000 forum posts every 10
minutes.  Each post is assigned to one of $m$ computers for
processing, and each computer works sequentially through its assigned
tasks.  Processing an average post takes a computer $1/4$ second.
Some posts, such as pointless grammar critiques and snide witticisms,
are easier.  But the most protracted harangues require 1 full second.
...
fluctuations in task length and imperfect load balance.  But would 11
be enough? \dots or 15, 20, 100?  We'll answer that question with a
...
makes it very unlikely that any server is overloaded in a given ten
minute period.  Measuring workload in seconds, this means a server is
overloaded when it is assigned more than 600 units of work.

To begin, let's find the probability that the first server is
overloaded.  Letting $T$ be the number of work units assigned to the
first server, this means we want an upper bound on $\prob{T \geq
...
$T = \sum_{i = 1}^n T_i$ is the total amount of work assigned to the
...
than 1 second to process; that's why we chose work units to be
seconds.
...
the red first appears (check it yourself).
