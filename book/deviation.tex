...
a sea wall, you need to know how strong to make it to likely withstand
...
\term{Deviation from the mean} is the focus of this chapter.
...
It is an almost trivial result by itelf, but it actually leads fairly
...
Hat-Check problem where the probability of probability everyone gets
...
failure,, and a similar approach works $C^2$.  Namely, the expected
...
If failures occur with probability $p$ indendently at each step, and
...
\begin{theorem}\label{thm:variance_additivity}[\idx{Pairwise Independent Additivity} of
  Variance]
...
with parameters $n$ and $p$, where we can choose $n$, but $p$ in unknown.
...
this example using only the \idx{variance} has the virtue of
illustrating an approach to estimation that is applicable to arbitrary
random variables, not just binomial variables, and it did lead to a
...
birthdays, we have $\expect{D} < 12.23$ and $\variance{D} > 12.22 (1-
1/365) >12.19$.  So by Chebyshev's Theorem
\[
\prob{\abs{D - 12.23} \geq x} < \frac{12.19}{x^2}.
\]
...
\idx{Pick-4} is a lottery game where you pay \$1 to pick a 4-digit
...
\subsection{The \idx{St. Petersburg Paradox}}
...
There is a notorious gambling strategy allegedly used in against the
...
the $i$th spin.  So $C_i = 2^i$ when red comes up for the first time
on the $i$th spin, and $C_i = -2^i$, when the first red spin comes up
after the $i$th spin.  We can define $C_i$ to be 0 at if the first red
...
won is
\[
-10 \cdot (1 + 2 + 2^2 + \dots + 2^{i - 1}) + 10 \cdot 2^i   = 10.
\]
...
\expect{C}
  ...
  &  = 10 \cdot \sum_{i  \in integers^+} 2^{-i} = 10.
\end{align*}
So sure enough,
...
\begin{falseclm*}
\[
\Expect{\sum_{i \in \integers^+} C_i} = \sum_{i = 1}^\infty \expect{C_i}.
\]
\end{falseclm*}
...
\begin{align*}
\expect{\abs{C_i}}
  & = (\abs{10} \cdot 2^i) \cdot \prob{\text{1st red in $i$th spin}}\\
  &\quad  + (|-10 \cdot 2^i|) \cdot \prob{\text{1st red after $i$th spin}}\\
  &\quad  + 0 \cdot \prob{\text{1st red before the $i$th spin}}\\
  & = (10 \cdot 2^i) \cdot 2^{-(i-1))}
       + (10 \cdot 2^i) \cdot 2^{-(i-1))}
       + 0 = 10,
\end{align*}
...
finally winning \$10 ---giving you a percentage profit of 0.

So yes, probability theory leads to the absurd conclusion that, even
