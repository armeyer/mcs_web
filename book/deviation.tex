...
conclusion; in fact, no \IQ\ of over 300 has ever been recorded.  By
the same logic, we can also conclude that at most 2/3 of the
...
pairs will be between 6 and 20.
...
Like all major web services, Fussbook has a \idx{load balancing}
problem.  Specifically, Fussbook receives 24,000 forum posts every 10
...
makes it very unlikely that any server is overloaded with more than
ten minutes of work in a ten minute period.

To begin, let's find the probability that the first server is
overloaded.  A server is overloaded when it is assigned more than 600
seconds of work.  Letting $T$ be the number of seconds of work
assigned to the first server, this means we want an upper bound on
$\prob{T \geq 600}$.  Let $T_i$ be the number of seconds that the
first server spends on the $i$th task: then $T_i$ is zero if the task
is assigned to another machine, and otherwise $T_i$ is the length of
the task.  So $T = \sum_{i = 1}^n T_i$ is the total length of tasks
