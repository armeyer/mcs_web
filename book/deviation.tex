...
for dependent variables.  For example, if $R_1=R_2$, then
equation~\eqref{vR+R} becomes $\variance{R_1 + R_1} = \variance{R_1} +
\variance{R_1}$.  By the Square Multiple Rule,
Theorem~\ref{var.const}, this holds iff $4 \variance{R_1} = 2
\variance{R_1}$, which implies that $\variance{R_1}=0$.  But
$\variance{R_1}>0$ unless $R_1$ is certain to be constant.
