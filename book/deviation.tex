...
The restate of~(\ref{chebE2}) for $\alpha=2$ is known as \term{Chebyshev's
  Theorem}.
...
canceling by taking an absolute value.\fi
%
...
\begin{proof}
  ...
  values 0 and 1, $B^2 = B^2$.  So Lemma~\ref{bernoulli-variance} follows
...
If a process fails during any given hour with probability $p$, independent
of what happened previously, then we saw in
section~\ref{mean_time_to_failure_subsec} that the mean time to the first
failure is $1/p$.  What about the variance?  That is, let $C$ be the hour
of the first failure, so $\pr{C=i} = (1-p)^{i-1}p$.  What is
$\variance{C}$?

Now
\begin{equation}\label{varCEC2}
\variance{C} = \expectsq{C} - (1/p)^2
\end{equation}
by Lemma~\ref{alt:var}, so all we need is $\expectsq{C}$:
\begin{align}
\expectsq{C}
   & \eqdef \sum_{i\geq 1} i^2(1-p)^{i-1}p \label{var_sum_time_to_fail}\\
   & = p\sum_{i\geq 1} i^2x^{i-1} & \text{(where $x=1-p$)}\label{time_to_fail_gen_func}.
\end{align}
But the generating function for the nonnegative squares is
$x(1+x)/(1-x)^3$ by~\eqref{squares_gen_func}, so the generating function
for the sum in~\eqref{time_to_fail_gen_func} is $(1+x)/(1-x)^3$, giving
\begin{align}
\expectsq{C} & = p\frac{(1+x)}{(1-x)^3} & \text{(where $x=1-p$)}\\
             & = p\frac{2+p}{p^3}
             & = \frac{1-p}{p} + \frac{1}{p^2}\label{plus1p2},
\end{align}
Finally, combining~\eqref{varCEC2} and~\eqref{plus1p2},
\begin{equation}\label{var_time_to_fail}
\variance{C} = \frac{1-p}{p} \,.
\end{equation}

It's great to be able apply our generating function expertise to knock off
equation~\eqref{var_time_to_fail} mechanically from the definition of
variance, but there's a more memorable alternative.  In
section~\ref{mean_time_to_failure_subsec} we used conditional probability
...
value of $(C+1)^2$.  So
\begin{align}
\expectsq{C} & = p\cdot 1^2 + (1-p)\expectsq{C+1}\\
             & = p + (1-p) \paren{\expectsq{C} + 2\expect{C} +1}\\
             & = TBA
\end{align}
...
\subsection{Expectation Squared [Optional]}
...
\expect{(R_1+R_2+\cdots+R_n)^2}
   & = \expect{\sum_{1\leq i,j \leq n} R_iR_j} & \text{since $\expect{R_i=0}$}\\
...
In Chapter~\ref{}, we used bounds on the binomial distribution to determine
...
The setup is the same as in Notes 12: \fi

Suppose we wanted to estimate the fraction of the Massachusetts voters who
favor Martha Coakley over everyone else in the coming Democratic primary
election to fill Senator Edward Kennedy's seat.
...
voters\footnote{We're choosing a random voter $n$ times \emph{with
  ...
  in accuracy.}  and count the fraction who favor Coakley.  That is, we
...
Coakley.  Since our choices are made independently, the $K_i$'s are
...
who favor Coakley.  We would expect that $S_n/n$ should
be something like $p$.
%Note that
%\[
...
\subsection{Sampling}
Suppose we want our estimate to be within $0.04$ of the Coakley favoring
...
A more exact calculation of the tail of this binomial distribution shows
that the above sample size is about four times larger than necessary, but
it is nevertheless a feasible sample size.  The fact that the sample size
derived using \idx{Chebyshev's Theorem} was unduly pessimistic should not
be surprising.  After all, in applying the Chebyshev Theorem, we only used
the variance of $S_n$.  It makes sense that more detailed information
about the distribution leads to better bounds.

But working through this example using only the \idx{variance} illustrates
an approach to estimation that is applicable to arbitrary random
variables, not just binomial variables.

\subsection{Matching Birthdays}
...
\idx{matching birthdays} for different pairs of students are not
\idx{mutually independent} events.  For example, knowing that Alice and
Bob have matching birthdays, and also that Ted and Alice have matching
birthdays obviously implies that Bob and Ted have matching birthdays.  On
the other hand, knowing that Alice and Bob have matching birthdays tells
us nothing about whether Alice and Carol have matching birthdays, namely,
these two events really are independent.  So even though the events that
various pairs of students have matching birthdays are not mutually
independent, indeed not even three-way independent, they are
\index{pairwise independent} \emph{pairwise} independent.
...
did for voter preference.  Namely, let $B_1,B_2,\dots,B_n$ be the
birthdays of $n$ independently chosen people, and let $E_{i,j}$ be the
indicator variable for the event that the $i$th and $j$th people chosen
have the same birthdays, that is, the event $[B_i = B_j]$.  According to
our probabillity model, the $B_i$'s are mutually independent variables,
and hence the $E_{i,j}$'s are \emph{pairwise} independent variables, which
is all we will need.  Also, the expectations of $E_{i,j}$ for $i \neq j$
...
the voting population who prefer Coakley.  \iffalse Estimates of the
binomial distribution show that a sample size around 664 would do.\fi

Notice that the actual size of the voting population was never considered
because \emph{it did not matter}.  People who have not studied probability
theory often insist that the \idx{population size} should matter.  But our
analysis shows that polling a little over 3000 people people is always
sufficient, whether there are ten thousand, or million, or billion \dots
voters.  You should think about an intuitive explanation that might
persuade someone who thinks population size matters.

Now suppose a pollster actually takes a sample of 3,125 random voters to
estimate the fraction of voters who prefer Coakley, and the pollster finds
that 1250 of them prefer Coakley.  It's tempting, \textbf{but sloppy}, to
say that this means:
\begin{falseclm*}
  With probability 0.95, the fraction, $p$, of voters who prefer Coakley
  is $1250/3125 \pm 0.04$.  Since $1250/3125 -0.04 > 1/3$, there is a 95\%
  chance that more than a third of the voters prefer Coakley to all other
  candidates.
\end{falseclm*}
...
actual fraction, $p$, of voters favoring Coakley is more than 1/3.  But $p$
...
who prefer Coakley is $1250/3125 \pm 0.04$.
\end{quote}
...
%Include example --maybe journal editors class problem
\iffalse
