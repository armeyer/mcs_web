...
\begin{theorem}\label{var.const}[Square Multiple Rule for Variance]
...
An independence condition is necessary.  If we ignored independence,
then we would conclude that $\variance{R + R} = \variance{R} +
\variance{R}$.  However, by the Square Multiple Rule for Variance,
Theorem~\ref{var.const}, the left side is equal to $4 \variance{R}$,
whereas the right side is $2 \variance{R}$.  This implies that
$\variance{R}=0$, which, by the Lemma above, \iffalse
Lemma~\ref{zvar}\fi essentially only holds if $R$ is constant.
