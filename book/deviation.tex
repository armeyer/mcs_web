...
Unlike the situation with voter preferences, having \idx{matching
  ...
  independent} events.\iffalse---as explained in
...
\index{pairwise independent} \emph{pairwise} independent.  This will
...
Specifically, Fussbook receives 24,000 forum posts in every 10 minutes
...
case of an astonishingly general one: \iffalse worked out in
...
\subsection{\idx{Martingales}}\label{sec:martingale}

%MAYBE CUT IBN FAVOR OF DISCUSSION OF INFINITE LINEARITY OF EXPECTATION IN RV CHAPTER
...
There is a notorious gambling strategy that is widely touted as a
``sure'' way to beat the casino: bet $\$10$ on red, and keep doubling
the bet until a red comes up.  This betting strategy is known as the
\term{Martingale}, and it guarantees that a player will leave the game
as a net winner of $\$10$ as soon as the red first appears (check it
yourself).
...
But as long as there is a fixed, positive probability of red appearing
on each spin of the wheel, it's \emph{certain} that red will
eventually come up, so you can be certain of leaving the casino having
won $\$10$.  Probability theory really implies that even with the odds
heavily against you, you're certain to win!
...
order to keep betting until the wheel finally spins red.  Then
$expect{L} = \infty$ (see Problem~\ref{FP_martingale}).
...
(Problem~\ref{FP_martingale})\fi to verify that, if you have only a
finite amount of money when you start following the bet doubling
strategy, then your expected win comes out sensibly: it will be zero
against a fair wheel and be negative when the wheel is biased against
you.
...
%\pinput{PS_martingale_bounded}  %WRITEUP NEEDED & fix ref above

\examproblems
\pinput{FP_infinite_repeat}
\pinput{FP_martingale}  %infinite expected stake
\end{problems}
