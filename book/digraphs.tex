...
\begin{equation}\label{G+def}
a \mrel{G^+} b \eqdef \mbox{there is a positive length walk in $G$ from $u$ to $v$}.
\end{equation}
...
\begin{proof}
We know that $D^+$ is transitive.  Also, if there was a positive
length walk from a vertex to itself, then by
Lemma~\ref{shortestclosedwalk_lem}, that vertex is on a positive
length cycle, contradicting the fact that $D$ is acyclic.  This means
...
pieces are called the blocks of the partition
  (see Definition~\ref{def:partition}).} of $A$ into sets $A_0, A_1,\dots,$ such
...
A \term{partition} of a set~$A$ is a set $\mathcal{P}$ of nonempty
...
\item if $B$ and $B'$ are different blocks, then $B \intersect B' =
  \emptyset$, and
\item $A= \lgunion_{B\in \mathcal{P}} B$.
\end{itemize}
\end{definition}

For example, one possible partition of $\set{a, b, c, d, e}$ into
three blocks is
\[
B_1 = \set{a, c} \qquad  B_2 = \set{b, e}  \qquad   B_3 = \set{d}.
\]
