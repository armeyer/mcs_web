...
a walk from $u$ to $w$, both walk relations have a relational property
...
prerequisite of $t$.  of course, in order to take subject $t$, you not
only have to take subject $s$ first, but you also have to
take all the prerequisites of $s$, as well as any prerequisites of
these prerequisites, and so on.  We can state this precisely in terms
of the positive path relation: if $D$ is the direct prerequisite
...
example, in analyzing concurrency control; we'll expand on this in
...
positive length cycle through that vertex.
\end{lemma}
The proof is essentially the same as for
Theorem~\ref{shortestwalk_thm} (see
Problem~\ref{PS_shortest_directed_closed_walk}).  This implies that a
graph $D$ is a DAG iff it has no positive length walk from any vertex
to itself.  This relational property of $D^+$ is called
\term{irreflexivity}.
...
length two path, there is an edge.  This implies by induction that
where there is a path of any positive length, there is an edge (see
Problem~\ref{PS_transitive_closure_proof}), namely:
...
graph is a DAG, it cannot have two vertices with positive length paths
in both directions between them.  This relational property of a
positive path relation is called \term{asymmetry}.
...
The following lemma gives another characterizations of weak partial
...
\begin{definition}\label{relation-isomorphism}
  ...
  a bijection $f:A \to D$, such that for all $a,a' \in A$,
  \[
...
we will prove that every \emph{finite} partial order has a topological sort.
...
\caption{A parallel schedule for the tasks-in-getting-dressed partial order in
...
shows the critical path for the getting-dressed partial order.
...
thing about partial orders is that this is always possible!  In other words,
for any partial order, there is a legal parallel schedule that runs in $t$
...
subsets of $A$ called the \term{blocks}\footnote{We think it would
  ...
  ``blocks'' is the standard terminolgy.} of the partition, such that
\begin{itemize}
...
A \term{parallel schedule} for a strict partial order, $\prec$, on a
...
\subsection{Equivalance Classes}
...
\end{equation*}
Notice that 7, 12, 17, etc., all have the smae equivalence class; that
...
\item[Weak Partial Order] $R$ is a \term{weak partial order} iff it is
  transitive and anti-symmetric and reflexive iff it is the walk
  relation of a DAG.
