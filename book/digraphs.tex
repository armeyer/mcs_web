...
about following \term{paths} through the graph.  Here is the precise
definition:
...
technically, paths only have points, not edges.  So to be precise,
we'll say a path \term{traverses} an edge $\diredge{a}{b}$ when $a$
and $b$ are consecutive vertices in the path.
...
question of finding the \emph{smallest} DAG with the same path
relation.  This DAG turns out to be unique and easy to find (see
problem~\ref{CP_covering_edges}).
