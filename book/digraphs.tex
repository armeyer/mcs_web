...
$a$ can ever be scheduled must be at least as long as any chain that
ends at $a$.  A \emph{longest} chain ending at $a$ is called a
\term{critical path} to $a$, and the length of the critical path is
called the \term{depth} of $a$.  So in any possible parallel schedule,
...
complete all tasks is simply one plus the length of a critical path:

\begin{corollary}\label{cor:critical-path-time}
Parallel time = one plus length of critical path.
\end{corollary}
...
In a DAG, $D$, if the length of the longest chain is $t$, then
$\vertices{D}$ can be partitioned into $t+1$ antichains.
\end{corollary}
...
$n$ vertices must have either a chain of length $t$ or an
antichain of size at least $n / t$.
\end{lemma}

\begin{proof}
Assume there is no chain of length $t$, that is, the length of the
longest chain is less than $t$.  Then by
