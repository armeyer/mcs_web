...
\begin{definition}
When there is a path from vertex $v$ to vertex $w$, we say that $w$ is
...
finding an order for the tasks is known as \emph{topological sorting}.
...
Topological sorts for finite DAG's are easy to construct by starting
...
It is kind of peculiar to use the words ``minimum'' and ``minimal''
just to talk about vertices that start paths.  These words come from
the perspective that regards a vertex as ``smaller'' than any other
vertex it connects to.  We'll explore this way of thinking about DAG's
in the next section.
...
a topological sort picking vertics arbitrarily and simply inserting
them into the list wherever they will fit.\footnote{In fact, the DAG
  doesn't even need to be finite, but you'll be relieved to know we
  have no need to go into this.}
...
\index{end of chain}\emph{end at} its maximum element, that is, at a
vertex that is reachable from all the other vertices in the chain.
\end{definition}
...
If the largest chain in a DAG, $D$, is of size $t$, then
...
between different types of relations.  For instance, we an apply
...
a postive length closed walk that starts at one vertex, goes to the
