...
\begin{definition}\label{graphdef}
  ...
  set,~$\vertices{G}$, called the \emph{vertices} of~$G$, and a set,
  $\edges{G}$, called the \emph{edges}\index{edge (digraph)|textbf} 
of $G$.  An element of
...
\index{directed graph!cycle|textbf}
 is a closed walk whose vertices are distinct
...
is a directed graph with no positive length cycles.
\end{definition}
...
\index{end of chain}\emph{end at} its maximum element, the vertex that
...
A \emph{partition} of a set~$A$ is a set of nonempty subsets of $A$
...
\begin{editingnotes}
b\chapter*{Well-founded partial orders}
omitted
\end{editingnotes}
