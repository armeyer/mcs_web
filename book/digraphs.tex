...
Chapter~\ref{comm_net_chap}, we'll use digraphs are used to describe
...
with $\walkv{f}$ and continues with $\walkv{r}$.\footnote{It's
  tempting say the \emph{merge} is the \idx{concatenation} of the two
  walks, but that wouldn't quite be right because if the walks were
  concatenated, the vertex $v$ would appear twice in a row where the
  walks meet.}  Two walks can only be merged if the first ends with
the same vertex, $v$, that the second one starts with.  Sometimes its
useful to name the node $v$ where the walks merge; we'll use the
...
\]
\end{lemma}
In the next section we'll get useful mileage out of walking this way.
...
In any digraph, if there is a walk of length $k$ that starts at vertex
$u$ and ends at vertex, $v$, then there is a path of length \emph{at
  most} $k$ that starts and ends with these vertices.  Moreover, the
shortest walk between a pair of vertices is, in fact, a path.
\end{corollary}
\fi
...
\begin{definition}
  ...
  length of a shortest walk from $u$ to $v$.
\end{definition}
...
to $v$ is typically different from the the distance from $v$ to $u$.
...
length-2 walks between verices $a$ and $c$ in the graph $H$, namely
\begin{align*}
...
$n$-vertix graph, $G$, is to compute the successive powers of $A_G$
...
particular vertex to another.  So for any digraph, $G$, we interested
in a binary relation, $G^*$, called the \term{path relation} on
$\vertices{G}$ where
...
\subsection{Composition of path relations}\label{relation_compose_subsec}
...
is the \term{length-$n$ path relation}:
\[
...
identity relation $\ident{\vertices{G}}$ on the set of vertices.  That
is, $u \mrel{\ident{\vertices{G}}} v$ iff $u = v$.  So now we
have\footnote{Equation~\eqref{g*union} involves a harmless abuse of
  notation: we should have written
\[
...
\begin{definition}
A \term{closed walk} in a digraph is a walk that begins and ends at
the same vertex.  A \term{cycle} in a digraph is a closed walk whose
...
The consequences of having a positive length cycle in a prerequisite
graph like this would have dire consequences on the time it takes to
graduate.
