...
\emph{directed edges} or \term{arrows}, so the digraph in
...
vertices on it, and we will describe walks that way whenever it's
convenient.  For example, for the graph in Figure~\ref{fig:4N6E},
\begin{itemize}

\item
$a$, $b$, $c$, $b$, $d$ is a length-4 walk,

\item
$a$, $b$, $d$ is a length-3 is a path,

\item
$d$, $c$, $b$, $c$, $b$, $d$ is a length-5 closed walk,

\item
$b$, $d$, $c$, $b$ is a length-3 cycle,

\item $b$, $c$, $b$ is a length-2 cycle, and

\item $c$, $b$, $a$, $d$ is \emph{not} a walk.  A walk is not allowed
  to follow edges in the wrong direction.  For this sequence to
  describe a walk, $\diredge{b}{a}$ would have to be an edge of the
  graph.  This edge is the reversal of an edge of the graph, but it is
  not itself an edge of the graph.
\end{itemize}

Length-1 cycles are also possible.  The graph in Figure~\ref{fig:4N6E}
...
Length-1 cycles are sometimes called \term{self-loops}.
...
\begin{definition}
A \term{directed acyclic graph (DAG)} is a directed graph
...
Problem~\ref{PS_shortest_closed_walk}).
...
\begin{definition}
A relation that is transitive and asymmetric is called a \term{strict partial order}.
\end{definition}
...
A strict partial may be the positive path relation of different DAG's.  
\iffalse
...
with any other DAG with the same path relation.  \fi
...
\emph{path-total orders}.\footnote{Being a
  path-total partial order is a much stronger condition than being a
  partial order that is a total relation.  For example, any weak partial
  order such as $\subseteq$ is a total relation but generally won't be path-total.

  Path-total partial orders are conventially just referred to as
  ``total.''  But this terminology conflicts with the definition of
  ``total relation,'' and it regularly confuses students.  So we chose the
  terminology ``path-total'' to avoid the confusion.}
...
\begin{lemma}\label{path_total_lem} In a path-total digraph, for any
  finite, nonempty set of vertices, there is a directed path going through
  exactly these vertices.
\end{lemma}
We'll skip the proof which follows routinely by induction on the
cardinality of the set of vertices.
...
respecting the dependency constraints.  This amounts to finding a path-total
order that is consistent with the partial order.  This task of finding a
path-total ordering that is consistent with a partial order is known as
...
To construct a path-total ordering for getting dressed, we pick one of these
...
\begin{problems}
\homeworkproblems
\pinput{PS_equivalence_same_property}
\end{problems}
...
For any finite, nonempty set of vertices of $G$, there is a directed path
