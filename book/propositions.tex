...
because the English language is riddled with ambiguities such as:
%
\begin{itemize}
...
problems we come up with, but not all, then does the third sentence mean
you can get an \emph{A} for the course?  And if you can't solve a single
one of the problems, does it mean you can't get an \emph{A}?  Finally,
does the last sentence mean that all Americans have the same dream---say
of owning a house---or might different Americans have different
dreams---say, Eric dreams of designing a ``killer'' software application,
Tom of being a tennis champion, Albert of being able to sing?
...
understanding is that these variables, can take on only the values
\true~(true) and \false~(false).  These true/false variables are called
\term{propositional variables} and also \term{Boolean variables} after
their inventor, the nineteenth century mathematician George---you guessed
it---Boole.

\subsection{\QNOT, \QAND, and \QOR}

Mathematicians use the words $\QNOT$, $\QAND$, and $\QOR$ for operations
that change or combine propositions.  Their precise definitions can be
specified by \term{truth tables}.  For example, if $P$ is a proposition,
...
for each possible true/false setting of the variables.  For example, the
...
should use \term{exclusive-or} ($\QXOR$):
%
\[\begin{array}{cc|c}
P & Q & P \QXOR Q \\ \hline
\true & \true & \false \\
...
One of our initial examples demonstrates an even stranger side of
implications.
\begin{center}
...
their meaning is easier to remember.  For the most part, we will stick to
using the words.
...
propositional logic.  Let $P$ be the proposition ``I am hungry'', and let
$Q$ be ``I am grumpy''.  The first sentence says ``$P \QIMPLIES Q$'' and
the second says ``$\QNOT(Q) \QIMP \QNOT(P)$'' and is called the
\term{contrapositive} of the implication ``$P \QIMP Q$.''  We can compare
...
the same.  Statements like these which always have the same truth value
are called \term{logically equivalent}.  So we've just shown that an
implication and its contrapositive are equivalent---the two are just
...
\]
%
Since the column of truth values for $P \QIMP Q$ is not the same as the
column for $Q \QIMP P$, we have verified that an implication is \emph{not}
necessarily equivalent to its converse.
...
science.  The kind of expression simplification illustrated above in a
program can increase execution speed, in addition to making a program
easier to read and understand.  In chip design, minimizing the number of
gates on a chip can have enormous payoff: a chip with fewer gates is
smaller, consumes less power, has a lower defect rate, and is cheaper to
manufacture.  But minimizing gates corresponds to another instance of
expression simplification because digital circuits can be described by
logical expressions (see Problems~\ref{CP_binary_adder_logic}
and~\ref{PS_faster_adder_logic}), and minimizing the expressions
corresponds to reducing the number of gates in the circuit.
