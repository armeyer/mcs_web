...
B) \QOR (A \QAND C)$ is called the \term{distributive law} of \QAND\
...
\QNOT(A \QAND B) &\iff \bar{A} \QOR \bar{B} & \text{(DeMorgan for \QAND)} \\
\QNOT(A \QOR B) &\iff \bar{A} \QAND \bar{B} & \text{(DeMorgan for \QOR)}\label{DeMQOR} 
\end{align}
