...
be stated using mathematically precise language.  Here are some simple
examples.
...
\subsection{Propositions from Propositions}
...
To experiment with this definition, we're going to examine a famous
unsolved problem in mathematics called the Riemann
Hypothesis.\footnote{We'll explain a little about the Riemann
  Hypothesis in a later chapter (see Definition~\ref{RiemannHyp}).
  Whether or not the Riemann Hypothesis holds is generally considered
  to be one of the most important unolved problems is mathematics.}
For now, it doesn't matter what the Riemann Hypothesis happens to be;
all that's important in the following example is it is a proposition
and no one knows whether it is true or false.
...
number $x$.''
\end{center}
Is this if-then proposition true or false?  The answer (that a
mathematician would give, anyway) is that it's true!  That's because
it means that $P \implies Q$, where the if-part, $P$,
called the\term{hypothesis}, is ``the Riemann Hypothesis is true,''
and the then-part, $Q$ called the \term{conclusion}, is ``$x^2 \geq 0$
for every real number $x$.''  Since the conclusion is definitely true,
we're on either line~(tt) or line~(ft) of the truth table.  Either
way, the proposition as a whole is \textit{true}.
...
\end{center}
(Don't take this as an insult to your understanding; we just need to
figure out whether this proposition is true or false.)  Curiously, the
answer has \emph{nothing} to do with whether or not you know anything
about the Chebyshev bound.  Pigs do not fly, so we're on either line
(ft) or line (ff) of the truth table.  In both cases, the proposition
is \textit{true}!

Sometimes it's worth emphasizing that an implication is true because
its hypothesis is false, and we do this by saying it is
\term{vacuously true}.
...
But words such as ``$\QOR$'' and ``$\QIMP$'' generally serve just
...
P & Q &
    (P &\QIMP& Q) &\underline{\QAND}&  (Q &\QIMP& P) & Q &\underline{\QIFF}& P \\
\hline
...
It's worth noticing that however the phrase is meant to be quantified,
it appears as part of a larger ``if \dots, then'' statement, which is
typical.  Quantified statements themselves define predicates or
propositions and can be combined with $\QAND$, $\QOR$, $\QIMP$, \etc,
