...
\chapter{Logical Formulas}\label{propform_chap}
...
There is a subtlety in the truth for ``$P \QOR Q$'':
%
\[
\begin{array}{cc|c}
P & Q & P \QOR Q \\ \hline
...
This equivalence of $A \QAND (B \QOR C)$ and $(A \QAND B) \QOR (A \QAND
C)$ is called \term{\QAND/\QOR-distributivity} because of its obvious
resemblance to the distributivity of multiplication over addition in
arithmetic. \footnote{There is also \term{\QOR/\QAND-distributivity}
in propositional logic in striking contrast to arithmetic.}

Applying the same reaasoning to the \false\ entries of a truth table
yields a \term{conjunctive form} for any formula, namely a \QAND\
...
``$\iff$'' between equivalent formulas.  These axioms are
...
\begin{align}
A \QAND (B \QOR C) & \iff (A \QAND B) \QOR (A \QAND C)
       & \text{(\QAND/\QOR-distributivity)}
\end{align}
...
\QNOT(A \QAND B) &\iff \bar{A} \QOR \bar{B} & \text{(DeMorgan-\QAND)} \\
\QNOT(A \QOR B) &\iff \bar{A} \QAND \bar{B} & \text{(DeMorgan-\QOR)}\label{DeMQOR} 
\end{align}
...
duplicates, finally yields a unique sorted CNF:
\begin{align*}
(A \QAND \bar{B} \QAND \bar{C})
    & \QOR (\bar{A} \QAND B \QAND C) \QOR (\bar{A} \QAND B \QAND \bar{C}) \QOR\\
    &(\bar{A} \QAND \bar{B} \QAND C) \QOR (\bar{A} \QAND \bar{B} \QAND \bar{C})
\end{align*}

You're probably bored enough by this long series of formulas that you
won't be interested in questioning what the theorem they are intended
to explain:

\begin{theorem}\label{completeDNF}
Any propositional formula can be transformed into \idx{disjunctive
normal form} using the equivalences named above.
\end{theorem}
...
form over the set of variables that appear in the terms.  Usw
commutativity to sort the variables and \QAND-terms so they all
appear in some standard order.  We claim the formulas are equivalent
iff they have the same sorted disjunctive normal form.  This is
obvious if they do have the same disjunctive normal form.  But
conversely, the way we read off a disjunctive normal form from a truth
table shows that two different sorted DNF's over the same set of
variables correspond to different truth tables and hence to
inequivalent formulas.  This proves:
...
equivalent using the equivalence axioms named above.
\end{theorem}
...
Determining whether or not propositions are satisfiable is not so
easy.  For example, how about this one?
%
\[
...
\]
The general problem of deciding whether a proposition
is \idx{satisfiable} is called \term{SAT}.  One approach to SAT is to
construct a truth table and check whether or not a $\true$ ever
appears, but, as for validity, this approach quickly bogs down as for
formulas with many variables because because truth tables
grow \idx{exponentially} with the number of variables.
...
number of steps that \index{polynomial growth}(grows
...
Decrypting coded messages would also become an easy task (for most
codes).  Online financial transactions would be insecure and secret
communications could be read by everyone.  In later chapters we'll see
some explanations of why this would happen.
...
The ``for all'' notation, $\forall$, introduced in
Section~\ref{prop_sec} has already made an early appearance.
...
\]
is a true statement.  On the other hand, the
predicate
%
\[
\text{``$5x^2 - 7 = 0$''}
...
%
\begin{equation}\label{solvegetA}
  \text{If you can solve any problem we come up with, then you get
  an \emph{A} for the course.}
\end{equation}
%
...
We hope doing this was helpful.
