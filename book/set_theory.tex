...
infinite sets.  Their study led to the discovery of widespread logical
...
called the Axioms of \idx{Zermelo-Fraenkel Set Theory|textbf} with Choice (\idx{ZFC|see{Zermello-Fraenkel Set Theory}}).
...
We're \emph{not} going to be studying the axioms of ZFC \idx{ZFC
  axioms}in this text, but we thought you might like to see them---and
while you're at it, get some practice reading quantified formulas:

\begin{description}

\item[\index{ZFC axioms!axiom of extensionality}\emph{Extensionality}.]
  Two sets are equal if they have the same members.
\[
(\forall z.\; z \in x \QIFF z \in y) \QIMPLIES x = y.
\]

\item[\index{ZFC axioms!axiom of pairing}\emph{Pairing}.] For any two sets $x$ and $y$, there is
...
\item[\index{ZFC axioms!power set axiom}\emph{Power Set}.]  All the
  subsets of a set form another set:
\[
\forall x.\; \exists p.\; \forall u.\: u \subseteq x \QIFF u \in p.
\]

\item[\index{ZFC axioms!replacement axiom}\emph{Replacement}.]
...
\item[\index{ZFC axioms!foundation axiom}\emph{Foundation}.] 
...
\item[\index{ZFC axioms!axiom of choice}\emph{Choice}.]  Given a set, $s$, whose
...
\item The \idx{ZFC axioms} weren't etched in stone by God.  Instead,
  ...
  \index{ZFC axioms!axiom of choice}\emph{axiom of choice}, a solid
...
\item But even if we use more or different axioms about sets, there
  ...
  that, assuming that an axiom system like ZFC is
