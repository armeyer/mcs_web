...
Schr\"oder-Bernstein Theorem is that if a set is \emph{not} as big as
...
Now the fundamental thing that logically just can't be done by
...
---written in your favorite programming language such as C++, or Java,
or Python ---a \term{string procedure} when it is applicable to
strings over a standard alphabet ---say the 256 character ASCII
alphabet.  When a string procedure applied to an ASCII string returns
the boolean value \True, we'll say the procedure \term{recognizes} the
string.  If the procedure does anything else ---returns a value other
than \True, aborts with an error, runs forever,\dots ---then it
doesn't recognize the string.

As a simple example, you might think about how to write a string
procedure that recognizes precisely those \term{double letter} ASCII
strings in which every character occurs twice in a row.  For example,
...
only the ASCII characters; they usually are anyway.  When a
string $s \in \asciistr$ is actually the ASCII description of some string
...
think of $P_s$ as the result of compiling $s$.\footnote{The string, $s
  ...
  $P_s$ to \texttt{aabbccdd}.  \iffalse This should result in a
  returned value \True, since \texttt{aabbccdd} is a double letter
...
$P_s$ to be some default string procedure ---say one that always
returns \False.  \iffalse So $s$ is an ill-formed string, $P_S$ will
...
\nohalt \eqdef \set{s \in \asciistr \suchthat P_s \text{ does not recognize } s}.
\end{equation}
\end{definition}
...
\begin{proof}
Namely for any string $s \in \asciistr$, let
...
arbitrary program returns \True, it's easy to show that it's
impossible to have a procedure that is a perfect recognizer for
\emph{any} overall run time property.\footnote{The weasel word
  ``overall'' creeps in here to rule out some run time properties that
  are easy to recognize because they depend only on part of the run
  time behavior.  For example, the set of programs that halt after
  executing at most 100 instructions is recognizable.}
