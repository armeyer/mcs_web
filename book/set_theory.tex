...
were an infinite number of points where they didn't converge.  As a result,
...
infinite sets: he regarded two infinite sets as having the ``same
...
The fundamental thing that logically just can't be done by
...
or no answers, for an infinite number of programs it is supposed to be
able to analyze!
...
only the ASCII characters; they usually are anyway.  When a string $s
...
Focussing just on string procedures, the general \term{halting
  problem} is to decide, given strings $s$ and $t$, whether or not the
procedure $P_s$ halts when applied to $t$.  We'll show that the
general problem can't be solved by showing that a special case can't
be solved, namely, whether or not $P_s$ applied to $s$ halts.  So let's define
