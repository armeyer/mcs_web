...
sequences thereof---infinite sets, all.  Further, do you really want
...
in a bounded universe?  It's pretty convincing and a lot simpler
...
them.  Likewise, an infinite set $A$ should be considered ``as big as'' a set
$B$ when $A \surj B$.  So we could consider $A$ to be
``strictly smaller'' than $B$ when $A$ is \emph{not} ``as big as'' $B$.
This motivates the definition of a $\strict$ relation on sets:
\begin{definition}\label{def:strict}
\[
A \strict B \eqdef \QNOT(A \surj B).
\]
\end{definition}
...
The above proof is known as a ``diagonal argument.''  The diagonal
aspect might not be immediately obvious, so here is another version of
the proof.  Suppose there were a bijection between $\naturals$ and
$\set{0,1}^{\omega}$.  If such a relation existed, we would be able to
display it as a list of the infinite bit strings in some countable
order or another.  Once we'd figured out a viable way to organize the
list, any given string in $\set{0,1}^{\omega}$ would appear in a
finite number of steps, just as any integer you name will show up a
finite number of steps from 0.  This hypotherical list would look
something like the one below, extending to infinity both vertically
and horizontally:


\[
\begin{tabular}{l c c c c c c l}
$A_0 = $ & 1 & 0 & 0 & 0 & 1 & 1 & $\cdots$ \\ $A_1 = $ & 0 & 1 & 1 &
  1 & 0 & 1 & $\cdots$ \\ $A_2 = $ & 1 & 1 & 1 & 1 & 1 & 1 & $\cdots$
  \\ $A_3 = $ & 0 & 1 & 0 & 0 & 1 & 0 & $\cdots$ \\ $A_4 = $ & 0 & 0 &
  1 & 0 & 0 & 0 & $\cdots$ \\ $A_5 = $ & 1 & 0 & 0 & 1 & 1 & 1 &
  $\cdots$ \\ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ &
  $\vdots$ & $\ddots$ \\
\end{Tabular}

But we can show that this list wil never be complete---that is, a
countable list of bit strings, however arranged, will always fail to
include some members of $\set{0,1}^{\omega}$.  Here is why the
diagonal argument has its name: consider the diagonal in our sample
list.

\[
\begin{tabular}{l c c c c c c l}
$A_0 = $ & \textcolor{blue}{1} & 0 & 0 & 0 & 1 & 1 & $\cdots$ \\
$A_1 = $ & 0 & \textcolor{blue}{1} & 1 & 1 & 0 & 1 & $\cdots$ \\
$A_2 = $ & 1 & 1 & \textcolor{blue}{1} & 1 & 1 & 1 & $\cdots$ \\
$A_3 = $ & 0 & 1 & 0 & \textcolor{blue}{0} & 1 & 0 & $\cdots$ \\
$A_4 = $ & 0 & 0 & 1 & 0 & \textcolor{blue}{0} & 0 & $\cdots$ \\
$A_5 = $ & 1 & 0 & 0 & 1 & 1 & \textcolor{blue}{1} & $\cdots$ \\
      & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\ddots$   \\
\end{Tabular}

Now, let's define a sequence $y \in \set{0,1}^{\omega}$ as the
opposite of that diagonal sequence, exchanging all the 1's for 0's and
\emph{vice versa}.  This means that the $n+1^{\textrm{th}}$ term of
$y$ does not equal the $n + 1^{\textrm{th}}$ term of $A_n$ on our list
for all $n \in \naturals$.  But that means that $y$ has at least one
bit different from \emph{every} term on our list---it can never appear!

\[
\begin{tabular}{l c c c c c c l}
 & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ &
  \\ $A_n \neq$ & \textcolor{red}{0} & \textcolor{red}{0} &
  \textcolor{red}{0} & \textcolor{red}{1} & \textcolor{red}{1} &
  \textcolor{red}{0} & $\cdots$ \\
\]

This diagonal set $y$ is analagous to \set{a \in A \suchthat a \notin
  g(a)} in the proof of Theorem~\ref{powbig}.  It shows that any
function arranging the elements of~$\set{0,1}^{\omega}$ into a
countable list will necessarily generate a list that is incomplete.
Elements of the codomain will be omitted from the range of the
function, so~$\qnot (\naturals \surj \set{0,1}^{\omega})$, and~$(\naturals \strict \set{0,1}^{\omega})$.
...
to show that lots of problems logically just can't be solved by
...
\item[\index{Union axiom}\emph{Union}.] The union, $u$, of a collection, $z$, of sets is also a set:
\[
