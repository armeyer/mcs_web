...
the proof.  Suppose we try to make a bijection between $\naturals$ and
$\set{0,1}^{\omega}$.  This bijection could take the format of a list
like the one below, extending infinitely in all directions:
...
Now, consider the diagonal highlighted in blue. Specifically, consider
the binary string defined as not having any of the diagonal bits--that
is, the $k+1^{\textrm{th}}$ bit of this string does not equal the $k +
1^{\textrm{th}}$ bit of $A_k$ on our list for all $k \in \naturals$.  This is a
well-defined number that can be found by an easy procedure. But this
number is defined as having at least one digit from every number that
will ever appear on our list.  In other words:

\[
\begin{tabular}{l c c c c c c l}
 & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ &
  \\ $A_n \neq$ & \textcolor{green}{0} & \textcolor{green}{0} &
  \textcolor{green}{0} & \textcolor{green}{1} & \textcolor{green}{1} &
  \textcolor{green}{0} & $\cdots$ \\
\]

No matter what system we use to arrange the elements of
$\set{0,1}^{\omega}$, there will always be a term that can never
appear on our list!  
