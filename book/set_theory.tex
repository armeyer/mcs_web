...
Part~\ref{surjvsinj}.\ follows from the fact that $R$ is a surjective
function iff $\inv{R}$ is a total, injective relation.
Parts~\ref{bigtrans}.\ follows from the fact that compositions of
surjections are surjections.
...
$\naturals$, of nonnegative integers.  But the set, $\integers$, of
\emph{all} integers is also countable, because they can be listed in
the order,
...
integers, so $(\naturals \cross \naturals)$ is also countable, and
from that it's a small step to reach the conclusion that the set,
$\rationals$, of rational numbers is countable.  This may be a
surprise ---after all, the rationals densely fill up the space between
integers, and for any two, there's another in between, so it might not
seem as though you could write them all out in a list.
Combining this with~\eqref{intlist} we get
...
\item $\integers$
\item $\naturals \cross \naturals$
\item $\integers \cross \integers$
\item $\rationals^+$
\item $\rationals$
\end{itemize}
\end{corollary}
Problem~\ref{CP_rationals_are_countable} illustrates how
Corollary~\ref{countable_examples} can be proved.
...
\[
\power(\naturals) \bij \set{0,1}^{\infty}.
\]
This immediately implies
\begin{corollary}
$\set{0,1}^{\infty}$ is uncountable.
\end{corollary}
...
Granted that towers of larger and larger infinite sets is at best of
only romantic concern to a computer scientist, the \emph{reasoning}
that leads these conclusions plays a central role in the theory of
...
to eatablish lots that things logically just can't be done by
computation, and there is no getting around it.
...
``\idx{Halting Problem}.''  The general ''Halting Problem'' for some
programming language is to recognize, given any program text in that
language that defines a procedure and some values,
whether or not the procedure will finish its computation ---halt
---when it is applied to those values.  We claim there is no program
to do this, and this is a basic example of what programs can't do.

To be precise about this, let's call a programming procedure
---written in your favorite programming language such C++, or Java, or
Python ---a \term{string procedure} when it is applicable to strings
over a standard alphabet ---say the 256 character ASCII alphabet.
When a string procedure, $P$, applied to an ASCII, $s$, returns the
boolean value \True, we'll say that $P$ \term{recognizes} $s$.  $P$
could fail to recognize $s$ by returning some value other than \True,
or not returning any value ---say because it aborts with an error or
runs forever.

As a simple example, you might think about how to write a string
procedure that recognized precisely those ``double letter'' strings
where every character occurs twice in a row.  For example,
\texttt{aaCC33}, and \texttt{zz++ccBB} are double letter strings, but
texttt{aa;bb}, \texttt{b33}, and \texttt{AAAAA} are not.  Even better,
how about actually writing a recognizer for the double letter ASCII
strings in your favorite programming language?

We'll call a set of strings \term{recognizable} if there is a
procedure that recognizes precisely that set of strings.  So your
program would demonstrate that the set of double letter strings is
recognizable.

When you actually program a procedure, you have to type the program
text into a computer system, which a can be done using only the ASCII
characters.  This means that every procedure is described by some
string of ASCII characters.  If a string, $s$, is actually the ASCII
description of some string procedure, let's refer to that procedure as
$P_s$.  You can think of $P_s$ as the result of compiling
$s$.\footnote{The string, $s$, and the procedure, $P_s$, have to be
  ...
  and then apply $P_s$ to \texttt{aabbccdd}.  \iffalse This should
  result in a returned value \True, since \texttt{aabbccdd} is a
  double letter string.\fi }
...
\end{equation}
\end{definition}
Recognizing \nsl\ is essentially a special case of the general Halting
Problem.  We'll blow way any chance of having a program solve the
general problem by showing that no program can solve this special case.
In particular, we're going to prove
...
strings, and define $f: \asciistr \to \power(\asciistr)$ to be the
set of strings recognized by $P_s$:
\[
f(s) \eqdef \set{t \in \asciistr \suchthat P_s recognizes t}.
\]
By convention, a string procedure, $P_s$, is defined for every string,
...
particular language just to solve this special case of the general
Halting Problem for programs in that language.  And having proved that
it's impossible to have a procedure that figures out whether a
arbitraary program returns the value \True, it's easy to show that
it's impossible to have a procedure that is a perfect recognizer for any
complete run time property of programs.

For example, most compilers do ``static'' type-checking at compile
time to ensure that programs won't make run-time type errors.  Running
any program that type-checks is guaranteed not to cause a type-error.
But since it's impossibe to recognize perfectly when programs won't
cause type-errors, it follows that the compilers type-checker must be
rejecting programs that really wouldn't cause a type-error. So there
is no perfect type-checker ---you can always do better!
...
So from now on, if you hear people making claims about general program
analysis/verification/optimization methods, you'll know they can't be
telling the whole story.

One more important point: there's no hope of getting around this by
switching programming languages.  We've only proved that no Java
procedure, for example, can perfectly analyze all Java programs.  You
might wonder if a language like C++, which allows more intimate
manipulation of computer memory than Java, might have a procedure that
could handle Java programs.  But there is no loophole here: since it's
possible to write a virtual machine for C++ in Java, if there were a
C++ procedure that analyzed Java programs, so could the Java virtual
machine, and we know that's impossible.
