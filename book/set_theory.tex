...
2,4,6,6,6,\dots.
\]
This obvious remark leads to an alternative characterization of
...
\begin{lemma}
A set, $C$, is countable iff there is a total surjective function
$g:\naturals \to C$.
\end{lemma}
...
\sqrt{2} & = 1.4142\dots,\\
5 & = 5.000\dots,\\
\frac{1}{10} & = 0.1000\dots,\\
1/3 & = 0.333\dots,\\
%0 & = 0.000\dots,\\
1/9 & = 0.111\dots,\\
4\,1/99 & = 4.010101\dots.
\end{align*}
...
b(1/10) & = 1000\dots,\\
b(1/9) & = 111\dots,\\
b(4\,1/99) & = 010101\dots\\
b(\sqrt{2}), b(1/3)\ \text{undefined}.
\end{align*}
Now $b$ is a function from real numbers to infinite bit strings.  It
is not a total function, but it clearly is a surjection.  This
shows that
\[
\reals \surj \reals,
\]
and the uncountability of the reals now follows by
Corollary~\ref{UinjAu}.\ref{AsurjUA}.

For another example, let's prove
\begin{corollary}\label{countN*}
The set $\strings(\naturals^+)$ of all finite sequences of positive
...
e(20) & = (2,1),\\
e(6615)) & = (3,1,2),\\
e(5^{13}11^{9}47^{817} 103^{44}) & = (13, 9, 817, 44),\\
...
shows that
\[
\naturals \surj \strings(\naturals^+),
\]
and the countability of the finite strings of positive integers now follows by
Corollary~\ref{UinjAu}.\ref{CsurjAc}.
...
``\index{diagonal argument|textbf}{diagonal arguments}.''  The diagonal
...
$\naturals$ and $\binw$.  If such a relation existed, we
...
to organize this list, any given string in $\binw$ would
...
%\pinput{TP_cardinality_class}  %after mideterm 3/3/14
...
in your favorite programming language\iffalse such as C++, or Java,
or Python\fi---a \emph{string procedure} when it is applicable to
...
halts precisely when it is applied to a \emph{double letter} ASCII
...
Focusing just on string procedures, the general Halting Problem
...
set theory called the Axioms of \term{Zermelo-Fraenkel Set Theory} with Choice (ZFC).%
\index{ZFC|see{Zermello-Fraenkel Set Theory}}%
...
get some practice reading quantified formulas: \index{axiom!ZFC
  axioms|textbf}

\begin{description}

\item[\index{axiom!ZFC axioms!axiom of extensionality}\emph{Extensionality}.]
  Two sets are equal if they have the same members.
\[
(\forall z.\; z \in x \QIFF z \in y) \QIMPLIES x = y.
\]

\item[\index{axiom!ZFC axioms!axiom of pairing}\emph{Pairing}.] For any two sets $x$ and $y$, there is
  a set, $\set{x,y}$, with $x$ and $y$ as its only elements:
\[
\forall x,y.\; \exists u.\; \forall z.\;
[z \in u \QIFF (z = x \QOR z = y)]
\]

\item[\index{axiom!ZFC axioms!union axiom}\emph{Union}.]  The union, $u$, of
  a collection, $z$, of sets is also a set:
\[
\forall z.\, \exists u.\, \forall x.\; (\exists y.\; x \in y \QAND y \in z) \QIFF x \in u.
\]

\item[\index{axiom!ZFC axioms!infinity axiom}\emph{Infinity}.]  There is an
  infinite set.  Specifically, there is a nonempty set, $x$, such that
  for any set $y \in x$, the set $\set{y}$ is also a member of $x$.

\item[\index{axiom!ZFC axioms!subset axiom}\emph{Subset}.] Given any set, $x$,
  and any definable property of sets, there is a set containing
  precisely those elements $y \in x$ that have the property.
\[
...
\item[\index{axiom!ZFC axioms!foundation axiom}\emph{Foundation}.] 
...
\item[\index{axiom!ZFC axioms!axiom of choice}\emph{Choice}.]  Given a set, $s$, whose
