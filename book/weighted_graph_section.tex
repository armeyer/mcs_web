...
\begin{definition}
  A \term{weight function} for a graph $G$ is a function $\weight: \edges{G} \to
  \reals$, where $\weight{e}$ is called the \term{weight of edge} $e$.
...
The \term{weight matrix} for a weighted graph~$G$
...
But there is another trick that cuts down the number of $\minplus$
