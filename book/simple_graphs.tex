...
\index{vertex!simple graph|textbf} of~$G$, and a set $\edges{G}$ called the 
\emph{edges}%
\index{edge!simple graph|textbf} of $G$.  An element of
...
An edge whose endpoints are the same is called a \emph{\idx{self-loop}}.
...
\index{simple graph!empty graph} 
...
Let's look at another man/woman matching problem with an equal number
of men and women.  The set up is that each person has preferences
about who they would like to marry: each man has preference list of
all the women, and each woman has a preference list of all of the men.
...
\emph{vice-versa}---no polygamy.  Moreover, we would like to find a
...
which makes it surprising.  In fact, in the apparently similar
``buddy'' matching problem where people are supposed to be paired off
as buddies, regardless of gender, a stable matching \emph{may not} be
possible.  An example of preferences among four people where there is
no stable buddy match is given in Problem~\ref{PS_no_buddy_match}.
But when men are only allowed to marry women, and \emph{vice-versa},
then we will be able to describe a simple procedure to produce a
stable matching.\footnote{Once again, we disclaim any political
  statement here---it's just the way that the math works out.}
...
\end{center}
The Internet infrastructure company Akamai, cofounded by Tom Leighton,
also uses a variation of the Mating Ritual to assign web traffic to
its servers.
...
Ritual: if on some morning a woman has any suitor, then her favorite
suitor will still be serenading her the next morning---his list won't
have changed.  So she is sure to have today's favorite suitor among
her suitors tomorrow.  That means she will be able to choose a
favorite suitor tomorrow who is at least as desirable to her as
today's favorite.  So day by day, her favorite suitor can stay the
same or get better, never worse.  This sounds like an invariant, and
it is.

\begin{definition}\label{def:P8}
Let $P$ be the predicate:
\begin{quote}
for every woman, $w$, and every man, $m$, if $w$ is crossed off $m$'s
list, then $w$ has a suitor whom she prefers over~$m$.
\end{quote}
\end{definition}
...
\begin{proof}
$w$ gets crossed off $m$'s list only when $w$ has a suitor she prefers
  to $m$.  Thereafter, her favorite suitor doesn't change until one
  she likes better comes along.  So if her favorite suitor was
...
serenading anybody, that is, his list must be empty.  So Bob every
woman must have been crossed off his list and, since $P$ is true,
every woman has a suitor whom she prefers to Bob.  In particular,
every woman has \emph{some} suitor, and since it is the last day, they
have only one suitor, and this is who they marry.  But there are an
equal number of men and women, so if all women are married, so are all
men, contradicting the assumption that Bob is not married.
\end{proof}
...
under the Mating Ritual.  We'll begin by observing that while the
Mating Ritual produces one stable matching, there may be other stable
matchings among the same set of men and women.  For example, reversing
the roles of men and women will often yield a different stable
matching among them.  So a man may have different wives in different
sets of stable marriages.

In some cases, there is a man who can stably marry every one of the
woman, but in most cases, there are some woman who cannot be a man's
wife in any stable matching.  For example, given the preferences shown
in Figure~\ref{fig:minWtMatch2}, Jennifer cannot be Brad's wife in any
stable matching because if he was married to her, then he and Angelina
would be a rogue couple.  That is, it is not feasible to have Jennifer
married to Brad.
...
Let $Q$ be the predicate: for every woman, $w$, and every man, $m$, if
...
What's more since $Q$ holds, all Bob's feasible wives are still on his
...
less than Alice, making Alice and Ted a rogue couple, contradicting
...
Now suppose there are two stable sets of marriages, a first set and a
second set.  So each man has a first wife and a second wife, and
likewise each woman has a first husband and a second husband.  The
following Lemma will follow directly from the definition of stability.
...
\begin{proof}
Let's call someone \emph{active} if their first spouse is different
from their second spouse.  So a spouse of an active person must also
be active, and the number of active men is the same as the number of
active women.

Let's count the number of times people do better in one of their
marriages.  This may seem like a weird thing to be interested in,
since every active person does better in exactly one of their two
marriages, so the number of betterments contributed by men equals the
number of active men, which is the same as the number of betterments
contributed by women.

Now instead of counting betterments contributed by people, let's count
them by marriages.  The first observation is that a given first
marriage can't contribute two betterments, because if both spouses did
better in this first marriage than in their second marriages, they
would be a rogue couple in the second set of marriages.  Similarly, no
second The second observation is that a given first marriage can't
contribute zero betterments, because then the only way the of
betterments by marriage could equal number of male betterments

in the marriage set that included this marriage and both did worse in
the other marriage set

Since the number of first and second
marriage betterments
But a marriage involving an active person
contributes two second marriage betterments iff some other marriage
contributes two first marriage betterments
\end{proof}

\begin{corollary}\label{opt-pess}
...
marriages.  Suppose to the contrary that he was not her pessimal
husband.  Then there must be a second set of marriages in which
his wife had a worse husband, and by Lemma~\ref{lem:better-worse},
this would mean he had a better second wife, contradicting optimality
of his first wife.
\end{proof}

So we conclude with worse news for the women.  Not only does the
Mating Ritual favor the men, but it penalizes the women:

\begin{corollary}\label{wifepess}
The Mating Ritual marries every woman to her pessimal spouse.
\end{corollary}

\begin{proof}
By Theorem~\ref{boyopt}, the Mating Ritual marries all the men to
their optimal wives, so by Lemma~\ref{wifepess} it must marry all the
women to their pessimal husbands.
\end{proof}
...
A \term{cycle} is a closed walk of length three or more whose
...
than the weight of the NYC/Boston edge.  The \term{weight of a graph} is
