...
be the same).  The following Lemma will follow directly from the
...
A first observation is that the total number of active marriages is
the same as the total number of active people.  One simple way to see
this is to observe that the number of active first marriages is the
same as the number of active men, and the number of active second
marriages is the same as the number of active women.

Second, let's count the number of times active people do better in one
of their marriages.  Since every active person by definition does
better in exactly one of their two marriages, this total count simply
equals the number of active people.

Third, instead of counting by active people, let's count by active
marriages.  Hypothetically, each active marriage could contribute a
count of zero, one, or two to the total number of times people do
better.
...
their spouse did worse in their other marriage.
Lemma~\ref{lem:better-worse} can now be rephrased as saying that
\emph{every} active marriage has a score of one.

So all that's needed is to prove that no active marriage can
contribute a count of zero or two.  But a score of two is impossible:
if both spouses in a given active marriage do better than in their
other marriages, they would be a rogue couple for the other set of
marriages.  This in turn implies that score of zero is impossible,
because if one active marriage contributed a count of zero, some other
active marriage would have to contribute of score of two in order for
the total count to equal the number of active marriages.
\end{proof}
...
As an immediate corollary of Theorem~\ref{boyopt} and
Lemma~\ref{lem:opt-pess} is worse news for the women.  Not only does
the Mating Ritual favor the men, but it thoroughly discriminates
against the women:
...
\pinput{PS_stable_marriage_lub}  %needs soln

\examproblems
\pinput{FP_rogue_pair}
% \pinput{FP_stable_matching_unequal_invariants}  uncomment after S15.mid3
