...
\begin{proof}
  ...
  at least one edge, so there will be a vertex $v$ that comes just
  before $u$ on the path.  Now if there was an edge from $u$ to a
  vertex $w \neq v$ on the path, then the part of the path from $w$ to
  $u$ followed by the edge back to $w$ would be a cycle.  So
  $\edge{v}{u}$ must be the only edge incident to $u$.  That is, $u$
  is a degree one leaf.  Likewise, the vertex at the other end of the
  path must also be a leaf.
\end{proof}
\end{lemma}
...
\item Adding an edge between any two nonadjacent vertices in a tree creates a
  graph with a cycle.
