...
This procedure works because in any solid coloring of the spanning
forest $S$ built up at some point by the procedure, each connected
component of $S$ by definition is all white or all black.  So the
endpoints of any edge of $S$ will be the same color, which means no
gray edge will be an edge of $S$.  Also, adding a gray edge decreases
the number of connected components of $S$ by one.  So if $G$ has $n$
vertices, $S$ will be connected after adding $n-1$ edges.  By the Gray
Edge Lemma~\ref{lem:necessary-gray}, every edge of $S$ has to be in
every MST of $G$.  So we finish with a connected spanning forest, that
is, a spanning tree $T$ each of whose edges must be in
\emph{every} MST.  So $T$ must be an MST.

There's more: since $T$ consists entirely of edges that have to be
there, all MST's have these same edges, and there is no room for other
ones.  So $T$ is the only possible MST:
...
Let $G$ be a connected weighted graph, and let $e$ be the minimum
weight gray edge of some solid coloring of some spanning subgraph of
$G$.  Suppose $C$ is a connected spanning subgraph of $G$, and $e$ is
...
spanning tree of $G$, then $C - g + e$ is also a spanning tree and $C
- g + e$ has smaller weight.  So $C$ cannot be minimum weight unless
