...
\emph{must} be an edge of an MST.
...
component and one white connected component---this follows because the
blocks of a partition are nonempty by definition.  (So in order to
have a solid coloring, $S$ must not be connected.)
...
path there has to be an edge that starts with a black vertex and end
...
not yet connected, then solid color $S$, and add to $S$ the minimum
weight gray edge for the coloring.  Continue until $S$ becomes
connected.
\end{quote}
...
$S$ be the current spanning forest at any point in the procedure.  In
any solid coloring of $S$, the vertices in each connected component of
$S$ are all white or all black, by definition of solid coloring.  So
the endpoints of any edge of $S$ will be the same color, which means
no gray edge will be an edge of $S$.  By Lemma~\ref{lem:enough-gray},
if $S$ is not connected, there will be a gray edge that can be added
to $S$.  Adding a gray edge decreases the number of connected
components of $S$ by one, so after adding $n-1$ gray edges, the
procedure will terminate with a connected spanning forest, that is, a
spanning \emph{tree} $T$.
...
\item $C - g + e$ is connected, where $C - g + e$ is the graph
  obtained by removing $g$ and adding $e$ to the set of edges of $Cax$.
\end{itemize}
\end{lemma}
...
Pick a solid coloring for which $e$ is a min-weight gray edge.
