...
$v_0$, to \index{end of path}\emph{end} at $v_k$, and \index{length of
  path}\emph{length} of the path is defined to be $k$.  An edge,
$\edge{u}{v}$, is \term{traversed} $n$ times by the path if there are $n$
different values of $i$ such that $\edge{v_i}{v_{i+1}} = \edge{u}{v}$.
The path is \term{simple}\footnote{Heads up: what we call ``paths'' are
  commonly referred to in graph theory texts as ``walks,'' and simple
  paths are referred to as just ``paths''.  Likewise, what we will call
  \emph{cycles} and \emph{simple cycles} are commonly called ``closed
  walks'' and just ``cycles''.}  iff all the $v_i$'s are different, that
is, $v_i = v_j$ only if $i=j$.
\end{definition}
...
\]
graph is a \term{simple cycle} of length $n$ iff it is isomorphic to $C_n$
...
logical error called \term{buildup error}, illustrated in
...
\term{valid coloring} of the graph ---a ``coloring,'' for short.  A graph
$G$ is $k$-\term{colorable} if it has a coloring that uses at most $k$
