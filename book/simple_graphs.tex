...
We refer to Lemma~\ref{sumedges} as the \term{Handshake Lemma}: if we
total up the number of people each person at a party shakes hands
...
For this and related work, Shapley was awarded the 2012 Nobel prize
in Economics.
...
\end{definition}
Since a walk going back and forth on the same edge is always
possible---and is of little interest---we don't count length two
closed walks as cycles.  There are no closed walks of length one,
since simple graphs don't have self loops.  Also, a length zero closed
walk---that is, a single vertex---by definition does not count as a
cycle; this contrasts with the convention for digraphs, where

Note that a single vertex counts as a length zero path and a closed
walk.  But in contrast to digraphs, a single vertex is not considered
to be a cycle.    Of course
...
\arm{Maybe include subsection on TSP within a small factor of minimal
  when distances are Euclidean?  This may already be in an old problem
