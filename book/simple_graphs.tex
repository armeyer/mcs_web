...
<a href=''the myth, the math, the sex - new york times.pdf''>reported</a>
\iffalse
...
\begin{enumerate}
\item Simple graphs do \emph{not} have edges self-loops.  (An undirected edge is define
to be a set of \emph{two} vertices.)
...
\textbf{Base Case}: ($\card{G}=1}) If $\card{G} = 1$, then the matching condition implies that the
lone girl likes at least one boy, and so a matching exists.  

\textbf{Inductive Case}: Now suppose that $\card{G} \geq 2$.  There are two possibilities:
