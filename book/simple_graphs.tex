...
at all.  For example, the empty graph with five vertices is shown in
...
\begin{definition}
  ...
  vertices can be divided into two sets, $\leftbi{G}$ and
...
likes.\footnote{By the way, we do not mean to imply that marriage
  ...
  up all the women with different men.  \iffalse
  So please don't take offense.\fi}
...
\term{neighbors} of some set $S$ of vertices is the \idx{image} of $S$
under the edge-relation, \iffalse set of all vertices adjacent to some
vertex in $S$.  \fi that is,
\[
...
\begin{definition}\label{degree-constrained_def}
  ...
  \degr{r} \geq 1$ for every $l \in \leftbi{G}$ and $r \in \rightbi{G}$.
\end{definition}

For example, the graph in Figure~\ref{fig:5J} is degree-constrained
since every vertex on the left is adjacent to at least two vertices on
the right while every vertex on the right is adjacent to at most two
...
\end{equation}
  Since $G$ is degree-constrained, there is a $d\geq 1$ such that
...
\term{Regular graphs} are a large class of degree-constrained graphs that
...
The simplest property is being a \idx{cycle}: an even-length closed
...
For example, the $n$-node \term{star graph} shown in
...
expensive, it is important to minimize the number handed out.  This
...
graphs was acclaimed when it was discovered about forty years ago.
...
find an efficient procedure to tell if any particular planar graph
really \emph{needs} four colors, or if three will actually do the job.
A proof that testing 3-colorability of graphs is as hard as the
million dollar SAT problem is given in Problem~\ref{PS_3color_SAT};
this turns out to be true even for planar graphs.  (It is easy to tell
if a graph is 2-colorable, as explained in
Section~\ref{subsec:odd_cycles}.)  In
...
More precisely, a cycle is a closed walk that can be described by a
walk of length at least three whose vertices are all different except
for the beginning and end vertices.  So in contrast to \emph{paths},
the length of a \emph{cycle} is the \emph{same} as the number of
distinct vertices that appear in it.\footnote{Technically speaking, we
  haven't ever defined what a cycle \emph{is}, only how to describe it
  with paths.  But we won't need an abstract definition of cycle,
  since all that matters about a cycle is which paths describe it.}
\fi

\subsection{Cycles as Subgraphs}

We don't want think of a cycle as having a beginning or an end, so
\emph{any} of the paths that go around it can represent the cycle.
For example, in the graph in Figure~\ref{dg}, the cycle starting at
$b$ and going through vertices $bcdehb$ can also be described as
starting at $d$ and going through $dehbcd$.  Furthermore, cycles in
simple graphs don't have a direction: $dcbhed$ describes the same
cycle as though it started and ended at $d$ but went in the opposite
direction.

A precise way to explain which closed walks represent the same cycle
is to define cycle as a subgraph.  Specifically, we could define a
cycle in $G$ to be a \emph{subgraph} of $G$ that looks like a
length-$n$ cycle for $n \ge 3$.
...
\subsection{Euler Tours}
 ...
 could you visit every hallway exactly once in a museum with the floor
 plan in Figure~\ref{fig:5BC}?
...
every edge exactly once.  Graphs with Euler tours turn out to be easy
to recognize:
...
Similarly, a graph has a walk that includes every edge exactly once
but is \emph{not} a tour---it begins and ends at different
verticies---if and only there are exactly two vertices of odd degree.
The graph shown in Figure~\ref{fig:5BC} has exactly two vertices with
odd degree, so it has a such a walk but not an Euler tour.
...
characterization of all graphs with a Hamiltonian cycle.  In fact,
determining whether a graph has a Hamiltonian cycle is the same
category of problem as the SAT problem of Section~\ref{SAT_sec} and
the coloring problem in Section~\ref{sec:coloring}: you get a million
dollars for finding an efficient way to determine when a graph has a
Hamiltonian cycle---or for proving that no procedure works efficiently
on all graphs.
...
you started.  Can you find way to do this by with a weight~15 cycle?
...
more than one vertex, and every pair of distinct vertices in the graph are
$k$-edge connected.
\end{definition}
\iffalse
...
1-connected according to Definition~\ref{def:k-connected}.
...
Theorem~\ref{th:treeprops}.\ref{treeprops:v=e+1}
...
tree shown in Figure~\ref{fig:5KA} is~19.
\begin{figure}
...
We'll now explain a simple, general way to find MST's.  As a first
step, we'll define a special property that determines when an edge
must be an edge of an MST.

\begin{definition}
Let $S$ be a spanning subgraph of some simple graph $G$.  A
\term{solid coloring}\index{coloring!solid} of $S$ is a partition of
the vertices into two blocks---the ``black'' vertices and the
``white'' vertices---so that all the vertices in the same connected
component of $S$ have the same color.  A \term{gray edge} of the solid
coloring is an edge of $G$ with different colored endpoints.
\end{definition}

\begin{lemma}\label{lem:enough-gray}
Let $S$ be a spanning subgraph of a \emph{connected} graph $G$.  Every
solid coloring of $S$ contains a gray edge.
\end{lemma}

\begin{proof}
In any solid coloring of $S$ there will be least one black connected
component and one white connected component---this follows by
definition, because the blocks of a partition are nonempty .  (So in
order to have a solid coloring, $S$ must not be connected.)

Since $G$ is connected, there will be a path that begins at a black
vertex and ends at a white vertex.  Therefore at some point there has
to be an \emph{edge} on the path that goes between a black vertex and
a white vertex.  This is the gray edge.
\end{proof}

Weighted graphs commonly have edges with the same weight, but our
story becomes simpler if we
\begin{quote}
\emph{assume there are no same-weigh edges: different edges have
  different weights}.
\end{quote}

There is really no loss of generality assuming this, since everything
we do below assuming distinct weights would work if we assigned an
arbitrary rank to same-weight edges and treated higher rank vertices
as having larger weight.

The following lemma now makes it easy to find MST's.

\begin{lemma}[Gray Edge Lemma]\label{lem:necessary-gray}
For any solid coloring of a spanning subgraph of a graph $G$, the
minimum weight edge among the gray edges of the coloring
must be an edge of \emph{every} MST of $G$.
\end{lemma}

The Lemma provides a simple way to build up an MST of $G$ by adding
successive minimum weight gray edges:
...
the vertices of $G$ with no edges.  Next, suppose $S$ is a spanning
forest built up at some point by the procedure.  If $S$ is not yet
connected, then solid color $S$, and add to $S$ the minimum weight
gray edge for the coloring.  Continue until $S$ becomes connected.
\end{quote}

In any solid coloring of the spanning forest $S$ built up at some
point by the procedure, each connected component of $S$ by definition
is all white or all black.  So the endpoints of any edge of $S$ will
be the same color, which means no gray edge will be an edge of $S$.
Also, adding a gray edge decreases the number of connected components
of $S$ by one.  So if $G$ has $n$ vertices, $S$ will be connected
after adding $n-1$ edges.  By the Gray Edge
Lemma~\ref{lem:necessary-gray}, every edge of $S$ has to be in every
MST of $G$.  So we finish with a connected spanning forest, that is, a
spanning tree $S$.  Moreover, each edge in $S$ must be in \emph{every}
MST.  So $S$ has to be an MST, and moreever $S$ has to be the only
possible MST.  In particular, we have:
...
Let $G$ be a connected graph, and let $e$ be the minimum weight gray
edge of a solid coloring of a spanning subgraph of $G$.
...
\item $C - g + e$ is connected.
\end{itemize}
\end{lemma}
Notice that Lemma~\ref{lem:necessary-gray} now follows immediately: if
$C$ is a spanning tree of $G$, then $C - g + e$ is also a spanning
tree and $C - g + e$ has smaller weight.  So $C$ cannot be minimum
weight unless $e$ is one of its edges.
...
\begin{proof}
  ...
  contain some gray edge $g \neq e$.  The trick is to add $e$ and
  remove $g$ from $C$ to obtain a subgraph $C+e-g$.
  Notice that $\walkv{p} + e$ is a cycle, so when $g$ is removed from
  the cycle, the endpoints of $g$ remain connected by the rest of the
  cycle.  This means $C+e-g$ is still a connected, spanning subgraph.
\end{proof}
