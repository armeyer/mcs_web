...
\emph{perfect} \index{perfect matching}\index{graph!perfect
  matching}when it covers $\vertices{G}$.  The set $\neighbors{S}$ of
\term{neighbors} of some set $S \subseteq \vertices{G}$ of vertices is
the \idx{image} of $S$ under the edge-relation, \iffalse set of all
...
A graph is \term{regular}\index{graph!regular} when all its vertice have the same degree.
...
\item\eqref{maxacyclic}~implies~\eqref{uniquepath}: Call two vertices
  ...
  necessarily on a cycle, see Problem~\bref{MQ_closed_walk_to_cycle}.)
