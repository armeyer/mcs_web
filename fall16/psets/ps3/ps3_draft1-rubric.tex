...
\end{array}
\]
Hence we have $x = 3, y = -2$.  Considering the equation modulo $43$, we have that $-2 \cdot 64 \equiv 1$ (mod $43$).  Thus the inverse of $64$ mod $43$ is $41$, as $41 \equiv -2$ (mod $43$).
}
\ppart{4} Prove that $2 \mid (n)(n+1)$ for all integers $n$. 
\proofrubric{
[2pts] Case that $n$ is even.
\par
[2pts] Case that $n$ is odd.
}{4}
\solution{
...
\proofrubric{
[2pts] Case that $n \equiv 0$ (mod 3).
\par
[2pts] Case that $n \equiv 1$ (mod 3).
\par
[2pts] Case that $n \equiv 2$ (mod 3).
}{6}
\solution{
...
\proofrubric{
[2pts] Rewrite $b$ in terms of $a$. \par
[2pts] $bc = (ak)c$ must also be a multiple of $a$ then.
}{4}
\solution{
...
\proofrubric{
[2pts] Show $a \mid b$ implies $ca \mid cb$. \par
[2pts] Show $ca \mid cb$ implies $a \mid b$.
}{4}
\solution{
...
\proofrubric{
[2pts] Write out $gcd(ka, kb)$ as $s(ka) + t(kb)$. \par
[2pts] Argue $sa + tb = gcd(a,b)$.
}{4}
\solution{
...
\proofrubric{
[2pts] $a \equiv b$ (mod $n$) means that $n \mid (a-b)$ and so on. \par
[2pts] Rewrite $ac - bd$ in terms of $(a-b)$ and $(c-d)$.
}{4}
\solution{
...
\proofrubric{
[3pts] $1 = ab + pc \equiv ab$ (mod $p$). \par
[3pts] Show that there is a unique $b$.
}{6}
\solution{
...
\proofrubric{
[1pt] $p \mid (a+1)(a-1)$ \par
[2pts] $p \mid (a+1)$ or $p \mid (a-1)$ \par
[3pts] The other direction.
}{6}
\solution{
...
\proofrubric{
[1pt] Expansion of $(p-1)!$ \par
[2pts] Pair integers up with unique inverses modulo $p$ \par
[3pts] Case where $a = b$, which is only true for $a = 1$ and $a = (p-1)$ \par
[2pts] $(p-1)! \equiv 1 \cdot (p-1)$, where $p - 1 \equiv -1$ (mod $p$)
}{8}
\solution{
...
\rubric{[2pts] $3^{31} \equiv 1$ (mod 7)}
\solution{
...
\proofrubric{
[2pts] Factor $n^7 - n$ correctly\par
[2pts] $6 \mid n(n+1)(n-1)$\par
[1pt] Case where $7 \mid n$\par
[1pt] if $7 \nmid n$, then $7 \mid n^6 - 1$
}{6}
\solution{
We have that
 \begin{equation*} 
...
\proofrubric{
[2pts] Combine into common denominator.\par
[3pts] Show $3 \mid n(3n^4 + 5n^2 + 7)$ (1 point for if $3 \mid n$, 2 points for Fermat's little theorem)\par
[3pts] Show $5 \mid n(3n^4 + 5n^2 + 7)$ (1 point for if $5 \mid n$, 2 points for Fermat's little theorem)\par
}{8}
\solution{
...
\proofrubric{
$m$ is the smallest positive linear combination of $a,$ $b,$ and $c$.\par
[1pt] $m$ must be greater than or equal to  $\gcd(a, b, c)$\par
[8pts] Show that $\gcd(a, b, c) \leq m$ by showing that $\gcd(a, b, c) \mid s a + t b + u c$\par
[2pts] $a = q \cdot m + r$\par
[2pts] $a = q\cdot (sa + tb + uc) + r$\par
[3pts] Argue that since $m$ is the smallest positive linear combination, $r = 0$\par
[1pt] Claim $m \mid a$\par
[1pt] Same argument for $m \mid b$\par
[1pt] Same argument for $m \mid c$\par
[1pt] $m$ must be the less than or equal to the greatest common divisor of $a, b,$ and $c$
}{20}
  \solution{
...
\proofrubric{
[3pts] $p \mid (x + y)(x - y)$ \par
[2pts] $p \mid (x+y)$ or $p \mid (x-y)$ \par
}{5}
...
\rubric{
[5pts] Substitute in $a^2 \equiv n$ .
}
...
\rubric{
[2pts] $\frac{p-1}{2} = \frac{4k+3-1}{2}=k+1$ \par
[2pts] $n^{2k+2} \equiv n \pmod p$ \par
[2pts] $\big(n^{k+1}\big)^2 \equiv n \pmod p$ \par
[3pts] $n^{\frac{p-3}{4}+1}$ \par
}
