...
\ppart{10} Is it possible to cover the 6 x 6 board tiled with letters $A, B, C, D$ below with rectangles of size 1 x 4?
 \[
...
\solution{
Observing the board, we see that each 1 x 4 rectangular tile must cover exactly one of each of $A, B, C, D$ based on the tiling.  However, there are 9 $A's$, 10 $B's$, 9 $C's$, and 8 $D's$ in our tiling, meaning that there is no way to tile a 6 x 6 board with such rectangles.
}
\eparts
\end{problem}
...
\begin{problem}{20}\textit{The Well Ordering Principle (WOP)} states that ``every \textit{nonempty} set of 
