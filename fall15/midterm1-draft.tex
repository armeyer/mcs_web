\documentclass[quiz]{mcs}

\renewcommand{\exampreamble}{   % !! renew \exampreamble
  \begin{tabular}{l}
    \textbf{Fill in your}\quad   \teaminfo
  \end{tabular}

  \begin{itemize}

  \item
   This exam is \textbf{closed book} except for a 2-sided cribsheet.
   Total time is 80 minutes. 

  \item
   Write your solutions in the space provided.  If you need more
   space, \textbf{write on the back} of the sheet containing the
   problem.

%   Please keep your entire answer to a problem on that problem's page.   
   \item In answering the following questions, you may use without
     proof any of the results from class or text.

\iffalse
  \item
   GOOD LUCK!
\fi

  \end{itemize}}

\begin{document}

\midterm{February 25}

\begin{staffnotes}
Topics: WOP; Predicate Logic; Binary Relations; Induction.
\end{staffnotes}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Problems start here
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\examspace

Use a variation of this bogus proof using 1/2 as a base of the logarithm instead of 10
\pinput[points = 25, title= \textbf{Buggy Proof}]{CP_buggy_highschool_proofs}
\examspace

Use a variation of this problem. Instead of seventh root of 35 use square root of 6.
\pinput[points = 25, title= \textbf{An Irrational Number}]{MQ_seventh_root_35_irrational}
    %MQ_log12_of_18_irrational used midterm S14
    %CP_log9_of_12_irrational

\examspace
We might want to delete "you do not need to explain your answer" in case students who can't solve the problem want to get partial credit
\pinput[points = 15, title=\textbf{Predicate Logic}]{MQ_swapping_quantifiers}

\examspace

\pinput[points = 10, title=\textbf{Well Ordering Principle}]{FP_Wop_Sum_Of_Inverse_Sqrt}

\examspace
\pinput[points = 25, title=\textbf{Sets}]{PS_set_union}


%\examspace
%\pinput[points = 25, title=\textbf{Induction}]{MQ_sum_of_3i-1_squares_S15}

% \examspace
% \pinput[points = 20, title= \textbf{Strong Induction}]{FP_3_exponent_inequality_induction}

% \examspace
% \pinput[points = 25, title= \textbf{State Machines}]{MQ_stamps_state_machine}

% \examspace
% \TBA{\textbf{DRAFT.  Problems here were listed but unused in midterm1, F13}}
% \pinput[points = 25, title= \textbf{Induction}]{FP_gcd_linear_combination_induction}

% \examspace
% \pinput[points = 20, title= \textbf{Preserved Invariant}]{FP_binary_gcd}

% \examspace
% \pinput[points = 20, title= \textbf{Preserved Invariant}]{MQ_fast_exponentiation}

% \examspace
% \pinput[points = 20, title= \textbf{Well Ordering}]{TP_divide_product_wop}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Problems end here
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\end{document}
