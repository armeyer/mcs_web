...
\ppart
Which of the preceding are preserved invariants?

\begin{solution}
All except for the first one. All invariants are also preserved invariants.
In addition, $n_b - n_w \equiv 2 \pmod{3}$ is preserved since $n_b - n_w$
changes by $+3$ or $-3$ in each step.
On the other hand, $n_b + n_w \equiv 1 \pmod{3}$ is not a preserved invariant
since $n_b + n_w = n$ can be any positive integer.
\end{solution}

\examspace[1.0in]

\ppart
Using induction on $n$, prove that all states that satisfy any invariant
and for which $n_b, n_w \ge 0$ are reachable.

\begin{solution}
\inductioncase{Base case}: ($n = 1$).
The only valid state for $n=1$ is $(n_b, n_w) = (1,0)$, which is the starting
state and is therefore reachable.
...
\ppart
Why do you think the proof above holds regardless of what the invariant is?

\begin{solution}
Any invariant must hold for all reachable states.
\end{solution}
...
Prove that $n_w - n_b \equiv 2 \pmod{3}$ is a state invariant.
...
Write down any invariant variable that is a function of $n_b$ and $n_w$.

\begin{solution}
$n_w - n_b \equiv 2 \pmod{3}$ (equivalent expressions can be made).
\end{solution}

\examspace[1in]

\ppart
Prove that it is indeed an invariant.
