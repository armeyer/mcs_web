...
The real numbers in the interval $\Zintvcc{0}{1}$ \hfill\examrule[0.4in]

\item %\label{itm:roots}
% The complex numbers $c$ such that $\exists m,n \in \integers.\,
% (m+nc)c = 0$. \hfill\examrule[0.4in]
The rational number in the interval $\Zintvcc{0}{1}$ \hfill\examrule[0.4in]
...
$\mathcal{U} - \mathcal{F}$ \hfill\examrule[0.4in]
...
\[
A \strict B \strict \reals.
\]
%Recall that $A \strict B$ means that $A$ is not ``as big as'' $B$.

\begin{solution}
Let $A = \set{1, 2, 3}$ and $B$ be $\integers$.
\end{solution}
