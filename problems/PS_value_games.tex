...
The \term{plays} of a game $G \in \VG$ are defined recursively on the
definition of $\VG$:
...
$G_j$ and continues with a play of $G_j$.  By structural induction, we
know that this play of $G_j$ is a sequence of some finite length $n$
that ends with a value, so this play of $G$ is a length $n+1$ sequence
that ends with the same value.
\end{solution}
...
min-capped-value are equal for each of the games $G_i$.

\inductioncase{Case} ($a = \texttt{max}$):
We may assume by structural induction that each of the next moves of
$G$ has a value.  Let $v$ be the maximum of these values.  This max
will exist because $V$ is assumed to be finite.

Now a strategy whereby the max-player can ensure $v$ is:
\begin{quote}
Choose a first move, $G_i$, that has value $v$, and then follow the
...
\end{quote}
So $v$ can be both ensured and capped at, and hence must be both the
max-ensured-value and the min-capped-value.  So $G$ has value $v$.
...
Now a strategy whereby the min-player can cap payoff to $v$ to choose
a first move, $G_i$, with value $v$, and thereafter, to follow the
strategy in $G_i$ that caps at $v$.
...
So $v$ can be both capped at and ensured, and hence must be both the
min-capped-value and max-ensured-value.  So $G$ has value $v$.

So in any case, the $G$ has a value, $v$, which completes the
