...
Before we start, it helps to picture any model for these symbols by
...
Since the model must interpret the symbols $\next$ and $\prev$ as
total functions on the domain, we can see that every point has exactly
one $\next$-arrow, and exactly one $\prev$-arrow, going out of it.
...
express the requirement as a predicate formula:
\begin{equation}\label{previnvnext}
\forall x.\, \prev(\nextel(x)) = x\ .
\end{equation}
Notice that the $\prev$-arrow out of a point could not go back to the
beginning of a $\next$-arrow coming in more than one $\next$-arrow was
coming in.  So~\eqref{previnvnext} implies that at most one
$\next$-arrow can go into any element.  Another way to say this is
that $\next$-arrows that go out from different points have to go into
different points:
...
specify: they will be elements we get by starting at the $\numerl{0}$
element by following $\next$-arrows, corresponding to starting at zero
and repeatedly adding one.  Specifically, we can introduce some
convenient abbreviations for some actual terms using just the allowed
symbols:
\begin{align*}
\numerl{1} & \eqdef \nextel(\numerl{0})\\
\numerl{2} & \eqdef \nextel(\numerl{1})\\
\numerl{3} & \eqdef \nextel(\numerl{2})\\
           &\vdots
\end{align*}
For example, $\numerl{3}$ is an abbreviation for an actual term using
the allowed symbols:
\[
\nextel(\nextel(\nextel(\numerl{0}))).
\]
...
elements in the copy of $\naturals$ we are look to create, with the
...
the numerals having the same value.  (You should check this yourself
right now.)  We might try to fix this by requiring that no
...
\exists x, y.\, \nextel(x) = y \QAND \nextel(y) = x.
\]
We could go on to forbid two $\next$-arrows in a cycle like this, but
...
We want the interpretation of $\numerl{0}$ to be an element that is
not ``plus one'' of anything.  By convention ``subtracting'' one from
zero should have no effect.  This is also easy to say:
\begin{equation}\label{zeronosuccessor}
\forall x. \next(x) \new \numerl{0} \QAND \prev{\numerl{0}} = \numerl{0}.
\end{equation}
