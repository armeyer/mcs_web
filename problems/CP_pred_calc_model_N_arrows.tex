...
Our first objective will be to find some elements that make up a copy
of $\naturals$.  This means that the domain of the model includes an
infinite sequence of distinct points with a $\next$-arrow going from
each point to the next in the sequence:
\[
e_0 \nextarrow e_1 \nextarrow e_2 \nextarrow  \dots \nextarrow e_n \nextarrow \dots.
\]
Since $\next$ is supposed to act like plus one and $\prev$ like minus
one, we require that if a $\next$-arrow goes into an element $e$, then
the $\prev$-arrow going out of $e$ goes back to the beginning of the
$\next$-arrow:
...
$\naturals$ will be obtained by starting at the point that is the
interpretation of $\numerl{0}$ and following consecutive
$\next$-arrows.  More precisely, lets define some convenient
abbreviations for some actual terms that using the allowed symbols:
\begin{align*}
\numerl{1} & \eqdef \nextel(\numerl{0})\\
\numerl{2} & \eqdef \nextel(\numerl{1})\\
\numerl{3} & \eqdef \nextel(\numerl{2})\\
...
above do not even imply that $\next(x) \neq x$.  (You should check
this yourself right now.)  We might try to fix this by requiring that
no $\next$-arrow can begin and end at the same element, but there
still could be a pair of elements, each of which was ``plus one'' of
the other.  That is, the model might satisfy:
\[
\exists x, y.\, \nextel(x) = y \QAND \nextel(y) = x.
\]
...
\exists x, y,z.\, \nextel(x) = y \QAND\ \nextel(y) = z \QAND\ \nextel(z) = x\ ,
\]
and so on.  Fortunately, it turns out that something we want to do
anyway will fix this problem.
...
\begin{proof}
Suppose we start at the point denoted by $\numerl{0}$ and proceed to
...
Now with $n,m>0$, by~\eqref{previnvnext} the interpretation of the
numerals will satisfy
\begin{align}
\prev(\numerl{n}) & \eqdef \prev(\next(\numerl{n-1}) = \numerl{n-1},\notag\\
\prev(\numerl{m}) & \eqdef \prev(\next(\numerl{m-1}) = \numerl{m-1},\notag\\
\numerl{n} &  = \numerl{m}\ \QIMPLIES \numerl{n-1} = \numerl{m-1},\label{n-1 better}
\end{align}
contradicting the minimality of $n$.

Since the assumption that two different numerals had different values
led to a contradiction, we conclude that in every model
satisfying~\eqref{previnvnext} and part~\eqref{zero-part}, the
numerals all have different values.\footnote{A more careful look at
this argument reveals \mrel{without appeal to Well Ordering}, that
for $n\neq m \in \naturals$, there is a simple formal proof of the
formula
\[
\QNOT(\numer{n} = \numer{m})
\]
