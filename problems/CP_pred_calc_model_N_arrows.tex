...
is the numeral for three would be:
...
\hint It helps to describe the meaning of~\eqref{previnvnext} in terms
what it says about arrows.  There is also an explanation based on the
...
$\nngint$.  This helps explain why verifying valid formulas of
predicate calculus is so hard.  The explanation begins with the
observation that lots of famous mathematical problems---the Goldbach
or the Twin Primes Conjectures, for example---can easily be expressed
in predicate logic using a symbols $\nextsym,\prevsym$ along with
symbols for plus and times (see Problem
...
But if we use formula~\eqref{previnvnext} and the formula
...
Goldbach's Conjecture or The Twin Primes Conjecture is a \emph{true}
proposition about the actual nonnegative integers, all we have to do
is determine if these Conjectures are \emph{valid} consequences of the
axioms.  So if we accept that determining truth is generally very
hard, then determining if a predicate formula is valid is also going
to be very hard.

This distinction between formulas that are true over the domain
$\nngint$ and formulas that are valid over all domains can be
confusing on first sight.  To highlight the distinction, it is worth
seeing that the formula of part~\eqref{zero-part} together
with~\eqref{previnvnext} do \emph{not} ensure that their models
consist \emph{solely} of a copy of $\nngint$.
...
we just assert that there is the only zero-like element.
