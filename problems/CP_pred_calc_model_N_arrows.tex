...
fix this potentia; problem with the numeral values : we haven't yet
...
copy~\eqref{Ncopyshape} of $\naturals$ we want.  To verify this, we
...
equations~\eqref{previnvnext},part~\eqref{zero-part},
and~\eqref{uniquezero}.
...
\ppart\label{x=nx} Describe a model that satisfies $\exists x.\, x =
\nextsym(x)$.
...
\end{staffnotes}
$\naturals \union \set{\infty}$ will serve.
\end{solution}
...
must have a single copy of $\naturals$ along with some two-way
infinite chains
\[
\nextarrow \dots c_{-2} \nextarrow c_{-1} \nextarrow c_0 \nextarrow
c_1 \nextarrow c_2 \nextarrow \dots.
...
The trichotomy property,~\eqref{trichot} ensures all these chains will
have the property that every element of one chain will be $\ltsym$ of
every member of the other chain.  So the chains themselves are
``ordered'' by $ltsym$.

\bparts
\ppart Verify that the set $\reals$ of real numbers is a model
satisfying all the above formulas.
...
\begin{staffnotes}
Could require zero is $\ltsym$ everything, so only $\reals^{\geq 0}$
will be a model.  Might not want stuff between $x$ and $\nextfun(x)$:
\[
\forall x, y.\, \QNOT(x \ltsym y \ltsym \nextsym(x)).
\]

There is nothing further that can be said of how chains are ordered:
there might be none, some finite number, or even an infinite number of
them.  And the chains might be ordered among themselves like the
integers, nonnegative integers, real numbers,\dots.
