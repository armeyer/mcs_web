...
\nextsym\nextsym(\zerosym) = \zerosym.
\]
We could go on to forbid a length two cycles of $\nextsym$-arrows
among the values of numerals, but then there might be a cycle
of three:
\[
\exists x, y,z.\, \nextsym(x) = y \QAND\ \nextsym(y) = z \QAND\ \nextsym(z) = x\ ,
\]
...
for predicate formulas.  But the formulas above do not ensure models
behave much like $\naturals$.  The issue is that in forcing there to
be a copy of $\naturals$ in the model, we have put no constraints on
what else might appear.  For example, there might be two separate
copies of $\naturals$.  Saying that there is only one zero-like
element, namely only one element that is not in range of $\nextfun$
prevents that:

\begin{equation}\label{uniquezero}
\forall x.\, x \neq \zerosym\ \QIMPLIES \exists y.\, \nextsym(y) = x
\end{equation}
...
\begin{solution}
\TBA{revise}
We know that every element $e neq e_0$ equals
$\nextfun(e')$.  So $\nextfun(\prevfun(e)) =
\nextfun(\prevfun(\nextfun(e'))) = \nextfun(e') \eqdef e$
\end{solution}

\ppart\label{x=nx} Show that there is a model in which $\exists x.\, x = \nextsym(x)$ is true.
...
To forbid models from satisfying~\eqref{x=nx} or having longer finite
cycles of $\nextsym$-arrows, we need to use another symbol.  We will
now allow the binary relation symbol $L(\ ,\ )$ .  Our objective will
be to force $L$ to act like the less-than relation, $<$.  For this
