...
hoping the other computer will hear it. The problem is that if {\em
  more} than one computer broadcasts at once, a {\em collision} occurs
that garbles all messages we are trying to send. The transmission only
works if {\em exactly one} machine broadcasts at one time.
...
%to get around this problem by breaking symmetry is {\em
%randomization}.
