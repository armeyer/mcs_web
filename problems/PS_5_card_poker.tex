...
\begin{pcomments}
  ...
  \pcomment{further revised by ARM 11/3/009}
\end{pcomments}
...
\begin{problem}
  ...
  the set, $R$, and one four \term{suit}s in the set, $S$, where
\begin{align*}
...
\end{align*}
  ...
  counted using the Product and Sum Rules described in the Notes, and the
  set of hands matching the specification.  \emph{Give bijections, not
    numerical answers.}
...
\begin{solution}
There is a bijection with the set $R \cross S_2 \cross R_3 cross R^3$
where an element
\[
\paren{r, \set{s_1,s_2}, \set{r_1,r_2,r_3}, (s_1, s_2, s_3)} \in R \cross
S_2 \cross R_3 cross R^3
\] 
specifies
\begin{enumerate} 
\item the rank, $r$, of the pair,
\item the set of two suits, $\set{s_1,s_2}$, of the pair,
\item the set, $\set{r_1, r_2, r_3}$, of three ranks of the unpaired cards,
\item the sequence, $(s_1, s_2, s_3)$, of suits of the unpaired cards, in
  rank order.
\end{enumerate}
For example,
\[
\paren{Q, \set{\heartsuit,\spadesuit}, \set{3, J, 2},
...
\begin{solution}
  ...
  S \cross R_2 \cross S \cross S$, of
...
$A_{\text{full}} \eqdef S \cross R \cross S_2$, of sequences of the form
...
Finally, there is a bijection between the hands with exactly 4 aces and
the set, $A_4 \eqdef R cross S$, of sequences of the form $(r,s)$
