...
\begin{pcomments}
  \pcomment{PS_unit_linear_recurrences} \pcomment{ARM 4/10/12}
...
\ppart\label{ppart:inverse-repeat} A linear-recursive function is
\term{inverse-linear} if its $d$th coefficient $c_d = \pm 1$.
Prove that if an inverse-linear-recursive function repeats modulo $m$ at
$n$ and $k$ for some $n \geq d$, then it repeats modulo $m$ at $n-1$
and $k-1$.

\begin{solution}
Solving~\eqref{fnc1c2} for $f(n-d)$, we have
\[
f(n-d) = 1/c_d \paren{f(n) - \sum_{i=1}^{d-1} c_i f(n-i)}
\]
for all $n \geq d$.  Since, $c_d = \pm 1$, it has an inverse $b_m$
modulo $m$, so that
\begin{equation}\label{fn-dmodm}
f(n-d) \equiv b_m \paren{f(n) - \sum_{i=1}^{d-1} c_i f(n-i)} \pmod m,
\end{equation}
...
inverse-linear-recursive function must repeat modulo $m$ at $d-1$ and
$(d-1)+j$ for some $j>0$.

\begin{solution}
By part~\eqref{ppart:mustrepeat}, the function repeats at some $n$ and
$n+j$ for some $j>0$, and by part~\eqref{ppart:inverse-repeat}, the
least such $n$ must be $d-1$, since otherwise we could use
part~\eqref{ppart:inverse-repeat} to decrement $n$ by 1 to obtain another
value satisfying the condition.  In fact, part~\eqref{ppart:mustrepeat}
implies that there will be such a $j$ less than $d+ m^d$.
\end{solution}

\ppart\label{ppart:alldivisors}
Conclude that if $f$ is an inverse-linear-recursive function and $f(k) =
0$ for some $k \in [0,d)$, then every positive integer is a divisor
of $f(n)$ for infinitely many $n$.

\begin{solution}
Let $m$ be a positive integer that we want to show divides $f(n)$ for
infinitely many $n$.
...
\begin{solution}
Let $f$ be the inverse-linear-recursive function such that $f(0) =0,
