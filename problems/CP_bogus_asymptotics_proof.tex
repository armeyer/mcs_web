...
(This is a minor simplification, since actually the definition of
$O()$ tells us that the supposedly $O(1)$ function $f$ must be
bounded by a constant $c$ after some $n$ threshold $n_0$, but in that
case we just take the \emph{maximum} of $c$ and all the values of $f$
before $n_0$ to produce a new $c$ that applies everywhere.)
