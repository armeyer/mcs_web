...
\begin{pcomments}
  ...
  \pcomment{may still require too much ingenuity}
\end{pcomments}
...
\begin{problem}
  ...
  that are distance $n$ apart in $C_{2n}$.  Let $G$ be the graph formed
...
\begin{solution} If $n=2k$ the diameter is $k$.  If $n = 2k + 1$ the diameter
  is also $k$.  (For $d \in \Zintvco{1}{n}$ find the maximum value of
    $\min(d,n-d+1)$.)
\end{solution}
