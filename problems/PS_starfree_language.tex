...
concatenating any number of copies of words in $S$ is called
$S^*$---pronounced ``$S$ star.''  (By convention, the empty word,
$\emptystring$, always included in $S^*$.)  For example,
$\set{\STR{0},\STR{11}}^*$ is the language of all words you can make
by stringing together $\STR{0}$'s and $\STR{11}$'s.  This language
could also be described as the set of all words whose blocks of
$\STR{1}$'s are always of even length.  Another example is $(D^2)^*$,
which consists of all the even length words.  Finally, the set, $B$,
of \emph{all} binary words is just $D^*$.

A language is called \emph{concatenation-definable} (\emph{c-d}) if it
...
$B$) to these languages a finite number of times.\footnote{ We can
...
some nonnegative integer $n$.}  Note that the $^*$-operation is
\emph{not} allowed.  For this reason, the c-d languages are also
called the ``star-free languages,'' \cite{Meyer69}.

Lots of interesting sets of words turn out to be
concatenation-definable, but some very simple sets are not.  This
problem ends with the conclusion that the set $\set{\STR{00}}^*$ of
even length words whose bits are all \STR{0}'s is not a c-d set.
...
\[
\set{\STR{01}}^* = \bar{B\set{\STR{00},\STR{11}}B} \intersect 0B1.
\]
\end{solution}
