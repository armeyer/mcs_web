...
For $P$'s that satisfy~\eqref{Sk2}, some of the assertions below
\textbf{C}an hold for some, but not all, such $P$, other assertions
\textbf{A}lways hold no matter what the $P$ may be, and some
\textbf{N}ever hold for any such $P$.  Indicate which case applies for
each of the assertions \inbook{and briefly explain why}\inhandout{  \textbf{circling} the correct letter}.

\begin{problemparts}

\problempart $\forall n \ge 0.\; P(n)$
...
So this case is possible.  But~\eqref{Sk2} also holds when $P$ is
always false, so the asertion does not always hold when \eqref{Sk2}
does.
\end{solution}
