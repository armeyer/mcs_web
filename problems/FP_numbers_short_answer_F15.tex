...
side-conditions needed to make the assertion true,\footnote{Condition
  $C$ is \emph{weaker} than condition $D$ when $D$ implies $C$.  For
  example, $C \eqdef [\gcd(a, b) = 1]$ is a weaker condition than $D
  \eqdef [\gcd(a,b)=1=\gcd(a,c)]$, so writing $D$ as a required a
  side-condition when $C$ would suffice would not get full credit.},
or write \False\ if none of the side-conditions imply the assertion is
correct.
