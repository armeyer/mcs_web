...
\begin{definition*}
  ...
  Consider a new recursive definition, $\MB{0}$, of the same set of
  ``matching'' brackets strings as $\MB{}$ (definition of $\MB{}$
  is provided in the Appendix):

\begin{itemize}

\item \textbf{Base case:} $\emptystring \in \MB{0}$.
...
\item If $s$ is in $\MB{0}$, then $\lefbrk s \rhtbrk$ is in $\MB{0}$.

\item If $s,t \in \MB{0}$, $s\neq \emptystring$, and $t \neq
  \emptystring$, then $st$ is in $\MB{0}$.
...
\ppart Suppose structural induction was being used to prove that
$\MB{0} \subseteq \MB{}$.  \inbook{Indicate}\inhandout{Circle} the one
predicate below that would fit the format for a structural induction
hypothesis in such a proof.

\begin{itemize}

\item $P_0(n) \eqdef  \lnth{s} \leq n \QIMPLIES s \in \MB{}$.

\item $P_1(n) \eqdef  \lnth{s} \leq n \QIMPLIES s \in \MB{0}$.

\item $P_2(s) \eqdef s \in \MB{}$.

\item $P_3(s) \eqdef s \in \MB{0}$.

\item $P_4(s) \eqdef\ (s \in \MB{} \QIMPLIES s \in \MB{0})$.

\end{itemize}

\begin{solution}
  $\MB{0} \subseteq \MB{}$ means that $\forall s \in \MB{0}.\ s \in
  \MB{}$.  To prove this, the only hypothesis above that fits the format
  for a structural induction would be $P_2$ in a structural induction on
  the definition of $\MB{0}$.
\end{solution}


\ppart The  recursive definition $\MB{0}$  is \emph{ambiguous}.  Verify
...
\begin{solution}
  $\MB{0}$ is ambiguous because $\lefbrk \rhtbrk \lefbrk \rhtbrk
  \lefbrk \rhtbrk \in \MB{0}$ can be derived from the second
  constructor with $s = \lefbrk \rhtbrk$ and $t = \lefbrk \rhtbrk
  \lefbrk \rhtbrk$, but also with $s = \lefbrk  \rhtbrk \lefbrk  \rhtbrk$ and $t = \lefbrk  \rhtbrk$.
\end{solution}
...
% \naturals$\fi based on the recursive definition of $\MB{i}$, for
