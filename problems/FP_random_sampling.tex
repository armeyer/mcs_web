...
\begin{pcomments}
  ...
  \pcomment{from: tutor problem practice13/prob1}
\end{pcomments}
...
\ppart
Our theorems about sampling and distributions allow us to
calculate how confident we can be that the random variable $P$ 
...
\item Given a particular voter, the probability of that voter preferring the President is $p$.
...
\item The probability that the second voter chosen will favor the President, 
...
$n$ grows beyond 100.  (4) holds by definition of randomly
...
You cannot say (1): the only way to know the exact value of the constant $p$ is to 
ask all 250,000,000 voters. 

You cannot say (2) either: $p$ is a constant which can either be or not be 
...
either $\abs{0.53 - p} \leq 0.04$
or   $\abs{0.53 - p} > 0.04$
which is obviously true. Now read it as follows: Either $p$ is within 0.04 of 0.53
or it is not and therefore my random variable $P$ took a value from a set that is
hit only 5 times in 100. So, clearly, either $p$ is within 0.04 of 0.53
