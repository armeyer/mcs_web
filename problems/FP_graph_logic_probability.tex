...
\begin{pcomments}
  ...
  \pcomment{soln revised zoki & ARM 11/14/15} 
\end{pcomments}
...
\begin{problem} \textbf{Graphs, Logic \& Probability}
  ...
  Let $G$ be a simple graph with $n>3$ vertices.  Let $E(x,y)$ mean
  that $G$ has an edge between vertices $x$ and $y$.  Since $G$ is
  simple, we have $E(x,y) \QIFF\ E(y,x)$ and also $\QNOT(E(x,x))$.
...
\begin{solution}
\True, since edges are chosen independently, and the left hand event depends only on the two
  edges $\edge{w}{x}$ and $\edge{w}{y}$, and the right hand event
  depends on two different edges.
\end{solution}
...
\[
\pr{E(x,y)\QAND\ W(x,y)} = p^{n-1}(1-p)^{n-2}.
\]
\end{solution}
