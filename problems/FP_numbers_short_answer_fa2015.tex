...
\ppart No integer linear combination of $a^2$ and $b^2$ equals 2.
...
\ppart If $a c = b c \inzmod{n}$, then $a = b \inzmod{n}$.  
...
\iffalse
\ppart If $a c = b c \inzmods{n}$, then $a = b \inzmods{n}$.  
...
\ppart If $j = k \inzmod{\phi(n)}$, then $m^j = m^k \inzmod{n}$.

\begin{solution}
  Need $m$ relatively prime to $n$.
\end{solution}

\ppart If $a = b \inzmod{nm}$, then $a = b \inzmod{n}$.
