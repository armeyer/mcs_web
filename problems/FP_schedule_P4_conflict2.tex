...
\begin{pcomments}
  \pcomment{FP_schedule_P4_conflict2}
  \pcomment{ARM 4/5/16}
\end{pcomments}
...
A minimum time schedule takes length of max chain steps, which by
part~\eqref{maxis5} is 5.  There is a unique minimum task, namely the
empty set$\emptyset$, that must come first, and a unique maximum task
...
implies that $k \ge 4$.  Moreover, there is a 4-processor schedule that takes 5 steps:
