...
\begin{pcomments}
  ...
  \pcomment{same as PS_check_factor_by_digits w/o required explanation}
...
\]
For exactly what integers $k \in \Zintvcc{2}{15}$ is it true that the
sum of the digits of the base 16 representation of every nonnegative
integer is congruent modulo $k$ to that integer?  \iffalse
...
Summing the digits mod $k$ clearly works when $16 \equiv 1 \pmod k$.
This is equivalent to $k \divides 16-1 = 15$.  So the three factors of
15 are $k$'s that work.

To see why only these $k$'s work, just look at hex representation of
16, namely, the string \STR{10}.  The digit-sum requirement means
$16 \equiv 1 + 0 = 1 \pmod k$.
\end{solution}
...
\ppart
Let $x_{d-1}\dots x_1 x_0$ be the representation of $x$ in base $b$,
