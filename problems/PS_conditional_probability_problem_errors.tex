...
\end{solution}
  ...
  Assuming that the correctness of the lecturer's answer and the TA's
  answer are independent of each other, regardless of whether there is
  an error, what is the probability that there is an error in the
  problem?

\begin{editingnotes}
NO, their prob of correctness can depend on the problem while
preserving independence:
\begin{quote}
This assumption is questionable: by
...
\end{solution}
  ...
  \ppart\label{TL} Is event $T$ independent of event $L$ (that is, $\prcond{T}{L} = \pr{T}$)? First, give an argument based on intuition, and then calculate both probabilities to verify your intuition.
