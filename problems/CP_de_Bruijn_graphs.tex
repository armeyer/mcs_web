...
\begin{solution}
A bit-string of length $n$ has exactly $n-k$ locations where a length-($k+1$)
subsequence can begin.  Since there are $2^{k+1}$ length-$(k+1)$
bit-strings, the mininum length, $n$, of any $(k+1)$-good bit-string must
satisfy $n-k \ge 2^{k+1}$, so the mininum length is $2^{k+1} + k$.
This is exactly the length of the bit-string that would be determined by a
walk containing all $2 \cdot 2^k$ edges, $\edges{B_k}$, in the graph $B_k$.
...
Since a walk of length $k$ exists, a path of length at most $k$ can be obtained by removing the cycles in the walk.
