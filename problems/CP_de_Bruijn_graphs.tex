...
\ppart\label{gf3str} Say a string \emph{3-good} if it contains every
...
\ppart Explain why a path in the graph of Figure~\ref{3-debruijn} that
...
satisfy $n -k /ge 2^{k+1}$, so the mininum length is at least $2^{k+1}
+ k$.  This is exactly the length string that would be determined by a
path containing all $2 \cdot 2^k$ edges in the graph $B_k$.

\[
\edges{B_k} \eqdef \set{\diredge{xa}{bx} \suchthat x \in
  \set{0,1}^{k-1} \QAND a,b \in \set{0,1}}
\]

If $y \in \set{0,1}^k$, then $y = xa$ and $y = bz$ for unique strings
$x,z \in \set{0,1}^{k-1}$ and bits $a,b \in \set{0,1}$.  Then by
definition of $\edges{B_k}$, there are exactly two edges out of $y$,
one going to $0x$ and the other to $1x$, so $\outdegr{y} = 2$.
Likewise, there are only two edges into $y$, one from $z0$ and the
other from $z1$, so $\outdegr{y} = 2$.

To get from vertex $b_1b_2\dots b_k$ to $c_1c_2\dots c_k$ with a
length $k$ path, proceed as follows:
\begin{align*}
b_1 b_2\dots b_k & \to c_k b_1 b_2\dots b_{k-1} \to c_{k-1} c_k b_1 b_2 \dots b_{k-2}\\
                & \to \dots \to c_2 c_3\dots c_k b_1 \to c_1c_2\dots c_k
\end{align*}
