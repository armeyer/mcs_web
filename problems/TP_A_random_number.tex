...
Here is a process to construct a random number: first,
...
Let $N$ be the number that you return, and let $F$ be 1 if the first coin flip came up heads, and 0 otherwise.
\bparts
...
Given that $F=0$, we know that the first coin flip was tails. So, we are in the 
...
\begin{align*}
\prob{N+F=5} = \\
\prob{F=1 \text{ and } N=4} = \\
\prob{F=1} \prob{\text{roll }4} = \\
(3/5)*(1/6) =
2/20
\end{align*}
...
\begin{align*}
\prob{N+F=6} = \\
\prob{F=1\text{ and }N=5} + \prob{F=0 \text{ and } N=6} = \\
(3/5)*(1/6) + (2/5)*(1/8) =\\
3/20.
\end{align*}
