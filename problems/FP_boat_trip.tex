...
\ppart Find a closed form for the generating function with coefficients that
represent the number of different ways for T-Pain to bring $n$ items
(burgers, pairs of flip flops, towels, and/or afghans) on his boat trip.
...
\ppart Find a closed form (in terms of $n$) for the number of ways T-Pain
can bring exactly $n$ items with him.
...
\]
so the generating function for T-Pain is $xG(x)$.
We know that the coefficient of $x^n$ in the
...
we conclude that the $n$th coefficient in the series for $G(x)$ is $n+1$.
