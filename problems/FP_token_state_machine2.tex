...
F(b,w) \eqdef\ [b \not\equiv w \pmod{3}],
\]
where $x \equiv y \pmod{3}$ means that $x-y$ is a multiple of 3, or
equivalently, $x$ and $y$ have the same remainder on division by
three.\footnote{The relation $x \equiv y \pmod{n}$ is the central
  topic of Chapter~\bref{number_theory_chap}.}
Prove the following
\begin{claim*}
