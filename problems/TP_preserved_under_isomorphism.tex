...
don't have length.  Likewise for~\eqref{edgecross}, since edges are
not lines on a surface.  On the other hand, the property of being
\textbf{drawable} in the plane without edges crossing is preserved
under isomorphism, and it is an important enough property that
Chapter~\bref{planar_graphs_chap} is devoted entirely to such
\emph{planar} graphs.

Item~\eqref{vertexsubset} is not preserved under isomorphism.
Although it can be useful to use sets as vertices---as was
  done for representing DAGs\inbook{ in
  Theorem~\bref{thm:posetrepsets}}---vertices need not be
