...
\begin{pcomments}
  ...
  \pcomment{excerpted From F07.final, unchanged from S07.final and S06.final}
\end{pcomments}
...
\begin{problem}%[11] \textbf{Stable Marriages}
...
why Bottlecap must be adopted by the Hatfields, or else there will
...
\begin{center}
\begin{tabular}{c|c|c}
Family & Children & Rank in the original list\\ \hline
Grinches:    & Dingdong & 2\\
Scrooges: & Lucy & 2\\
Hatfields:   & Bottlecap & 2\\
McCoys:   & Zippy & 2
\end{tabular}
\end{center}

Since the Hatfields adopt Bottlecap in the family-optimal
(child-pessimal) matching, and the Hatfields are Bottlecap's
favorite and hence optimal choice, they must be matched in all
stable matchings.
...
\parbox{5.5in}{
\ppart[3]\label{ranmarr} For a given assignment, let $X$ be the
...
\parbox{5.5in}{
\ppart[1]\label{HG} If an assignment is chosen at random as in
...
\solution[\vspace{1in}]{Zippy can be assigned to at most one of
  those families, so both families cannot get their first choice,
  Zippy, at the same time.}

\ppart[3]\label{M} Sometimes the upper bound found using Markov's 
...
\solution[\vspace{1in}]{ By part (\ref{ranmarr}) $\expect{X}=1$ and
