...
\begin{pcomments}
  ...
  \pcomment{from: S10}
  \pcomment{edited from MQ_gen_fcn_for_counting ARM 4/28/12}
\end{pcomments}
...
Example: A bouquet of 2 lilies, no roses and 8 tulips satisfies the
constraints.
...
\]
be the bouquet-counting generating function.
...
\[
B_W (x) = 1 + x^4 + x^8 + \cdots = \frac{1}{1-x^4}
\]
...
The function $1/(1-x)^2$ is the generating function for the number of
ways to select $n$ donuts when there are two flavors of donuts, as
explained in Section~\bref{sec:gf_counting}.  So
by~(\bref{cor:donut_binom}),
\[
