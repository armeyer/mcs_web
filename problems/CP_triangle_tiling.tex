...
\emph{Divided Equilateral Triangles}\footnote{Adapted from
  \cite{Velleman94}.} (DETs) can be built up as follows:

\begin{itemize}
\item A single equilateral triangle counts as a DET whose only unit
  subtriangle is itself.

\item If $T \eqdef$ \includegraphics[width=1cm]{tiling_1} is a DET,
  then the equilateral triangle $T'$ built out of four copies of $T$
  as shown in in Figure~\ref{4DET-fig} is also a DET, and the unit
  subtriangles of $T'$ are exactly the unit subtriangles of each of
  the copies of $T$.
\end{itemize}
...
starting triangle itself, so the total number of subtriangles is $1 =
1^2 = n^2$, proving this case.
