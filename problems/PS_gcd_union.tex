...
The theorem has an easy proof as a Corollary of the Unique
Factorization Theorem.  In this problem we develop a proof of
Theorem~(AuB) just by induction and repeated use of
...
\begin{lemma*}
\begin{equation}
d \divides A \QIFF\ d \divides \gcd(A).\tag{dAdgA}
\end{equation}
...
\begin{equation}
d \divides (a \union A) \QIFF d \divides \gcd(a,\gcd(A)),\tag{divauA}
\end{equation}
for all integers $a,d$ and finite sets $A \subset \integers$.
Explain why this proves Lemma~(dAdgA).
...
follows from~(gcd-associativity).
...
d \divides (a \union A)
   ...
   & \QIFF d \divides \gcd(a,\gcd(A))
           & \text{by~(gcddiv)}.
\end{align*}
Lemma~(dAdgA) follows from~(divauA) by choosing $a \in A$.
\end{proof}
\end{solution}
...
\begin{solution}
We prove~\tag{AuB} by showing that
\[
