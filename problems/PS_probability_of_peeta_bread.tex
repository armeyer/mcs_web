...
\begin{solution}
If $k$ is odd and $k \le n$, then the probability is $1/n$.
If $k$ is even and $k \le n$, then the probability is $2/n$.
If $k$ is odd and $k > n$, then the probability is $0$.
If $k$ is even and $k > n$, then the probability is $1/n$.
\end{solution}

\ppart What is the expected number of loaves Peeta will bake on any given day?
\begin{solution}
\begin{staffnotes}
THIS FORMULA NEEDS MORE EXPLANATION:
\end{staffnotes}

\[
\frac{1}{2n} \paren{\frac{n(n+1)}{2} (1+2)} = \frac{3}{4}(n+1)
\]
%ANSWER: (3n)/2
\end{solution}
