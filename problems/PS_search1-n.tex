...
\ppart Let $T$ be a search tree whose labels are the consecutive integers in the
interval $\Zintv{a}{b}$ where $a,b$ are odd integers with $b-a = \sz{T}-1$ (for example, $\Zintv{1}{\sz{T}}$). Prove by structural
...
\inhandout{You may use the fact that the size of a search tree is an
  odd number (see Problem~\bref{CP_leaves_vs_nodes}) and that all subtrees of a search tree have distinct labels.}
...
$\sz{\leftsub{(T)}}$ is odd, so $\nlbl{(T)}$ is even. Finally, again by the inductive hypothesis, the leaves of $\rightsub{(T)}$ are the odd numbers in $\Zintv{(\nlbl{(T)}+1)}{b}$. Because the leaves of $T$ are precisely the leaves of $\leftsub{(T)}$ and the leaves of $\rightsub{(T)}$ (and $T$ is itself not a leaf by assumption), this proves the claim.
