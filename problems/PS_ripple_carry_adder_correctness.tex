...
\begin{problem}
  ...
  Problem~\bref{CP_binary_adder_logic} described

  There is a straighforward way to implement an $n+1$-bit adder as a
  digital circuit: an $n+1$-bit \idx{ripple-carry circuit} has
  $1+2(n+1)$ binary inputs $a_n,\dots, a_1,a_0,b_n, \dots, b_1,b_0,
  c_0$, and $n+2$ binary outputs, $c_{n+1}, s_{n}, \dots, s_{1},
  s_{0}$.  As in Problem~\bref{CP_binary_adder_logic}, the
  ripple-carry circuit is specified by the following formulas:
  \begin{align}
...
\ppart Verify that equations~\eqref{siadddef} and~\eqref{ciadddef}
imply that
...
satisfy~\eqref{adder-iospec}.
 ...
 \begin{equation}\label{alphan+1binrep}
   \binnum{\alpha_{n_+1}} = a_{n+1}2^{n+1} + \binnum{\alpha_n}.
 \end{equation}  
