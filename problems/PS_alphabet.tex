...
popular 1980's band \STR{BANANARAMA} be arranged?
...
the other.  For $n=8$, for example, the sequences \STR{03088929} and
\STR{00238899} are the same type.  How many types of $n$-digit
sequences are there?

\begin{solution}
  The type of a string is determined simply by the numbers of
  occurrences of the digits 0--9 in the string.  So there is a
  bijection between types of strings and strings with $n$ 0's and nine
  1's: the length of the block of 0's before the $i$th 1 (starting
  with $i=0$) equals the number of occurrences of the digit $i$, and
  the length of the block of 0's following the last 1 equals the
  number of occurrences of the digit 9.  Therefore, the number of
  different types is
  \[
  \binom{n+9}{9}\, .
  \]
