...
Suppose $R$ is independent of $S$.  We originally had the following
argument in the class text:
...
Are you convinced by this argument?  We decided to replace it by a
reference to this problem.  We'd like your advice on whether it should
be put back it the text.  Before advising us, write out a careful
proof of~\eqref{RS1V}.
...
\begin{align*}
\pr{R=S}
   & = \sum_{b \in V} \pr{R=S \QAND S=b}
         & \text{(disjoint sum rule)}\\
   & = \sum_{b \in V} \pr{R=b \QAND S=b}\\
   & = \sum_{b \in V} \pr{R=b} \cdot \pr{S=b}
          & \text{$R$ independent of $S$}\\
   & = \sum_{b \in V} (1/\card{V}) \cdot \pr{S=b}\\
   & = (1/\card{V}) \cdot \sum_{b \in V} \pr{S=b}\\
   & = (1/\card{V}) \cdot 1 = 1/\card{V},
\end{align*}
The second equality follows from the fact that $[R=S] \intersect
[S=b]$ is the same event as $[R=b] \intersect [S=b]$.  This
proves~\eqref{RS1V}.
\end{proof}
