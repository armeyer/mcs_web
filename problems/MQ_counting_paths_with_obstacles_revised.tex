...
\begin{problem}
A robot moves on the 3-D integer lattice and it can move a unit
distance in ine direction at a time.  That is, from position
...
For any two points, $P$ and $Q$, in space, let $n(P,Q)$
...
the number or arrangements of the letters of a word with 10 $x$'s, 40
...
Answer the next two questions with arithmetic expressions involving
...
Any path from $A$ to $C$ that goes through $B$ consists of a arbitrary
...
\ppart How many paths from $A$ to $D$ go through neither
$B$ nor $C$?
...
one of $B$ or $C$ equals the number of paths that go through $B$ + the
