...
\begin{problem}
Suppose there is a system, built by Caltech graduates, with
$n$~components.  We know from past experience that any particular
component will fail in a given year with probability~$p$. That is,
letting $F_i$ be the event that the $i$th component fails within one
year, we have
\[
\pr{F_i} = p
\]
...
failures always occur together.  Formally, we model this by assigning
...
\end{align*}
Then the only outcome in $F_i$ with nonzero probability is $\Zintv{1}{n}$, so
\[
\pr{F_i} = \pr{\,\Zintv{1}{n}\,} = p.
\]
Likewise, the only outcome in $F$ with nonzero probability is also $\Zintv{1}{n}$, so
\[
\pr{F} = \pr{\,\Zintv{1}{n}\,} = p,
\]
as required.
...
\pr{F_i} = \pr{\,\set{i}\,} = p.
\]
Finally, the only outcomes in $F$ with positive probability are the
$n$ outcomes $\set{i}$ for $i \in \Zintv{1}{n}$, so
\[
