...
\begin{pcomments}
  \pcomment{PS_binomial_distribution_max}
  \pcomment{from: S02 ps9 revised by ARM 5/1/10}
  \pcomment{copied from CP_binomial_distribution_max}
\end{pcomments}
...
\begin{align*}
\pdf_J(k) & < \pdf_J(k+1) & \text{ for } k<np+p,\\
\pdf_J(k) & > \pdf_J(k+1) & \text{ for } k>np+p.
\end{align*}
...
Use \idx{Stirling's formula}~\bref{nfacsim}:
\[
