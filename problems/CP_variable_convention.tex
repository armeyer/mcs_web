...
\begin{problem}
In predicate formulas as in program procedure or parameter names, it
doesn't really matter if the same variable name is used in separate
places, and it's always safe to change a name to something brand new.
For example, $\forall x. P(x)$ is equivalent to $\forall y. P(y)$ for
any predicate symbol $P$.  So a formula like
...
\item if there is a subformula of the form $\forall x.F$ or the form $\exists
  x.F$, then all the occurrences of $x$ in that appear anywhere in the
...
A bigger example is the formula
...
an equivalent formula that satisfies the unique variable convention
just by renaming some of the occurrences of its variables.  We already
did this when we renamed the first two occurrences of $x$
