...
\begin{solution}
Note that the number of sub-DETs on the base of a DET will always be a power of 2.\\
Proof by induction\\
Induction hypothesis: $P(n)$: a DET with $2^n$ sub-DETs with an edge on its base has $4^n$ sub-DETs.\\
\textbf{Base case: $n=0$} There is 1 sub-DET on the base of T, and 1 sub-DETs total.\\
\textbf{Inductive step:} Assume  by induction that $P(n)$ is true. Then suppose we have a DET with $2^{n+1}$ sub-DETs with an edge on its base. This DET is made out of 4 copies of a DET with $n$ sub-DETs on its base. So the number of sub-DETs in total $= 4 * 4^n = 4^{n+1}$. Thus, $P(n+1)$ is true. Hence, $P(n)$ is true for all $n$.

\begin{comment}
This is a decent false proof. Putting it here for reference
Proof by induction\\
