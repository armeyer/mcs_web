...
\begin{problem}
  ...
  \begin{solution}
    ...
    integers, which appears in the notes.
    ...
    %
    \[
    \gcd(a, b) \mid s a + t b + u c
    \]
    %
    Thus, in particular, $\gcd(a, b, c)$ divides $m$, and so $\gcd(a, b,
    c) \leq m$.

    Now we show that $m \leq \gcd(a, b, c)$.  We do this by showing that
    $m \mid a$.  Symmetric arguments shows that $m \mid b$ and $m \mid c$,
    which means that $m$ is a common divisor of $a$, $b$, and $c$.  Thus,
    $m$ must be less than or equal to the \textit{greatest} common divisor
    of $a$, $b$, and $c$.
    ...
    However, $m$ is the \textit{smallest positive} linear combination and $0
