\documentclass[problem]{mcs}

\begin{pcomments}
 \pcomment{CP_independent_product}
 \pcomment{from: S09.cp14m}
\end{pcomments}

\pkeywords{
  expectation
  independent
  product
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Problem starts here
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{problem}

Justify each line of the following proof
\iffalse  \instatements{(without
  looking it up in Theorem~\ref{th:prod})}
\fi
that if $R_1$ and $R_2$ are
\emph{independent}, then
\[
\expect{R_1 \cdot R_2} = \expect{R_1} \cdot \expect{R_2}.
\]

\begin{proof}
\begin{align*}
\lefteqn{\expect{R_1 \cdot R_2}}\\
 & =  \sum_{r \in \range{R_1\cdot R_2}} r\cdot \pr{R_1\cdot R_2=r}\\
 & =  \sum_{r_i \in \range{R_i}}
            r_1 r_2 \cdot \pr{R_1=r_1\ \text{ and }\ R_2=r_2}\\
 & =  \sum_{r_1 \in \range{R_1}} \sum_{r_2 \in \range{R_2}}
            r_1 r_2 \cdot \pr{R_1=r_1\ \text{ and }\ R_2=r_2}\\
 & =  \sum_{r_1 \in \range{R_1}} \sum_{r_2 \in \range{R_2}}
            r_1 r_2 \cdot \pr{R_1=r_1}\cdot \pr{R_2=r_2}\\
 & =  \sum_{r_1 \in \range{R_1}} \paren{r_1\pr{R_1=r_1} \cdot
              \sum_{r_2 \in \range{R_2}} r_2 \pr{R_2=r_2}}\\
 & =  \sum_{r_1 \in \range{R_1}} r_1\pr{R_1=r_1} \cdot \expect{R_2}\\
 & =   \expect{R_2} \cdot \sum_{r_1 \in \range{R_1}} r_1\pr{R_1=r_1}\\
 & =   \expect{R_2} \cdot  \expect{R_1}.
\end{align*}

\end{proof}

\begin{solution}
Note that the event $[R_1 \cdot R_2=r]$ is the disjoint union of
events $[R_1 = r_1 \QAND R_2 = r_2]$ such that $r_i \in \range{R_i}$
for $i=1,2$ and $r_1 r_2 = r$.

\iffalse
\begin{equation}\label{rr1r2}
[R_1 \cdot R_2=r] = \lgunion_{r_1 r_2 = r} [R_1 = r_1 \QAND R_2 = r_2]
\end{equation}
\fi

\begin{proof}
\begin{align*}
\lefteqn{\expect{R_1 \cdot R_2}}\\
 & \eqdef \sum_{r \in \range{R_1\cdot R_2}} r\cdot \pr{R_1\cdot R_2=r}\\
        &\hspace{2.0in} \text{(by definition)}\\
 & =  \sum_{r_i \in \range{R_i}}
            r_1 r_2 \cdot \pr{R_1=r_1 \QAND R_2=r_2}\\
        &\hspace{2.0in} \text{(remarked above)}\\
 & =  \sum_{r_1 \in \range{R_1}} \sum_{r_2 \in \range{R_2}}
            r_1 r_2 \cdot \pr{R_1=r_1 \QAND R_2=r_2}\\
       &\hspace{2.0in}\text{ordering terms in the sum)}\\
 & =  \sum_{r_1 \in \range{R_1}} \sum_{r_2 \in \range{R_2}}
            r_1 r_2 \cdot \pr{R_1=r_1}\cdot \pr{R_2=r_2}\\
       &\hspace{2.0in} \text{(independence of $R_1,R_2$)}\\
 & =  \sum_{r_1 \in \range{R_1}} \paren{r_1\pr{R_1=r_1} \cdot
              \sum_{r_2 \in \range{R_2}} r_2 \pr{R_2=r_2}}\\
       &\hspace{2.0in} \text{(factor out $r_1\pr{R_1=r_1}$)}\\
 & =  \sum_{r_1 \in \range{R_1}} r_1\pr{R_1=r_1} \cdot \expect{R_2}\\
       &\hspace{2.0in} \text{(def of $\expect{R_2}$)}\\
 & =  \expect{R_2} \cdot \sum_{r_1 \in \range{R_1}} r_1\pr{R_1=r_1}\\
           &\hspace{2.0in} \text{(factor out $\expect{R_2}$)}\\
 & =  \expect{R_2} \cdot  \expect{R_1}.\\
           &\hspace{2.0in} \text{(def of $\expect{R_1}$)}
\end{align*}

\end{proof}
\end{solution}

%\eparts
\end{problem}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Problem ends here
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\endinput
