...
arrange the numbers on the wheel to maximize the expected value of\dots

\bparts
\ppart \dots the sum of the numbers chosen?  What is this maximum?
...
\ppart \dots the product of the numbers chosen?  What is this maximum?

(Hint, verify that the best way to maximize the expected product is to ensure that successive
integers are on the opposite ends of the spinner, that is, $1$ is opposite $2$, $3$ is
opposite 4, 5 is opposite 6, \dots.)

%\hint Verify that the best way to do this is to ensure that successive
%integers are opposite each other, that is, $1$ is opposite $2$, $3$ is
%opposite 4, 5 is opposite 6, \dots.

\begin{solution}
We claim that the best way to maximize the expected product is to have each odd number $k$
opposite $k+1$ starting with $k=1$ up to $k = 2n-1$.  Let's call this
the \emph{successor arrangement}.  We'll show that any arrangement
that differs from the successor arrangement can be altered to have a
larger expected product.  This implies that the successor arrangement
must be the unique arrangement with maximum expected product.
...
opposite numbers are both OK, or neither is OK.  Likewise, with any other
arrangement, numbers that are opposite in the successor arrangement
are either both OK or neither is OK.

Any given arrangement not equal to the successor arrangement must
have a smallest number, $k$, that is not-OK.  Since $k$ is the
smallest number that is not-OK, it must be opposite some larger not-OK number $p$.  Also, $k$ must be odd, or else $k-1$ would be a
smaller number that is not-OK.  Since $k$ is odd, it is opposite $k+1$
in the successor arrangement, and therefore $k+1$ is not-OK and $p
\neq k+1$, which implies $p>k+1$.  This also means that $k+1$ is
opposite some $q$ that is also not-OK.

Now, let's create a new arrangement by putting $k$ opposite $k+1$ and $p$
...
So the expected value of the product is 1/$n$th of
\[
