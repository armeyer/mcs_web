...
\begin{pcomments}
  \pcomment{PS_asymptotics_and_stirlings} 
  \pcomment{from S07, pset8}
\end{pcomments}
...
\begin{solution}Both $n > \ln n$ and $n > (\ln n)^2$ hold for all
...
It is then easy to see that this expression and $n^2 \ln n$ are big-O
of each other by looking at limits as $n$ goes to $\infty$, so we
conclude that $\ln (n^2!) = \Theta(n^2 \ln n)$.
...
\ppart
\[
\sum_{k=1}^{n} k \paren{1 - \frac{1}{2^k}}
\]

\begin{solution}The expression in parentheses is always at least $1/2$ and
