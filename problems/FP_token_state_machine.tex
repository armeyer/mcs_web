...
\item \true.
Same reasonong as for predicate~(2).
...
is an invariant, it may be helpful to think about the problem of
making different postage amounts using 6 and 15 cent stamps, for
example\inbook{ (Problem~\bref{FP_6_and_15_cent_stamps_by WOP})}.  An
easy invariant proof confirms the fact that every makeable postage
amount is divisible by three.  This postage invariant corresponds to
invariance of $T$ for reachable token machine states.
