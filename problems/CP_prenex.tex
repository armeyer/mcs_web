...
\item\label{varconvention} Rename variables to satisfy the \emph{variable convention}
  described in Problem~\bref{CP_variable_convention},

\item\label{toandor} Convert all connectives that have a variable liked to a quantifier to \QAND, \QOR, \QNOT,
...
[\forall x\, \exists y.\, P(x) \QAND Q(x,y)]\ \QIMPLIES 
  (\exists x.\, R(x,y)) \QOR \exists z\, \forall x.\, (S(z) \QXOR T(x)),
\]
...
\lefteqn{[\forall x\, \exists y.\, P(x) \QAND Q(x,y)]\ \QIMPLIES}\\ 
  & \qquad\qquad   (\exists x.\, R(x,y)) \QOR \exists z\, \forall x.\, (S(z) \QXOR T(x))\\
   & \corresp
\lefteqn{[\forall x\, \exists y.\, P(x) \QAND Q(x,y)]\ \QIMPLIES}\\ 
  & \qquad\qquad   (\exists x_1.\, R(x_1,y_1)) \QOR \exists z\, \forall x_2.\, (S(z) \QXOR T(x_2))
         &\text{(step~\ref{varconvention}.)}\\
 & \corresp
  \QNOT(\forall x\, \exists y.\, P(x) \QAND Q(x,y))\ \QOR\\
  & \qquad\qquad   (\exists x_1.\, R(x_1,y_1)) \QOR \exists z\, \forall x_2.\, (S(z) \QXOR T(x_2))
         &\text{(step~\ref{toandor}.)}\\
 & \corresp
  \exists x\, \QNOT(\exists y.\, P(x) \QAND Q(x,y)))\ \QOR\\
  & \qquad\qquad   (\exists x_1.\, R(x_1,y_1)) \QOR \exists z\, \forall x_2.\, ((S(z) \QOR T(x_2)) \QAND (\QNOT S(z) \QOR \QNOT T(x_2)))
         &\text{(start step~\ref{appDeM}.)}\\
 & \corresp
  \exists x\,\forall y.\, \QNOT(P(x) \QAND Q(x,y))\ \QOR\\
  & \qquad\qquad  (\exists x_1.\, R(x_1,y_1)) \QOR \exists z\, \forall x_2.\, ((S(z) \QOR T(x_2)) \QAND (\QNOT S(z) \QOR \QNOT T(x_2)))
         &\text{(repeat step~\ref{appDeM}.)}\\
 & \corresp
  \exists x\,\forall y\, \exists x_1.\,  \exists z\, \forall x_2.\,\\
  & \qquad\qquad  \QNOT(P(x) \QAND Q(x,y)) \QOR  R(x_1,y_1) \QOR  ((S(z) \QOR T(x_2)) \QAND (\QNOT S(z) \QOR \QNOT T(x_2))).
         &\text{(step~\ref{qtfront}.)}\\
\end{align*}
