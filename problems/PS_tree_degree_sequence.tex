...
Let $D = (d_1,d_2,\ldots,d_n)$ be a  sequence of positive integers were $n\geq 2$.

\bparts
\ppart\label{occnum}

Suppose $D$ is a list of the degrees of vertices of some tree $T$,
that is, $d_i$ is the degree of the $i$th vertex of $T$.  Explain why:
\begin{equation}\label{eq:tree_degree_seq}
\sum_{i=1}^n d_i = 2(n-1)
\end{equation}

\begin{solution}
By the Handshaking Lemma~\bref{sumedges}, sum of the degrees of the
vertices of $T$ is twice the number of edges, and since $T$ is a tree with
$n$ vertices, it has $n-1$ edges.
\end{solution}

\ppart\label{7pats} Prove that if $D$ satisfies
equation~\eqref{eq:tree_degree_seq}, then there is a tree with degree
sequence $D$.  \hint Induction.

\begin{solution}
    The proof will be by induction on $n$ with induction hypothesis
\[
P(n) \eqdef \forall D = (d_1,\dots,d_n).  D
satisfies~\eqref{eq:tree_degree_seq}\ \QIMPLIES\ \exists\text{ tree }
T.\, D \text{ is the degree sequence of } T.
\]

\inductioncase{Base case} ($n=2$): In this case $D$ must equal $(1,1)$
which is the degree sequence of a 2-vertex tree, $T$.
...
for all $i$ and the sum would be too large.  Similarly, there must be a
$j$ such that $d_j\geq 2$, otherwise the sum would be too small.  We
can assume wlog that $d_{n}=1$ and $d_{n-1} \geq 2$.

Let $k \eqdef n-1$ and $D' = (d_1,d_2,\ldots,d_{k})$.  Observe that
the sum of $D'$ equals $2(n-1)-2 = 2(k-1)$, so $D'$ satisfies equation
\eqref{eq:tree_degree_seq} for the case that $n$ is $k$.  Assuming the
induction hypothesis, $P(k)$, we know that there is a tree $T'$ with
degree sequence $D'$.  Let $T$ be the tree obtained by adding a new
vertex to $T'$ which is adjacent to the $k$th vertex of $T'$.  Now $T$
has degree sequence $D$, concluding the proof.
\end{solution}

\ppart\label{pat-decode} Assume that $D$ satisfies equation
\eqref{eq:tree_degree_seq}.  Show that it is possible to partition $D$
into two sets $S_1, S_2$ such that the sum of the elements in both
parts is $n-1$. \hint Trees are bipartite.
