...
$a \divides b$ iff $b=ak$ for some $k \in \naturals$.  The third row
starts with the less than relation on $\naturals$ and the complex
number, $i$, where $a < b$ iff $a, b \in \naturals$ and $a < b$.
...
\ppart What is the longest \emph{chain} for the subset relation,
...
\ppart What is the largest \emph{anti-chain} on the subset relation,
$\subseteq$, on $\power(\set{1, 2, 3})$?  (If there is more than one,
provide \emph{one} of them.)
...
\begin{solution}
One maximal antichain is $\set{1}$, $\set{2}$, $\set{3}$.

Another is $\set{1,2}$, $\set{2,3}$, $\set{1,3}$.
\end{solution}
