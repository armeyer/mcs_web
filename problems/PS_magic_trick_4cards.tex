...
Section~\bref{4_card_trick-subsec} explained why it is not possible to
...
Assistant computes, $s$, the sum modulo 4 of the four cards, and
chooses the card with suit $s$ to be placed \emph{face down as the
  first card}.  He then uses a permutation of the remaining three
face-up cards to code the rank of the face down card.\footnote{This
  elegant method was devised in Fall '09 by student Katie E Everett.}
}
...
down, so she knows that the hidden card must be of a suit different
from the three different suits of the face-up cards.  That is, the
Magician knows the suit of the face-down card, which means she knows
the number, $s$, of that suit.

Let $t$ be the mod 4 sum of the three face-up cards.  Since $s$ was
chosen by the Assistant to be the sum mod 4 of all four cards, the
Magician knows what the face-down card is congruent to mod 4, namely
it is $\equiv s - t \pmod 4$.  Since at most four numbers from 1 to 13
can be congruent to each other mod 4, the six possible permutations of
the three face-up cards are more than enough to code which of the four
possible numbers congruent to $s - t \pmod 4$ is the rank of the
hidden card.
...
cards) with a 53-card deck consisting of the usual 53 cards along with
...
4-card face-down trick placing the joker (face up) where the face-down
card would be.
\end{solution}
