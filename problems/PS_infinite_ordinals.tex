...
Prove conversely that if a set $x$ is full then $x\in\Ord$.
\hint use Foundation.

\begin{proof}
Suppose there is a full set not in \Ord.
Let $x$ be a member-minimal such set.  Since $x$ is full

So every element $y$ of $x$ is in \Ord.
...
Show that every set has a bijection to some ordinal.

The punchline: now we can define the \emph{cardinality} $\card{S}$ of
any set $S$ to be the member-minimal ordinal $\nu$ such that $S \bij
\nu$, and we arrive at the fundamental property that $S \bij T$ iff
$\card{S} = \card{T}$.
\end{staffnotes}
\eparts
