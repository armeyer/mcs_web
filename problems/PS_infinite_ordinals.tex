...
\emptyset & \in \Ord,\\
\text{if } \nu & \in \Ord, \text{ then } \nextset{\nu} \in \Ord,\\
\text{if } \emptyset \neq S & \subset \Ord, \text{ then } \lgunion_{\nu \in S} \nu \in \Ord.
\end{align*}
\end{definition*}
...
\begin{itemize}
\item $P(\emptyset)$
\item If $P$ holds for a set $x$, it holds for $\nextset{x}$, and
\item if $P$ holds for all members of some nonempty set $S$, then it holds for
  their union.
\end{itemize}  That is:
...
\Rule{P(\emptyset)\\
\forall x.\ P(x)\ \QIMPLIES\ P(\nextset{x}),\\
\forall S \neq \emptyset.\ (\forall x \in S.\, P(x))\ \QIMPLIES\ P(\lgunion_{x \in S} x)}
{\forall \nu \in \Ord.\, P(\nu)}
\end{mathrule*}
