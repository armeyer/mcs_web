...
\ppart\label{enumerate_numstr} Describe a bijection between $\mathbb{N}$ and $\numstr$.
...
\ppart\label{no_algorithm_for_numstr} Conclude that $\asciistr$ is not a valid set, and that Ben's efforts are futile.

\begin{solution}
The solution to part~\eqref{diagonalization} is an English description of a number, so it must be in $\numstr$.  That number $q$ must therefore be in $S$, but we have just shown otherwise---a contradiction.  So our assumption that $\asciistr$ is a valid set is false, and there is no algorithm which determines whether a string is a description of a number.
\end{solution}
