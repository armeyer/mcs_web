...
\begin{solution}
The proof given on Problem~\bref{CP_RSA_proving_correctness} that RSA
decryption works requires only that $de \equiv 1 \pmod{(p-1)(q-1)}$
where $n$ is the product of primes, $p,q$.  But this requirement is
symmetric in $d$ and $e$, so reversing their roles allows the private
key $d$ to be used to encrypt and the public key $e$ to decrypt.
...
Charlie can send the encrypted message $\hat{m}=\rem{m^{e}}{n}$ to
Sierra.  Sierra can reproduce $m$ from $\hat{m}$ by computing
$m=\rem{m^{*d}}{n}$ and no one else can read $m^*$ because they do not
know the private key.  However, since the public key is known to
everyone, a third party opponent Oscar can pretend to be Charlie and
send Sierra a message encrypted by the public key.  In this problem we
will see RSA can also prevent forged messages when the keys are used
in reverse.
...
$\hat{m}=\rem{m^{d}}{n}$ and sends $\hat{m}$ to Charlie.
Explain how Charlie can reproduce the original message $m$
with only $\hat{m}$ and $(e,n)$ and why this method will return $m$.
