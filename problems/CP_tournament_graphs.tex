...
\begin{pcomments}
  ...
  \pcomment{Could use some revision (one of the
      parts was redundant in the version used in class).}
  \pcomment{revised by ARM 10/16/09}
\end{pcomments}
...
\begin{problem} %{\bf Tournament Graphs}
  ...
  We can model the results of such a tournament using a \term{tournament
    digraph} where the vertices correspond to players and there is an edge
  $x \beats y$ if $x$ beat $y$.

  \bparts

  \ppart Explain why there can be no cycles of length  1 or 2.
