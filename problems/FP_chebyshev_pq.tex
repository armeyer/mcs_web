...
$n$ for $\expect{H}$, the expected number of Heads.
...
flips, that is, $\pr{H \geq \expect{H} n/100}$.
...
the probability that $H$ differs from $\expect{H}$ number by at least
$n/100$
\[
100^2\frac{p(1-p)}{n}.
\]
...
\begin{solution}
\begin{align*}
\pr{\card{H-expect{H}} \geq \frac{n}{100}}
...
m$ times for a certain number $m$, there is a 95\% chance that the
proportion of Heads among these $m$ flips will be within 0.01 of $p$.
