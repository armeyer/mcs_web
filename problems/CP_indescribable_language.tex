...
Problem\inbook{ Theorem~bref{{nohalt_thm}h}.
...
words over $A$.  For example, an \ascciibet\ word is a word over
...
\begin{equation} \tag{no-1s} %\tag{\all0s} %\label{no1}
...
\item  $\text{length-2}(t)$ meaning $t$ consist of two consective binary symbols.
...
Let
\begin{align*}
\text{starts-right}(t)& \eqdef \exist x.\, t = \mtt{01}x,\\
\text{ends-right}(t)  & \eqdef \exist x.\, t = x\mtt{01},\\
\text{length-2}(t)    & \eqdef t = \mtt{00}\ \QOR t = \mtt{01} \QOR t = \mtt{10} \QOR t = \mtt{11},\\
\text{moves-right}(t) & \eqdef \exist x,y,z.\,
...
the formula where the variable $s$ is replaced by the \acsii\ word
$y$.  So,
\[
\desc{x} = \set{y \in \ascciistr \suchthat x(y) \text{ is true}}.
\]
...
x \in \desc(x)
\]
when $x \in \assciistr$ is a word-logic formula.

It simplifies things if we make every \ascciibet\ word $x$ describe
