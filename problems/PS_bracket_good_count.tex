...
that $P(\mtt{[}s\mtt{]}t)$ is true.

The count values for $\mtt{[}s\mtt{]}t$ start with 0.  Reading the
initial left bracket yields 1 as the next count value.  This 1 serves
as the start of a series of count values exactly equal to the count
values of $s$, with each value incremented by one.  Since $s \in \GC$
by hypothesis, these incremented count values begin with 1, always
stay positive, and end with 1.  The right bracket immediately after
$s$ reduces the ending count to 0.  This 0 serves as the start of the
remaining count values which are exactly the count values of $t$.
Since $t \in \GC$, these remaining values never go negative and end at
0.  Hence the entire sequence of count values for
$\mtt{[}s\mtt{]}t$ starts with 0, never goes negative, and ends
  with 0, which proves that $\mtt{[}s\mtt{]}t \in \GC$.
...
case in the definition of $\RM$, we conclude $r = \mtt{[}s\mtt{]}t \in
\RM$.
