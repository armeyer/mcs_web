...
%\begin{definition}\label{RM-def_pset}

One way to determine if a string has matching brackets, that is, of it is
on the set, \RM,\inbook{ of
  Definition~\bref{RM_def}} 
\inhandout{\footnote{The set, $\RM$, of strings of brackets
...
\end{itemize}}}
is to start with 0 and read the string from left to right,
adding 1 to the count for each left bracket and subtracting 1 from the
count for each right bracket.  For example, here are the counts for two
sample strings:
...
by convention, that is, $\emptystring \GC$.  The matched strings can now
