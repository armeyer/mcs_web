...
give an upper bound to this.  Because the fail rate is $5\%$, $EX[X] =
50$.  From Chernoff, we have that
\[
\prob{X \geq 2 \cdot 50} \leq e^{-(2\ln 2 - 2 + 1)\cdot 50}
\]
...
\[
\prob{X \geq 18 \cdot 50} \leq e^{-(18\ln 18 - 18 + 1)\cdot 50}
\]
...
\ppart Now, do not assume that the loans are independent.  Give an
...
\[
\prob{X \geq 9/5*5000} \leq e^{-(9/5 \ln(9/5) - 9/5 + 1) \cdot 5000}
\] 
...
\[
\prob{X \geq 7/5*5000} \leq e^{-(7/5 \ln(7/5) - 7/5 + 1) \cdot 5000}
\] 
...
from Markov's theorem, like we did in part b.  Thus, the upper bound
on failures in the top tranche, or over $9000$ failures, is:
\[
