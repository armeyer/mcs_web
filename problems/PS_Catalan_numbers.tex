...
\begin{problem} The method of partial fractions can only be applied
to rational functions (i.e. quotients of polynomials) and there are 
many interesting generating functions that are not of this form. 
...
\footnote{\href{http://www-math.mit.edu/~rstan/ec/catadd.pdf}
               {http://www-math.mit.edu/~rstan/ec/catadd.pdf}}

We are only going to look at one that is relevant to a familiar 
recursively defined set from the 
\href{http://courses.csail.mit.edu/6.042/spring08/ln4.pdf#struct.induction}
     {lecture notes}
on structural induction:
     
\begin{definition}
\label{M}
Define the set, $M$, of strings of matched right and left parentheses
recursively as follows:
\begin{itemize}

\item \textbf{Base case:} $\lambda \in M$, where $\lambda$ is the
\emph{empty} string,

\item \textbf{Constructor case:} if $s,t \in M$, then
$\texttt{(}s\texttt{)}t \in M$.

\end{itemize}
\end{definition}

The $n$th Catalan number, $c_n$, is equal to the number of strings in 
$M$ having exactly $n$ left parentheses.

Note that $c_0 = 1$ since $\lambda$ is the only string in $M$ with $0$ 
left parentheses.
...
$s$ and $t$ are in $M$, what are the possible pairs of integers 
describing the numbers of left parentheses in each such that 
$\texttt{(}s\texttt{)}t$ has exactly $n$ left parentheses in total?})

\begin{solution}Define $|s|$ to be the number of left parentheses in the 
string $s \in M$.  Answering the question posed in the hint, if
