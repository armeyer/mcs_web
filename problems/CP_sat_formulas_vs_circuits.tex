...
(There is a simple way to construct, given $C$, a formula that is
equivalent to $C$ in the sense that the truth value of the formula and
the output of $C$ are the same for every truth assignment of the
variables.  In general, however, such a formula will be exponentially
larger than $C$.  So instead of an equivalent formula, we aim for an
``equisatisfiable'' formula $F_C$ that is assured to be roughly the
size of $C$.)
