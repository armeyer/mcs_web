...
\begin{solution}
We can assign a variable to each input wire to any gate. For each gate, with input wire variables $A$ and $B$ and output $C$, we can set up a proposition that represents the constraint of the gate. So, if for example the gate is an AND gate, the proposition is $(A \wedge B) \iff C$. There are $n$ total wires, and no two gates have the same output wire. So, there are at most $n$ propositions that correspond to all the gates. The AND of all these proposition together with the single variable corresponding to the output wire will be satisfiable if and only if the circuit is too. Thus, it satisfies the requisites to SAT-represent the circuit function. This means that, given any circuit-SAT problem, we can reduce it to an equivalent formula-SAT problem, so if we can solve formula-SAT efficiently we can do the same for circuit-SAT. This means that circuit-SAT is no harder than formula-SAT.
\end{solution}
