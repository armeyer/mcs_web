...
\begin{problem}
\PTAGfilename{CP0303_erasable_strings.tex}
\PTAGhistory{S09_cp5m}
\PTAGkeywords{structural_induction}
%
...
\solution{The base case is [$n = 0$].  Now $P(0)$ is true because the
...
\solution{In this case $x = p \in \RM$.}
...
\solution{The proof is essentially identical to the previous case, with
  $t,t'$ in place of $s,s'$:
...
\solution{Let $t' \eqdef \mtt{(}s\mtt{)}t$ and $s'$ be the empty string.
