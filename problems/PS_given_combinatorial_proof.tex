...
\begin{pcomments}
  \pcomment{PS_given_combinatorial_proof}
  \pcomment{used to be PS_binomial_problem}
...
\iffalse
This problem deals with combinatorial proofs.  When a combinatorial proof
is asked for, an algebraic proof will not be accepted.
\bparts
\ppart
Find a combinatorial proof that $\displaystyle\sum_{i=0}^n \binom{n}{i}$
is equal to $2^n$.

%\hint Think about the binomial theorem.
\begin{solution}%Both sides are the sum of the coefficients of $(x + y)^n$.

  There are $2^n$ subsets of $\set{1,2,\dots,n}$.  But the number of such
  subsets is the number of subsets of size $k$, namely,
  $\binom{n}{k}$ summed for $0 \leq k\leq n$.
\end{solution}
