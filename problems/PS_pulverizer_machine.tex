...
\text{transitions} & \eqdef (x,  y,  s,  t,  u,  v) \movesto\\
     & (y, \rem{x}{y}, u - s\qcnt{x}{y}, v - t\qcnt{x}{y}, s, t)
             & \text{(for $y>0$)}
\end{align*}

Note that ${x,y}$ follows the transition rules of the Euclidean
algorithm given in equation~(\bref{euclid_transition}), except that
this machine stops one step sooner, ensuring that $y \gcd(a,b)$ at the
end.  So for all inputs $x,y$, this procedure terminates after at most
the same of transitions as the Euclidean algorithm.
...
\begin{solution}
It's easy to check that all three preserved invariants are true just
