...
\begin{pcomments}
  ...
  \pcomment{add edited ARM 5/20/17}
\end{pcomments}
...
friends.  Hearts uses a standard deck of 52 cards with thirteen cards
of each suit: spadees, hearts, diamonds, clubs.  At the start,
...
\begin{solution}
Let $X_i$ be the indicator variable that is 1 if your $i$th card is a
heart and is 0 otherwise.  So $H = \sum_{i=1}^{13} X_i$.  Because
$X_i$ is an indicator variable, its expectation is the probability
that the $i$th card is a heart, namely, 1/4.  So
\[
\expect{H} = \sum_1^{13} 1/4 = 13/4.
\]
\end{solution}

\ppart\label{numHinvar} Write a numerical formula for $\variance{H}$.
Your formula may contain binomial coefficients; you do not need to
evaluate it.
...
\begin{align}
\Expect{H^2}
    & \eqdef \Expect{\paren{\sum_{i=1}^{13} X_i}^2} \notag\\
    & = \Expect{\sum_{i=1}^{13} X_i^2 + \sum_{i\neq j} X_iX_j} \notag\\
    & = \sum_{i=1}^{13} \Expect{X_i^2} + \sum_{i\neq j} \Expect{X_iX_j}.\tag{E2Eij}
\end{align}
Now we note that $\expect{X_i^2} = \expect{X_i}$ since $X_i$ is an
indicator variable, so $\expect{X_i^2} = 1/4$.

When $i \neq j$, the product $X_iX_j = 1$ iff the $i$th and $j$th
cards are both hearts.  The probability that both cards are hearts is
$(1/4)(12/51)$, and thus
\[
\expect{X_iX_j} = (1/4)(12/51) = 3/51.
\]
In the summation~(E2Eij), there are 13 $\expect{X_i^2}$ terms,
and $2\binom{13}{2} = 156$ terms of the form $\expect{X_iX_j}$.  Thus,
\[
...
\begin{solution}
Let $X_h$ be the indicator variable which is 1 if there is a heart in
the hand and 0 otherwise. Define $X_s, X_d, X_c$ similarly for the
three remaining suits.  Then
\[
N = X_h + X_s + X_d + X_c.
\]
Note that $X_h$ is the same as $H$, so
\[
\expect{N} \eqdef \expect{X_s + X_h + X_d + X_c} = 4 \expect{H} %\approx 3.94884.
\]
\end{solution}
