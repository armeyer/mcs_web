...
\begin{pcomments}
  \pcomment{DRAFT need last two parts}
  \pcomment{CP_expected_number_of_keys_deviation_no_replace}
  \pcomment{subset of PS_n_keys, CP_expected_number_of_keys}
  \pcomment{part on F15.final}
  \pcomment{ARM 12/18/15}
\end{pcomments}
...
each try, that is
\begin{equation}\label{prT=k1n}
\pr{T=k} = 1/n
\end{equation}
for $k \in \Zintv{1}{n}$, and hence the probability that it comes up after the $k$th try is
\[
...
\begin{solution}
We have from~\eqref{prT=k1n} that $\pr{T = k} = \frac{1}{n}$.
...
\expectsq{T}
   ...
   & = \sum_{k=1}^n k^2\frac{1}{n} & \text{(part~\eqref{prT=k1n})}\\
   & = \frac{\sum_{k=1}^n k^2}{n}\\
   & = \frac{n(n+1)(2n+1)}{6n}
           & \text{(Problem~\bref{CP_sum_of_square} or \bref{CP_gen_func_sum_of_squares})}\\
\variance{T}
    & = \frac{\binom{n}{3}}{2n}
         & \text{(algebra)}.
\end{align*}
...
\ppart \TBA{Chebshev} vs. actual $\pr{T>an}$
