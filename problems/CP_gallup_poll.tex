...
probability $q$, which is $(1-p)$, being 0.  By Lemma~\bref{bernoulli-variance}, $\variance{H} = pq = p(1-p)$.
...
Noting that the derivative of $p(1-p)$ with respect to $p$, $2p-1$, is zero when $p=1/2$, it follows that
$p(1-p)$ is maximal when $p = 1/2$, so the maximum value
