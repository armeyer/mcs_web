...
\ppart Explain wht
\[
\expect{T} =n,\quad \text{and} \variance{T} = n(n-1).
\]
...
\ppart Derive a bound for $\pr{T \geq an}$ that for fixed $n$ as $a
\to \infty$ is asymptoticaly smaller than Chebyshev's
bound~\eqref{T2nO1n2}.
...
\pr{T \geq an}
  ...
  & = q^{an-1}\\
  & = (1-p)^{-1} (1-p)^{an}\\
  & = \frac{n}{n-1} \paren{1-frac{1}{n}}^{an}\\
  & \sim \frac{1}{e^a} = o(\frac{1}{a^2},
\end{align*}
