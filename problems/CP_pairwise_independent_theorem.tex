...
% *****  Solution re-organized for Spring'14 cp13f.  Will revert back after PDF confirmed  *****
...
comment on where changes must be made.  Here's the revised proof:
...
% *****  Solution used for Spring'14 cp13f.  Will revert back after PDF confirmed  *****
\begin{solution}
Essentially identical to the proof of
%Theorem~\bref{th:pairwise-sampling} in the text, except that $S_n/n$
%Theorem~\ref{th:pairwise-sampling} in the text, except that $S_n/n$
...
\Variance{A_n}
 & =  \frac{1}{n^2} \Variance{\sum_{i=1}^n X_i} 
          & \text{(def of $A_n$ \& Square Multiple Rule for Variance, Theorem~\bref{var.const})}\\
 & =  \frac{1}{n^2} \sum_{i=1}^n \variance{X_i}
        & \text{(pairwise independent additivity)}\\
 & \leq \frac{1}{n^2}\cdot nb
   & (\text{since } \variance{X_i} \leq b)\\
