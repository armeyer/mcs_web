...
\ppart\label{unioncount} Prove that the union of two countable sets is
countable.
\begin{editingnotes}
...
Let $\finbin$ be the set of finite binary sequences, and let $F$ be
the set of infinite binary sequences that contain only a
\textbf{f}inite number of occurrences of $\mtt{1}$'s.

\bparts

\ppart\label{binwsurjbarF}  Describe a simple surjection from $\finbin$ to $F$.
...
\ppart $\bar{F} \eqdef \binw - F$ is the set of infinite binary
sequences with \emph{infinitely} many $\mtt{1}$'s.  Use the previous
problem part\inhandout{s} to prove that $\bar{F}$ is uncountable.
