...
One approach involves recasting this set equality as a propositional formula that must be true for all possible values of a test element, $x$.  Write this formula, but do not simplify it.\\\\
Your formula must contain only propositional variables and their negations, parentheses, and operators such as $\QNOT$, $\QOR$, $\QAND$, and $\QIFF$.\\\\
\textit{Hint:}  You may find it useful to define three propositions describing the membership of $x$ in each of the sets $A$, $B$, and $C$:
\begin{eqnarray*}
P & \eqdef & x\in A\\
Q & \eqdef & x\in B\\
R & \eqdef & x\in C\\
