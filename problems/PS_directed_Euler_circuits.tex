...
\begin{problem}
  In this problem we'll consider some special closed walks in graphs
...
incident to it and an Euler tour can't traverse $(E,F)$ twice.  But
...
walk that traverses each edge \emph{at most} once.
...
the walk whose head or tail is on the walk.

\begin{solution}
  If an edge is not on the walk but its head or tail is on the Euler
  walk, that already is the desired edge.  So suppose there's an edge,
  $e$, not on the walk and both ends of $e$ are not on the Euler walk.
  Since $G$ is weakly connected, there is a walk in $G \union G^{-1}$
  from any vertex, $v$, on the Euler walk an endpoint of $e$.  Then
  the first edge on the walk from $v$ to $e$ is the desired edge of
  $G$.
\end{solution}
...
\begin{solution}
Suppose $\walkv{w}$ is closed and some edge is not in $\walkv{w}$.  By
part~\eqref{conn}, there must be a vertex, $v$, in $\walkv{w}$ that is the head
or tail of an edge not in $\walkv{w}$.  Suppose $v$ is the tail of that edge.
Then starting at $v$, go around $\walkv{w}$ back to vertex $v$, and then the
follow the edge.  This makes a longer Euler walk, contradicting the
maximality of $\walkv{w}$.  Similarly, if $v$ is the head of an edge not in
$\walkv{w}$, then starting at the tail of that edge, following it to $v$ and
then going around $\walkv{w}$ to get back to $v$ also makes a longer Euler
walk, again contradicting maximality.

So no edge can be missing from $\walkv{w}$.
\end{solution}

\ppart\label{already} Explain why all the edges incident to the end of
$\walkv{w}$ must be on $\walkv{w}$.

\begin{solution}
Otherwise we could extend $\walkv{w}$ to a longer Euler walk with any edge
from the end not already in $\walkv{w}$.
\end{solution}
...
these pairs are distinct.  In addition, the final edge traversed by
$\walkv{w}$ as it ends at $v$ is distinct from all the paired edges.
Altogether, this implies that there are an equal number of edges into
$v$ and out of $v$ in $\walkv{w}$ except for the last edge, and the
last edge adds one more to the in-degree.  But by
part~\eqref{already}, these are all the edges incident to $v$, proving
that $\indegr{v} = 1+ \outdegr{v}$.
\end{solution}
