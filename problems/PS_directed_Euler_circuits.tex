...
\ppart\label{conn} Suppose that a trail in a connected graph
...
follow $\walkv{w}$ from $e$'s start vertex $v$ until the end, then
rejoin $\walkv{w}$ from its beginning and
continue until reaching $v$, and then finally follow $e$.  This
trail is longer than $\walkv{w}$ (by one), contradicting its maximality.
Similarly, if $e$ ends
