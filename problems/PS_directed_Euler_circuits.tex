...
\begin{solution}
  ...
  Since there is a walk in $G \union G^{-1}$ each endpoint of $e$ and
  a vertex on the Euler walk, there must be a shortest walk from an
  endpoint of $e$ to a vertex on the Euler walk.  All the edges on
  this shortest walk must also not be on the Euler walk or the
  shortest could be shortened, so the last edge on the shortest walk,
  or its reversal, will be the desired edge of $G$.
\end{solution}

\eparts

In the remaining parts, let $W$ be the \emph{longest} Euler walk in
the graph.
...
\begin{solution}
Otherwise we could extend $W$ to a longer Euler walk with any edge
...
walk, all the edges in all these pairs are distinct.  In addition, the
