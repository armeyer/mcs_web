...
\ppart\label{odd} Show that if $\walkv{w}$ was not closed and let $v$ be the end vertex of $\walkv{w}$,
then the in-degree of $v$ would be strictly bigger than its out-degree, ie. $\indegr{v} \geq \outdegr{v} +1$.
...
one more to the in-degree of $v$.  But by part~\eqref{already},  all
the edges starting at $v$ are accounted for. But we may also have edges not in $\walkv{w}$ ending at $v$  proving that $\indegr{v} \geq  \outdegr{v} +1$.
\end{solution}
...
\begin{solution}
If all vertices in $G$ have equal in-degree and out-degree, then by part~\eqref{odd}, the
