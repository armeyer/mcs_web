...
\begin{problem}
  An \term{Euler tour}\footnote{In some other texts, this is called an
    \term{Euler circuit}.} of a graph is a closed walk that includes
...
A \term{trail} is a walk in which each edge occurs \iffalse To do
this, let's define an \term{Euler walk} to be a walk that includes
each edge \fi \emph{at most} once.

\bparts

\ppart\label{conn} Suppose that an trail in a connected graph
...
\begin{solution}
  If an edge is not on the trail starts or ends on it, then that
...
In the remaining parts, let $\walkv{w}$ be the \emph{longest} trail in
the graph.
...
$v$, and then the follow $e$.  This makes a longer trail,
contradicting the maximality of $\walkv{w}$.  Similarly, if $e$ ends
on $\walkv{w}$, then the walk that begins with $e$ and then follows
$\trailv{w}$ from the end of $e$ back around to the end of $e$ is a
longer trail, again contradicting maximality.
...
\ppart\label{biggerthanout} Show that if the end of $\walkv{w}$ was not closed,
...
enters $v$ and so add 1 more to the in-degree.  So there is one more
