...
\item[iii.] Define the \emph{size} $\sigma$ of a game position to be
...
\ppart Conclude that there is no infinite antichain under the
coordinatewise partial order on
$\nngint^2$.\footnote{\term{Coordinatewise partial order} $\coordle$
  on $\nngint^2$ is defined by the rule:
\[
