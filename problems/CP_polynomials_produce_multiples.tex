...
\end{definition*}
  ...
  polynomials given above, prove by structural induction that for all
  $q \in P$,
\[
...
the integers from 0 to 39 (see Section~\bref{prop_sec}).  Well, $p$
didn't work, but are there any other polynomials whose values are
always prime?  No way!  In fact, we'll prove a much stronger claim.
\end{staffnotes}
...
\]
by definition of $\equiv \pmod v$.  Now part~\eqref{jk} implies that
...
q(k) \equiv 0 \pmod v.
\]
So all the elements in the sequence~\eqef{qkqkv} are multiples of $v$.
