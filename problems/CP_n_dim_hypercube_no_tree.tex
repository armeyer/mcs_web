...
\begin{pcomments}
  \pcomment{CP_n_dim_hypercube_no_tree}
  \pcomment{same as CP_n_dim_hypercube w/o part(a)}
...
\begin{problem}
  ...
  \STR{111} and \STR{011} are adjacent because they differ only in
  the first bit, while vertices \STR{101} and \STR{011} are not
...
differ in only one position.  For $n=4$, for example, vertex \STR{0000}
is adjacent to the length-4 binary strings with exactly one \STR{1},
namely, the four strings \STR{1000}, \STR{0100}, \STR{0010},
\STR{0001}.
\fi
...
by symmetry, just consider paths between the vertices \STR{000} and
\STR{001}.

Paths from \STR{000} to \STR{001}:
\begin{align*}
\mtt{000, 001}\\
\mtt{000, 010, 011, 001}\\
\mtt{000, 100, 101, 001}
...
Finally, for distance 3 from \STR{000} to \STR{111}:
\begin{align*}
\mtt{000, 001, 011, 111}\\
\mtt{000, 010, 110, 111}\\
\mtt{000, 100, 101, 111}
...
both start with \STR{0}.  That is $v=\mtt{0}v'$ and $w=\mtt{0}w'$ for
...
\STR{0}, the only vertices besides the endpoints that $\pi_{n+1}$ could
...
\end{align*}
\end{definition*}
The $n$-connectness of $H_n$ then follows immediately from a known
...
\begin{theorem*}[Cartesian connectedness]
If $G$ is $m$-connected and $H$ is $n$-connected, then
$\gprod{G}{H}$ is $(m+n)$-connected.
\end{theorem*}
We expect that the proof above for $H_n$ can be extended to prove
this Cartesian connectedness Theorem.
...
\conntd{\gprod{G}{H}} = \min\{\conntd{G} \cdot v_H,
                              \conntd{H} \cdot v_G,
                               d_G+d_H\}
\]
where $v_G = \card{\vertices{G}}$, $d_G$ is the minumum degree of the
vertices of $G$, and $v_H$ and $d_H$ are defined correspondingly.
\end{editingnotes}
