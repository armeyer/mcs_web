...
\begin{solution}
Clearly, $\card{\nu_0} =0$ by definitiion, and for $n>0$, 
\[
\nu_{n} = \set{\nu_0, \nu_1,\dots, \nu_{n-1}}
\]
by~\eqref{nun+1}.  Therefore $\card{\nu_n} = \card{\set{0,1,\cdots,
    n-1}} = n$ providing all the $\nu_i$'s \emph{are different}.  But
this follows immediately from~\eqref{nun+1}, because if $i<j$, then
$\nu_i \in \nu_j$, and so $\nu_i$ must not equal $\nu_j$ or it would
