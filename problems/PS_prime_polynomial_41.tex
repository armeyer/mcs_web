...
\begin{problem}
  ...
  as noted in Section~\bref{prop_sec}\inhandout{ of the course text}.  But
...
\ppart\label{consc>1} Show that if $q$ is nonconstant and $c > 1$, then as
$n$ ranges over the nonnegative integers, $\naturals$, there there are
infinitely many $q(n) \in \integers$ that are \idx{not primes}.
...
\begin{staffnotes}
(Spring 2011) This part of the problem was modified to make them prove there are \emph{infinitely} many non prime integers in the image of the polynomial, but originally only required them to show at least one exists. Do \emph{not} penalize them in the grading if they proved the weaker claim.
\end{staffnotes}
