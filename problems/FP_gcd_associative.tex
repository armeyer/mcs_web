...
\ppart What is the $\gcd(m,n,p)$?\hfill \examrule{1.5in}

\begin{solution}
$2^3 7^4$
\end{align*}

\ppart  What is the \emph{least common multiple}, $\lcm(m,n,p)$?\hfill\examrule{1.5in}
\begin{solution}
...
Suppose \iffalse $\emptyset \neq A \subseteq \naturals$, that is, \fi
$A$ is a nonempty set of nonnegative integers.
...
You may assume that $\min(A \union B) = \min(\min(A), \min(B))$.
...
We show that 
\[
\expof{p}{left hand side of~\eqref{gcdAUB}} = \expof{p}{{right hand side of~\eqref{gcdAUB}}
\]
Since $p$ is an arbitrary prime, this implies~\eqref{gcdAUB}.
Now
\begin{align*}
\expof{p}{\gcd(A \union B)}
   & = \min(\expof{p}{A \union B}) & \text{by~\label{gcdexp}}\\
   & = \min(\expof{p}{A} \union  \expof{p}{B}) & \text{by def of $\expof{p}{A \union B}$}\\
   & = \min(\min(\expof{p}{A}), \min(\expof{p}{B})) & \text{assumed}\\
   & = \min(\expof{p}{gcd(A)}, \expof{p}{\gcd(B)}) & \text{by~\label{gcdexp}}\\
   & = \expof{p}{(\gcd(\gcd(A), \gcd(B)))}  & \text{by~\label{gcdexp}}
\end{align*}
\end{solution}
