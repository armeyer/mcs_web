...
\begin{problem}
  ...
  These properties are described in some of the statements below.
  Mark each of these statements as \textbf{T} (true) or \textbf{F}
  (false), and then briefly explain your answer.

\begin{enumerate}

\item The probability that the ninth cookie in the
\emph{batch} is overcooked is $c$.\hfill\examrule
...
\item All cookies in the batch are equally likely to be the
third cookie chosen in the \emph{sample}.\hfill\examrule
...
\item The probability that the ninth cookie chosen for the
  \emph{sample} is overcooked, is $c$.\hfill\examrule
...
\item Given that the first cookie chosen for the \emph{sample} is overcooked,
the probability that the second cookie chosen will also be overcooked is greater
than~$c$. \hfill\examrule
...
\item  Given that the last cookie in the \emph{batch} is overcooked, the
  probability that the next-to-last cookie in the batch will also be
  overcooked is greater than~$c$. \hfill\examrule
...
\item Given that the first two cookies selected in the \emph{sample}
  are the same kind of cookie---they might both be chocolate, or both
  be ginger, cookies,\dots---the probability that the first cookie is
...
\item  The expectation of the indicator variable for the last cookie in
  the \emph{sample} being \text{overcooked} is~$c$.\hfill\examrule
...
\item The statistician's calculation of $n$ does not depend on the size of the batch.
...
cookies in the \emph{sample} are different approaches one. \hfill\examrule
