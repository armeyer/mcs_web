...
\pkeywords{
graphs, state_machines, connectivity
}
...
\problempart\pts{4} Let $G$ be the graph with vertices $\set{1,2,3,4}$ and edges
\[
\set{\set{1,2},\set{3,4}}
\]
...
\problempart\pts{8} \label{derived} On the line next to each of the derived
...
\item[i)] $c$ \hfill \brule{1.0in}

\solution{nonincreasing}

\item[ii)]  $c+e$ \hfill \brule{1.0in}

\solution{nonincreasing}

\item[iii)] $2c+e$ \hfill \brule{1.0in}
...
\problempart\pts{8} Explain why, starting from any state, $G$, the procedure
terminates.  If your explanation depends on answers you gave to part~(b),
you must justify those answers.

\solution{4 pts: The derived variable iii) is a natural-number valued
variable and is decreasing.  \emph{Alternative answer}: derived variable
iv) is decreasing under the lexicographic order on $\nat \cross \nats$
which we know from Problem Set 6 is a well-ordered set.

We saw in Notes \#6 that this means the procedure must terminate.

4pts: To show that the variable iii) decreases, note that the rule for
...
the added edge, for a net decrease of 1.  This also shows that variable
iv) decreases.}

\problempart\pts{10} Prove that any final state must be an \textbf{unordered
tree} on the set of vertices, \ie a spanning tree.

\solution{We use the characterization of a tree as a cycle-free,
