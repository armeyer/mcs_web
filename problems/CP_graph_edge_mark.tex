...
\begin{solution}
  As a state machine, the start state of $Mark$ is some given connected
...
satisfies~(*).  This means that $H$ has no fully marked cycles, and
...
definition of ``eligible.''  So $e$ is not on a fully marked simple
cycle in $H'$.  Since $H$ and $H'$ are otherwise the same, there is no
fully marked simple cycle elsewhere in $H'$.  That is, $H'$
satisfies~(*).
