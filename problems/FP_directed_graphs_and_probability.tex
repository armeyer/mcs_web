...
$\Gdir$:
\begin{align*}
T_1 \eqdef \text{vertices } 2,3,4 \text{are on a length-3 cycle},\\
T_2 \eqdef \text{vertices } 1,3,4 \text{are on a length-3 cycle},\\
T_3 \eqdef \text{vertices } 1,2,4 \text{are on a length-3 cycle},\\
T_4 \eqdef \text{vertices } 1,2,3 \text{are on a length-3 cycle)},\\
\end{align*}
...
\begin{staffnotes}
Something should be added to indicate that DAG iff $\QNOT(T_1 \union T_2
\union T_3 \union T_4)$, which is not instantly obvious
\end{staffnotes}

\ppart What is the probability that $\Gdir$ is a
DAG?  (You may answer with a simple numerical formula.)\\
Hint: $\Gdir$ can only have length-3 cycles. 
