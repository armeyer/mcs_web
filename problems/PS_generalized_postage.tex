...
\begin{pcomments}
  \pcomment{PS_generalized_postage}
...
\begin{problem} \textbf{The Generalized Postage Problem}
  ...
  Several other problems (\bref{PS_3_and_5_postage_by_WOP},
  \bref{TP_10_and_15_cent_stamps_by_WOP},\bref{TP_Postage_by_Induction},
...
\begin{solution}
Each row of the array contains all the integers at are least as large
as and are congruent to modulo $a$ to the first element in that row.
So $n$ appears in the row whose first element is $\leq n$ and is
$\equiv n \pmod a$.
\end{solution}
...
\begin{lemma*} (Generalized$^2$ Postage) If $m$ and $n$ are positive
  ...
  actually make any amount of postage greater than $mn/g-m-n$ which is a
