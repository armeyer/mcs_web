...
$\log_2$ is the inverse of $2^x$ so $\log_2 x \in \EF$.  Therefore so
  ...
  \EF$.\footnote{There's a little problem here: since $\log_2 x$ is
    ...
    $x(f(\abs{x}))^2$, where $\abs{x} = \sqrt{x^2}$.}
\end{solution}
...
\item[Constructor Cases:] ($\inv{f}$).  Assume $f, df/dx \in \EF$ to prove
  $d\, \inv{f}(x)/dx \in \EF$.
Letting $y = f(x)$, so $x=\inv{f}(y)$, we know from Leibniz's rule in
calculus that
