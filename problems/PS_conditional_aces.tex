...
The $\frac{2}{a}$ substitution for \prcond{\text{\ahrt\ is in your hand}}{K=2}$ is justified by considering the fraction of pairs of aces that include the \ahrt.  The number of pairs that \emph{don't} include \ahrt{} is $\binom{a-1}{2}$, while the total number of pairs is $\binom{a}{2}$.  Some algebra shows that the quotient of these two is $\frac{a-2}{a}$, which is the probability that \ahrt{} isn't used.  Subtracting from 1, to find the probability for the complement of this event, gives us $\frac{2}{a}$.
...
The $\frac{2}{h}$ substitution for $\prcond{\text{the revealed card is an Ace}}{K=2}$ is justified by noting that, in any hand with 2 aces (i.e., $K=2$), the revealed card must come from one of exactly 2 positions (with aces) out of $h$ total positions.
