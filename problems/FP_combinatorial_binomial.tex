...
many ways are there to divide $S$ into a pair of subsets?
...
blue, or yellow.  One way to assign colors is to choose red, green,
blue, or yellow successively for each marble.
...
\begin{solution}
\[
\sum_{i=0}^n \binom{n}{i} 2^i 2^{n-i} = 4^n.
\]

  There are 4 choices of color for each successive marble, so there are
  $4^n$ ways to color the marbles.

  In the second coloring process, there are $\binom{n}{i}$ ways to
  choose a set, $S$, of size $i$; by part~\eqref{divide-S}, there are
  $2^i$ ways to select the subset of $S$ be painted red, leaving the
  rest of $S$ green; similarly, there are $2^{n-i}$ ways to select the
  subset of the marbles not in $S$ to be painted blue, leaving the
  rest yellow.  So there are $\binom{n}{i} 2^i 2^{n-i}$ ways to paint
  the marbles with a total of $i$ of them painted red or green, and
  $n-i$ of them painted blue or yellow.  But $i$ can be any number
  from $0$ to $n$. This shows that the total number of ways to paint
  the marbles is given by the above sum. 
\end{solution}
...
\begin{solution}
\[
\sum_{i=0}^n \binom{n}{i} 2^i 2^{n-i} = (2+2)^n = 4^n.
\]
\end{solution}
