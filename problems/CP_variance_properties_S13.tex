...
otherwise.  So $\expect{R_n} = (n+1)/n + (n-1)/n = 2$ as required.
Also,
\[
\expect{R_n^2} = (n+1)^2/n + (n-1)/n  = (n^2+3n)/n = n + 3.
\]
So,
\[
...
\ppart If $R \leq 2$, that is, $R$ takes on only the values 1 and 2, how
...
variable.  If $\pr{R=1} = p$, then the variance of $R$ is $p(1-p)$,
which is maximized when $p=1/2$ (see Problem~\bref{CP_gallup_poll}).  Thus, $\variance{R}$ can be as large as 1/4.
\end{solution}
