...
\texttt{011} occurs starting at the 4th position.  Write a formula for
$r$ in terms of $n$.
...
substring \texttt{011} occupying positions 4--6.
\end{solution}

\ppart Let $A_i$ be the set of length $n$ binary strings in which
\texttt{011} occurs starting at the $i$th position.  (So $A_i$ is
...
\begin{solution}
  ...
  copies of \texttt{011} in a length $n$ binary sequence.  Since the
  copies can't overlap, this is the same as the number of sequences of
  $n-6$ \texttt{0}'s and two \texttt{1} where the \texttt{0}'s
  indicate positions not occupied by the two copies and the
  \texttt{1}'s indicate where the copies are placed.  By the
