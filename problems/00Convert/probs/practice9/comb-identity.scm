...
<em>(r - k)</em> balls from the remaining <em>(n - k)</em> balls.</p>
      ...
      <li>" (math "{n \\choose k} {k \\choose {r-k}} = {n \\choose k}{k
\\choose r}")"

      <p><li>" (math "{n \\choose r}{r \\choose k} = {n \\choose k}{n-k
\\choose r-k}")"

      <p><li>" (math "{n \\choose r}{r \\choose k} = {n \\choose r}{r \\choose r-k}")"

      <p><li>" (math "{n \\choose k}{n \\choose r-k} = {n \\choose r}{n-r \\choose r-k}")"

      <p><li>" (math "{k \\choose n}{n \\choose r-k} = {r \\choose n}{n-r \\choose r-k}")"

      <p><li>" (math "{n \\choose r}{r \\choose k} = {n \\choose k}{n \\choose r-k}")"
      </ol>"
     )
    (boxsize 5)
                                        ;      (compare equal?)
