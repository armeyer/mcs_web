...
(define-problem
  '(
    ...
    (intro "Pascal's Theorem gives us that " (math "{{n-1} \\choose {k}} + {{n-1} \\choose {k-1}} = {{n} \\choose {k}}") ". We will try and apply this result repeatedly to get a cool identity. For each of the parts, your answer should be a single number corresponding to the appropriate expression among the list of choices for that part."
           )
    (boxsize 5)
    ...
    (questions
     ...
     (! "Now to put it all together, which of the expressions below does this sum evaluate to: " (math "{{n} \\choose {n}} + {{n+1} \\choose {n}} + {{n+2} \\choose {n}} + \\ldots +  {{n+k} \\choose {n}}") "
      <p>
      <ol>
      ...
      <p><li>" (math "{{n+k+1} \\choose {n+1}}")"
        ...
        (answer  (4) (compare test-set) (rationale
       "Following the same steps as before, we get " (math "{{n+k+1} \\choose {n+1}}") " is the correct answer. If you would like to be rigorous, try and prove this result using induction on <em>k</em> or come up with an elegant combinatorial argument. Here is a not so elegant combinatorial argument but it is still important to understand: <p> The number of solutions to the equation " (math "x_1 + x_2 + \\cdots + x_k + x_{k+1} = n+1") " is " (math "{{n+k+1} \\choose {n+1}}") " as there is a bijection between solutions to this equation and bit sequenes of length <em>(n+k+1)</em> with exactly <em>(n+1)</em> zeros.
