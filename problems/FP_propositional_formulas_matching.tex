...
that both $S$ and $A-S$ are nonempty, and the set $N(S)$ of neighbors
of $S$ is the same size as $S$, that is, $\abs{N(S)} = \abs{S}$.

\examspace[1.0in]
\begin{solution}
$S \eqdef \set{P \QXOR Q, \QNOT(P \QAND Q)}$.

Now $N(S) = \set{\bar{P} \QOR \bar{Q}, \bar{P} \QOR \bar{Q} \QOR
  (\bar{P} \QAND \bar{Q})}$, so $\abs{S} = \abs{N(S)} = 2$.
\end{solution}
...
There is a matching from $\leftbi{G}$ to $\rightbi{G}$ if only if
