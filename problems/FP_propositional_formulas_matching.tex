...
why $\widehat{R}$ is not a partial order on the set $A \union B$.

\examspace[1.0in]

\begin{solution}
$\widehat{R}$ is not a partial order since the condition of
...
there is both a matching from $L$ to $N(L)$ and also a matching from
$\bar{L}$ to

\[
N(\bar{L}) \qquad \bar{N(L)} \qquad N^{-1}(N(L)) \qquad
N^{-1}(N(\bar{L})) \qquad N(\bar{L}) - \bar{N(L)} \qquad N(L) -
N(\bar{L})
\]
