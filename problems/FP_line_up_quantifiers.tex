...
\begin{pcomments}
  ...
  \pcomment{revised zabel 2/20/18}
\end{pcomments}
...
\begin{problem}
Some (but not necessarily all) students from a large class will be
lined up left to right.  There will be at least two students in the
line.  Translate each of the following assertions into predicate
formulas, using quantifiers $\exists$ and $\forall$ and any logical
operators like $\QAND$, $\QOR$, $\QNOT$, etc., with the set of
...
\item $F(x,y)$, meaning that ``$x$ is somewhere to the left of $y$ in
  ...
  $F(c,d)$ are true. (A student is not considered to be ``to the left
  of'' themself, so $F(x,x)$ is always false.)

\end{itemize}
As a worked example, the predicate ``there are at least two students
after $x$'' may be written as
\begin{equation*}
  \exists y\, \exists z.\, \QNOT(y=z) \QAND (F(x,y) \QAND F(x,z)).
\end{equation*}

In your answers you may use earlier predicates in later parts, even if
you did not solve the earlier parts.  So your answer to
...
\hint Since there are at least $2$ students in the line,
$\text{inline}(x)$ means $x$ is to the left or right of someone else.
