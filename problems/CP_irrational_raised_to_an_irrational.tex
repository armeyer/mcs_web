...
true\footnote{The number $\sqrt{2}^{\sqrt{2}}$, known as the
  Gelfond-Schneider constant, was proven to be a transcendental
  number---which not only implies that it is irrational, but also
  implies that it is not even the root of any polynomial with rational
  coefficients---by Rodion Kuzmin in 1930. In 1934, Aleksandr
  Gelfond~\cite{Gelfond34} proved the more general Gelfond-Schneider
  theorem, which states that if $\alpha$ is a real number not equal to
  zero or one, and $\beta$ is a rational number, then $\alpha^\beta$
  is transcendental.}, but that didn't matter.
\end{solution}
