ein\documentclass[problem]{mcs}

\begin{pcomments}
  \pcomment{MQ_one_committee}
  \pcomment{variation of MQ_committees}
\end{pcomments}
...
Ferdinand, and Georg would all serve together.  Find $|Q|$.
\examspace[1in]
\begin{solution}
...
is unhappy.  Clearly, $D = R\cup S$ and $R\cap S = \emptyset$.  So
apply the Sum Rule: $|D| = |R| + |S|$.  Hence:
\begin{align*}
|R| &= |D| - |S| \\
...
many ways are there to form such a pair of committees, if employee
happiness is \textbf{not} taken into consideration?

\examspace[2in]
\begin{solution}
There are $\displaystyle\binom{20}{6}$ ways to choose employees for
the first committee.  For each of these, there are $20-6=1$4 people
left over and so $\displaystyle\binom{14}{6}$ ways to assign employees
to the second committee.  By the Generalized Product Rule (remember,
the two committes do different things), the number of ways to form two
six-person committes is
\[\binom{20}{6}\binom{14}{6}.\]
Alternatively, notice that we are just counting the
$\paren{6,6,8}$-splits of the set of all employees.  So another way to
write the answer is
\[\binom{20}{6,6,8}.\]
\end{solution}
