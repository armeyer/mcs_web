...
the grammatical rules for programs.  The declaration defines a
procedure $P_s$, which we can think of as the result of compiling $s$
into an executable object.  If $s \in \asciistr$ is not a
grammatically well-formed procedure declaration, we arbitrarily define
$P_s$ to be a procedure that fails to halt when applied to any string.
Now every string defines a string procedure, and every string
procedure is $P_s$ for some $s \in \asciistr$.
