...
relation, $R$, on the set, $A$, is a weak partial order or a path-total order
...
\begin{center}
  \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} | c | c | c | c | c | c |}
    \hline
    $\mathbf{A}$ & $\mathbf{a\mrel{R}b}$ & \textbf{weak partial order} & \textbf{path-total order} & \textbf{minimal(s)} & \textbf{maximal(s)} \\ \hline
    $\mathbb{R} - \mathbb{R}^+$ & $a \divides b$ & & & & \\ \hline
...
\ppart What is the longest \emph{\idx{chain}} on the subset relation,
$\subseteq$, on $P(\set{1, 2, 3})$?  (If there is more than one, provide
ONE of them.)
...
\ppart What is the longest \emph{\idx{antichain}} on the subset relation,
$\subseteq$, on $P(\set{1, 2, 3})$?  (If there is more than one, provide
\emph{one} of them.)
