...
\item With probability $4/7$ the exam is graded by a \emph{TA},with
  probability $2/7$ it is graded by a \emph{lecturer}, and with
  probability $1/7$, it is accidentally dropped behind the radiator
  and arbitrarily given a score of 84.

\item TAs score an exam by scoring each problem individually and
then taking the sum.

\begin{itemize}
\item There are ten true/false questions worth 2 points each.  For each,
full credit is given with probability $n3/4$, and no credit is
...
\item Lecturers score an exam by rolling a fair die twice,
multiplying the results, and then adding a ``general impression''score.

\begin{itemize}
\item With probability $4/10$, the general impression score is 40.
\item With probability $3/10$, the general impression score is 50.
\item With probability $3/10$, the general impression score is 60.
\end{itemize}
\end{itemize}
...
& =  \paren{\frac{7}{2}}^2 + \left(  \frac{4}{10} \cdot 40 +
