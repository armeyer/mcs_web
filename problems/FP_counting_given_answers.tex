...
\newcommand{\apart}[1]{\inhandout{\vspace{0.2in}}
    \ppart\ \ \parbox[t]{4.5in}{#1}\hfill\examrule[0.6in]}
...
1.& n^m & 2. & m^n & 3. & (n-m)! & 4. & \dbinom{n+m}{m}\\
\\
5. & \dbinom{n-1+m}{m} & 6. & \dbinom{n-1+m}{n} & 7. & 2^{mn} & 8. & \dfrac{n!}{(n-m)!}
\end{array}\]
...
inequality $x_1 + x_2 + \cdots + x_n \leq m$?}
...
$x_1 + x_2 + \cdots + x_n + y = m$, and which has a bijection to sequences
...
possible flavors, which is the number of sequences with $m$ stars and
$n - 1$ bars.
\end{solution}
