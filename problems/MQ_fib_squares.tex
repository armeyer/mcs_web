\documentclass[problem]{mcs}

\begin{pcomments}
  \pcomment{MQ_fibonacci_by_induction}
  \pcomment{taken from book/recursive_data.tex staff notes.}
\end{pcomments}

\pkeywords{
  induction
  fibonacci
  recurrence
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Problem starts here
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{problem} 

The $n$th Fibonacci number, $F_n$, is defined recursively as follows:
\[ F_n = \left\{ \begin{array}{ll}
         0 & \mbox{if $n = 0$}\\
         1 & \mbox{if $n = 1$}\\
		 F_{n-1} + F_{n-2} & \mbox{if $n \geq 2$}\end{array} \right. \] 
These numbers satisfy many unexpected identities, such as
\begin{equation}\label{f02f12fnfn1}
F_0^2 + F_1^2 + \cdots + F_n^2 = F_n F_{n+1}
\end{equation}
Equation~\eqref{f02f12fnfn1} can be proved to hold for all $n \in
\naturals$ by induction, using the equation itself as the induction
hypothesis, $P(n)$.

\bparts

\ppart  Prove the \textbf{base case} $\left(n=0\right)$.

	\begin{solution}
	\[ \sum_{i=0}^0 F_i^2 = (F_0)^2 = 0 = \left(0\right)\left(1\right) = F_0 F_1 \]
	Therefore, $P(0)$ is true.
	\end{solution}

\examspace[2in]

\ppart Now prove the \textbf{inductive step}.

	\begin{solution}
	We need to prove that $P(n)$:
	\[ \sum_{i=0}^{n} F_i^2 = F_n F_{n+1} \]
	implies $P(n+1)$:
	\[ \sum_{i=0}^{n+1} F_i^2 = F_{n+1} F_{n+2} \]
	 
	\begin{proof}
	\begin{align*}
	\sum_{i=0}^{n+1} F_i^2 & = \sum_{i=0}^n F_i^2 + F_{n+1}^2 &\\
		                    & = F_n F_{n+1} + F_{n+1}^2 & \mbox{By $P(n)$.}\\
							& = F_{n+1}\paren{F_n+F_{n+1}} &\\
							& = F_{n+1}F_{n+2} & \mbox{By the definition of the Fibonacci sequence.}
	\end{align*}
	\end{proof}

	\end{solution}

\eparts

\end{problem}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Problem ends here
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\endinput
