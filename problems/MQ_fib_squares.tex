...
\begin{proposition}
  $\forall n \geq 0 (\Sigma_{i=0}^n F_i^2 = F_n F_{n+1})$.
	...
	\]
	Let's try a proof by induction.  The theorem
  ...
  \ppart Now state $P(n+1)$, and then prove it assuming $P(n)$, thereby proving the induction step.
	...
	$P(n+1) = (\Sigma{i=0}^{n+1}F_i^2 = F_{n+1}F_{n+2}$) \\
	\begin{proof}
	$\Sigma{i=0}^{n+1}F_i^2 = \Sigma_{i=0}^nF_i^2 + F_{n+1}^2$ \\
	$ = F_nF_{n+1} + F_{n+1}^2$ by $P(n)$ \\
	$ = F_{n+1}(F_n+F_{n+1}) = F_{n+1}F_{n+2}$ by def. of Fibonacci seq.
	\end{proof}
