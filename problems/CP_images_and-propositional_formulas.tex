...
\begin{pcomments}
  \pcomment{CP_images_and-propositional_formulas}
  \pcomment{from: Megumi Ando, 9/19/09, reworked by ARM}
  \pcomment{probably subsumed by CP_implies_relation_on_propositonal_formulas}
\end{pcomments}
...
\[(P \QAND  Q) \QIMPLIES  P,  Q,  (R  \QOR  \bar{R})\]

\[(P \QOR  Q) \QIMPLIES (R  \QOR  \bar{R})\]

\[\QNOT (P  \QAND  Q) \QIMPLIES (\bar{P} \QOR \bar{Q}), (R  \QOR  \bar{R})\]

\[(P \QXOR  Q) -> (\bar{P} \QOR \bar{Q}), (R  \QOR  \bar{R})\]

 \end{solution}

 \ppart[3]\label{Iprops} \inbook{Indicate}\inhandout{Circle} the properties below possessed by the
 relation $I$:
  \[
  \begin{array}{ccccc}
  \mbox{FUNCTION~~} & 
...
\end{solution}  
  ...
  \ppart[3]  \inbook{Indicate}\inhandout{Circle} the properties below possessed by the relation $\inv{I}$:
  \[
  \begin{array}{ccccc}
  \mbox{function~~} & 
  \mbox{~~total~~} &
