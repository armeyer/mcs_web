...
and~\eqref{x-yx>y}.  You may assume without proof elementary
...
\inductioncase{Case}~\eqref{xyex=y}: ($x=y$).  In this case, $(x',y',e') = (1,0,ex)$.
Now we have
\begin{align*}
\gcd(a,b) 
    & = e\gcd(x,x) 
...
and $y$ must be odd, and so this state satisfies~\eqref{xybev} since $y$ is odd.

Suppose the last rule gets executed resulting in state $(x-y,y,e)$.  Then if $y$ is odd this
...
Now if the first rule is not executed in some state $(x,y,e)$, then~\eqref{xybev} must hold,
