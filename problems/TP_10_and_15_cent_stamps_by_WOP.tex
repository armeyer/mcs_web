...
\begin{pcomments}
  ...
  \pcomment{variant of CP_6_and_15_cent_stamps, MQ_10_and_15_cent_stamps_by_WOP}
...
\begin{problem}
  ...
  In particular, let the notation ``$j \divides k$'' indicate that
  integer $j$ is a divisor of integer $k$, and let $S(n)$ mean that
  exactly $n$ cents postage can be assembled using only 10 and 15 cent
  stamps.  Then the proof shows that
%
\begin{equation}\label{Sn5n}
...
\begin{quote}
Let $C$ be the set of \emph{counterexamples} to~\eqref{Sn5n}, namely
\[
C \eqdef \set{n \suchthat \dots}
\]

\begin{solution}
 $n$ is a counterexample to~\eqref{Sn5n} if $n$ cents postage can be
...
which proves that~\eqref{Sn5n} holds.
