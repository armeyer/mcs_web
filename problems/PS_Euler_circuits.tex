...
\ppart\label{cycle-circuit} Show that if $\walkv{w}$ is a closed walk, then
it must be an Euler tour.

\hint part~\eqref{conn}

\begin{solution}
Suppose an edge was not traversed by $\walkv{w}$.  By part~\eqref{conn},
...
\ppart\label{already} Explain why all the edges incident to the end of $\walkv{w}$
...
\ppart\label{odd} Show that if the end of $\walkv{w}$ was not equal to the start
