...
\begin{pcomments}
  \pcomment{FP_random_sampling_fall11}
  \pcomment{from: F11.final, F09 tutor problem practice13/sampling_conceptual}
...
\ppart Our theorems about \idx{sampling} and distributions allow us
...
facts about voters and the way they are chosen.  Circle the true facts
among the following:
...
the following, circle the legitimate things you might say in a call to
the President:
...
\begin{solution}
  ...
  $\abs{0.53 - p} \leq 0.04$ or $\abs{0.53 - p} > 0.04$.  Now read it
  as follows: Either $p$ is within 0.04 of 0.53 or it is not and
  therefore my random variable $P$ took a value from a set that is hit
  only 5 times in 100.  So, clearly, either $p$ is within 0.04 of 0.53
