...
\begin{pcomments}
  \pcomment{CP_98_heads_and_4_tails}
...
\begin{solution}
  The set of states is $\nngint \times \nngint = \{(h,t)\mid h,t\in\nngint\}$, where $(h,t)$ represents the state with $h$ heads and $t$ tails.
  The start state is $(98,4)$,
  and the transitions are:
\[
(h,t) \to
\begin{cases}
(h-a+(10-a),t+a-(10-a)) &\mbox{if } 0\le a\le 10 \QAND a \le h \QAND (10-a) \le t\\
(h,t+h+1).
\end{cases}
\]
...
\ppart\label{derived-variables}
\newcommand\parity{\operatorname{parity}}
Define the following derived variables:\footnote{The function $\parity:\integers\to\{0,1\}$ is defined as follows: $\parity(n) = 0$ when $n$ is even, and $\parity(n) = 1$ when $n$ is odd.}
...
\ppart Prove using the Invariant principle that it is not possible to reach a state in which there is
exactly one head showing. If you wish to use a fact from part~\ref{derived-variables}, please prove it carefully here.
...
\end{enumerate}
Since the initial number 98 of heads is even, that is, $H_2((98,4))=0$,
the Invariant Principle now implies that the number of heads in a reachable
