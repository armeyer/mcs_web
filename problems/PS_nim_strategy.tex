...
pile.  Players alternate moves, and the loser is the one who is left
with no stones to remove.

It turns out there is a winning strategy for one of the players that
is easy to carry out but is not so obvious.

\iffalse So if you can as long as you're that first player, it's
likely going to be a while before your opponent figures out why you
keep winning.  \fi
...
\begin{solution}
 Notice that the largest pile may not be the one that is bigger than
 the Nim sum of the others; three piles of sizes 2,2,1 is an example.

 Now suppose the Nim sum of all the piles is $s$, and the high order digit
...
\ppart (Extra credit) Nim is sometimes played in a \emph{mis\`ere}
version in which the person who takes the last stone \emph{loses}.
Adapt the strategy above to a winning strategy for \emph{mis\`ere}
style.
