...
\begin{problem}
  ...
  in Sheboygan involves 3 posts and $n$ rings of different sizes.
  Initially, all the rings are on post \#1:
...
of moves.  A move consists of removing the top ring from one post and
dropping it onto another post with the restriction that a larger ring
can never lie above a smaller ring.  Furthermore, a local ordinance
requires that \emph{a ring can be moved only from a post to the next
  post on its right---or from post \#3 to post \#1.}  Thus, for
example, moving a ring directly from post \#1 to post \#3 is not
permitted.
...
\begin{staffnotes}
\textbf{Extra part if all other problems are done (unlikely).}

A better (indeed optimal, but we won't prove this) procedure to
...
\item Let $t_n$ be the number of moves needed to solve the Sheboygan puzzle
...
\item Derive values $a,b,c, \alpha, \beta$ such that
\[
