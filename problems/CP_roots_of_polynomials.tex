...
$a_0+a_1x+a_2x^2+\cdots + a_{n-1}x^{m-1} + x^m$ be
...
\ppart Explain why Lemma~\ref{poly} immediately implies that
$\sqrt[m]{k}$ is irrational whenever $k$ is not an $m$th power of some integer.

\begin{solution}
Saying that an integer, $k$, is not the $m$th power of an
