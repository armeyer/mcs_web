...
\begin{pcomments}
  ...
  \pcomment{from: F07.ps2; F13.cp7f}
\end{pcomments}
...
Now let $A$ be the \emph{set} of numbers in $S$.  (So $A$ is the
integers $\Zintvcc{1}{9}$ for the example above.)  There are two
straightforward linear orders for $A$.  The first is numerical order
where $A$ is ordered by the $<$ relation.  The second is to order the
...
\begin{solution}
The partial order $\prec$ is the positive path relation of the digraph
shown in Figure~\ref{precpathpic}.  The maximal elements are 8 and 9;
the minimal are 1, 4, and 6.
\begin{figure}[h]
\graphic[height=4in]{sequence-poset}
\caption{Path representation of $\prec$}
\label{precpathpic}
\end{figure}
