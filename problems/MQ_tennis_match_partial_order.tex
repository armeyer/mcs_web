...
partial order.  following partial order concepts correspond to the
properties~\eqref{unbeat}--\eqref{noloseself} of the partial order
$L^+$?
...
\bparts
\ppart\label{unbeat} An unbeaten player is a \brule{1in} element.
\begin{solution}
maximal
\end{solution}
...
\ppart A set of players whose rankings relative to each other are unique is \brule{1in}.
\begin{solution}
a chain
\end{solution}
...
\ppart\label{noloseself} The fact that no player loses to himself correspond to $L^+$ being
\brule{1in}.  
\begin{solution}
irreflexive
\end{solution}
