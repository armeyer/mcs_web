...
\begin{pcomments}
  \pcomment{TP_mean_time_variance_given}
...
is the number of hours until the first crash, we know 
\begin{align*}
\expect{H} &= \frac{1}{p},\inbook{ & \text{(Equation~(\bref{exp_time_to_fail}))}}\\
\variance{H} & = \frac{q}{p^2}\inbook{ & \text{(Equation~(\bref{var_time_to_fail}))}},
\end{align*}
where $q \eqdef 1-p$.
