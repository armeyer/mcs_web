...
To explain this bijection, 
let $(b_1, b_2, \ldots, b_n)$ be a permutation of students scores such that
$b_1 < b_2 < \cdots < b_n$, that is, the list of scores sorted in increasing order.
Let $g_1, \dots, g_n$ be grades assigned to these scores.
For any $(x_1, \dots, x_k) \in A$, the corresponding assignment of grades
is $g_i = 1$ for $i  \in \Zintv{1}{x_1}$, $g_i = 2$ for $i \in Zintv{x_1+1}{x_1+x_2}$,
\dots,  $g_i = k$ for $i \in \Zintv{\sum_{j=1}^{k-1}x_j)+1}{\sum_{j=1}^{k}x_j}$.
In other words, the lowest $x_1$ scores are assigned grade $1$, the next
$x_2$ scores are assigned grade $2$, and so on.
This assignment is unique since $g_1 \leq g_2 \leq \dots \leq g_n$ must
...
length $n+k-1$ with $k-1$ zeros, $\card{A} = \binom{n+k-1}{k-1}$, which
