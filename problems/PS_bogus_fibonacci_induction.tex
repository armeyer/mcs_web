...
\begin{pcomments}
   ...
   \pcomment{Adapted from TP_bogus_well_ordering_principle_proof by
     ARM 2/20/11}
...
\begin{problem}
A recursive definition of the Fibonacci numbers was given
Example~\bref{fib-def}.  Namely, letting $F(n)$ be the $n$th Fibonacci
number, we have
\begin{align}
F(0) & \eqdef 0,  \label{fibdef0PSbog}\\
F(1) & \eqdef 1,\label{fibdef1}\\
F(n+1) & \eqdef F(n) + F(n-1) &  \text{for } n > 0.\label{fibdefeqn}
\end{align}
