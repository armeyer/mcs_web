...
%$1, 3, 5, 7, \ldots$, i..e.,
$a_n = 2 \cdot 3^n+1, n \geq 0$.

\examspace[1.3in]

\begin{solution}
Using that the generating function for $3^n$ is $\frac{1}{1-3x}$ and for $1$ is $\frac{1}{1-x}$,
the generating function for $a_n$ is
\[
A(x) = \frac{2}{1-3x} + \frac{1}{1-x} = \frac{3-5x}{(1-3x)(1-x)} \, .
\]
\end{solution}
...
%$0, 1, 3, 5, \ldots$, i..e.,
\[b_n = \begin{cases}
           2 \cdot 3^{n-1}+1 & \text{for } n\geq 1,\\
           0       &  \text{for } n = 0.
       \end{cases}
\]
...
\begin{solution}
\[
B(x) = x A(x) = \frac{x(3-5x)}{(1-3x)(1-x)} \, ,
\]
...
\[c_n = \begin{cases}
           c_{n-1} + 2 \cdot 3^{n-1}+1 & \text{for } n\geq 1,\\
           0                           &  \text{for } n = 0.
       \end{cases}
\]
...
- B(x)     & = &        & - &  (2 \cdot 3^0+1) x & -  &  (2 \cdot 3^1+1) x^2 & - &   (2 \cdot 3^2+1) x^3 & - & \cdots\\
\hline
...
so
\[
C(x) = \frac{B(x)}{1-x} = \frac{x A(x)}{1-x} = \frac{x(3-5x)}{(1-3x)(1-x)^2} \ .
\]
\end{solution}
