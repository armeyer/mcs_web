...
class.  Parts (f) and (h) are the trickiest parts, usually where
...
\ppart The relation between any two nonnegative integers $a$ and $b$ such that 
$a \equiv b \pmod{8}$.
...
\ppart The relation between propositional formulas $G$ and $H$ such that $[G
\QIMPLIES H]$ is valid.
...
\ppart The relation `beats' on Rock, Paper, and Scissors (for those who don't
...
\begin{solution}
  ...
  order.  It's not linear.  It's not an equivalence relation because
  it is not reflexive.
\end{solution}
