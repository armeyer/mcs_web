...
\ident{A}$, and $C^{n+1} \eqdef R \compose C^n
$.\inhandout{\footnote{$\ident{A}$ is the equality relation on $A$
\[
a \mrel{\ident{A}} b \qiff a =b.
\]

For binary relations $R: M\to B$ and $S: A \to M$, the
    \idx{composition} of $R$ with $S$ is the binary relation $(R
    \compose S): A\to B$ defined by the rule
\[
a \mrel{(R \compose S)} b \eqdef\ \exists m \in M.\ (a \mrel{S} m)
\QAND (m \mrel{R} b).
\]}}
Regarding $R$ as a digraph, let $R^n$ denote the length-$n$ walk
relation in the digraph $R$, that is,
\[
