...
\begin{problem}
Use the Well Ordering Principle to prove by that the gcd of a
finite set of integers is an integer linear combination of the numbers
in the set.  You may assume the case for a set of two elements $a,b$
...
C &\eqdef \{n \geq 1 \suchthat\\
  & \qquad \exists A \subset \integers.\, \card{A} = n \QAND \QNOT(\gcd(A)\text{ is a linear
  combination of } a \in A,\}
\end{align*}
and assume for the sake of contradiction that $C$ is not empty.

By the WOP, there is a least integer $m \in C$.  So there must be
integers $ a_1, a_2, a_{m}$ such that $\gcd(a_1,a_2,\dots, a_m)$ is
...
\gcd(a_1,a_2,\dots,a_{m})
  & = \gcd(\gcd(a_1,a_2,\dots,a_{m-1}), a_{m})
       & \text{(given above)}\\
  & = s \gcd(a_1,a_2,\dots,a_{m-1}) + t a_{m}\text{ for some }s,t \in \integers
