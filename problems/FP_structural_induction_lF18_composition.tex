...
e \eqdef f \compose g^{(-1)} \in \LEF by ind
e^{(-1)} \in \LEF by constructor
= g \compose f^{(-1)} = g \compose h \in \LEF



\begin{align}
f^{(-1)} \compose g
   & = \paren{g^{(-1)} \compose \paren{f^{(-1)}}^{(-1)}}^{(-1)}
          & \text{(by the hint)}\notag\\
   & = \paren{g^{(-1)} \compose  f}^{(-1)}.\label{gcomf-1}
\end{align}
Now since $f, g \in \LEF$ by induction hypothesis, then
so is $g^{(-1)}$ by constructor rule~\eqref{L:inversefunc}.  Now by
induction hypothesis (with ``$g^{(-1)}$'' substituted for ``$g$''), we
have
\[
e \eqdef g^{(-1)} \compose f \in \LEF.    %%NO NO f on wrong side
 \]
 But $e^{(-1)} \in \LEF$ by~\eqref{L:inversefunc}, so finally
by~\eqref{gcomf-1},
\[
h \compose g = e^{(-1)} \in \LEF.
\]
\fi
