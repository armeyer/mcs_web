...
dollars. Using the Well Ordering Principle (WOP), prove the following 

\begin{quote}
\textbf{Property}: For any $m$,
given a dollar amount $0 \leq n < 2^m$, there exists a subset of
envelopes whose contents add up to {\em exactly} $n$ dollars. 
\end{quote}
...
We say that a positive integer $m$ is \emph{bad} if there are a set of $m$
...
contents add up to $n$ dollars, for any $n < 2^k$. In other words, we
have shown that $k$ is \emph{good}. Therefore, by
