\documentclass[problem]{mcs}

\begin{pcomments}
  \pcomment{FP_modular_arithmetic}
  \pcomment{from: Steven F09}
  \pcomment{Adapted from F05 Quiz 2 problem 2}
\end{pcomments}

\pkeywords{
  pulverizer
  eulers
  modular_arithmetic
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Problem starts here
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{problem}

\bparts 
\ppart
Using the Pulverizer, find an $x$ in the range $[0\dots 99]$ such 
that $x$ is an inverse of 19 modulo 100.
\vspace{3.5in}

\begin{solution}
There are two valid approaches.
\textbf{Solution 1:} Use the Pulverizer to find integers $s,t$ such that
$s19+t100 =1$.  Then $s \rem 100$ will be $19^{-1}$.
In this case the Pulverizer yields $-21\cdot 19 + 4 \cdot 100 =1$, so
$19^{-1} \equiv -21 \equiv 79 \pmod {100}$.
\textbf{Solution 2:} Find $k =\phi(100)$, so by Euler's Theorem, $19^{-1}
\equiv 19^{k-1} \pmod{100}$.  Then $19^{k-1} \rem 100$ will be $19^{-1}$.
This can be computed by at most $\floor{\log 100} = 6$ squarings modulo
100.
\end{solution}

\ppart[5] What is the value of $\phi(360)$, where $\phi$ is Euler's
function? \hfill\brule{4em}
\vspace{1.5in}

\begin{solution}
Note that $360=2^3\cdot3^2\cdot5$.  It follows that
$\phi(360)=4\cdot6\cdot4=96$.
\end{solution}

\ppart[5] What is the value of $7^{98} \rem 360$? \hfill\brule{4em}

\begin{solution}
Since 7 and 360 are relatively prime, we have by Euler's Theorem
that $7^{96} \equiv 1 \pmod {360}$, and so
\[
7^{98} = 7^{96}\cdot 7^2 \equiv 1 \cdot 49 \equiv 49 \pmod {360}.
\]
\end{solution}

\eparts
\end{problem}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Problem ends here
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%