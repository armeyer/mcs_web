...
\begin{pcomments}
  \pcomment{PS_no_odd_length_cycles}
  \pcomment{from: S09.ps6, S07.ps5, S06.ps4 S04.quiz1, S04.cp5w}
\end{pcomments}
...
from $v$ to $y$ followed by edge $e$ and vertex $x$.  If this path
does not include $e$, then it can be extended to a path to $y$ by
adding edge $e$ and vertex $y$.  In either case, the paths to these
vertices from $v$ differ by a single traversal of $e$, and so the
lengths of the paths differ by 1; in particular, one has odd length
and the other has even length, so $x$ and $y$ are differently colored.
\end{solution}
