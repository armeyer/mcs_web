...
\ppart Let $n$ and $a$ be positive integers. Then, $n$ and $a$ are relatively
prime then $a^{\phi(n)} \equiv 1 \pmod{n}$.
\inhandout{\hfill \textbf{T} \qquad \textbf{F}}
\begin{solution}
\textbf{False.} Euler's Theorem states that if $n$ and $a$ are
relatively prime, then $a^{\phi(n)} \equiv 1 \pmod{n}$. 
\end{solution}
...
\begin{solution}
 \textbf{True}. 
An eavesdropper could use the given algorithm as follows: read $(n, d)$ from the
 public key, calculate the prime  factors $p$ and $q$ of $n$, and
 compute the inverse of $d$ modulo $(p-1)(q-1)$ to get an $e$, which
 can decrypt any message transmitted by the sender.
\end{solution}
