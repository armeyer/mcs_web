\documentclass[problem]{mcs}

\begin{pcomments}
  \pcomment{FP_counting_4_digit_numbers_with_sum_of_digits_22}
  \pcomment{ZDz 11/16/15}
  \pcomment{f15.mid4}
\end{pcomments}

\pkeywords{
  counting
  bijection
  binomial
  sum_of_numbers
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Problem starts here
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{problem}

Suppose $x_1,x_2,x_3,x_4$ are nonnegative integers.  You may answer
the following questions using closed formulas possibly involving
binomial coefficients without calculating a final numerical answer.

\bparts

\ppart  How many solutions are there to the equation $x_1+x_2+x_3+x_4=22$?
\examspace[1in]

\begin{solution}
$\binom{25}{3}$.
The same as the number of ways to choose 3 out of 25 positions in a sequence
that separate 22 remaining positions into four groups.
\end{solution}

\ppart How many solutions are there such that $x_i > 9$ holds? Assume $i$ is in $\set{1,2,3,4}$.
%such that exactly one of the $x_i$'s is greater than nine?
\examspace[1in]

\begin{solution}
$\binom{15}{3}$.
The same as the number of solutions to the equation $x_1+x_2+x_3+x_4=12$
without constraints.
%\[
%\binom{4}{1} \cdot \binom{15}{3}
%\]
\end{solution}

\ppart How many solutions are there such that $x_i > 9 \QAND x_j > 9$ holds?
Assume $i$ and $j$ are in $\set{1,2,3,4}$ and $i \neq j$.
%How many solutions such that exactly two of the $x_i$'s are greater than nine?
\examspace[1in]

\begin{solution}
$\binom{5}{3}$.
The same as the number of solutions to the equation $x_1+x_2+x_3+x_4=2$
without constraints.
%\[
%\binom{4}{2} \cdot \binom{5}{3}
%\]
\end{solution}

\ppart How many 4-digit numbers with sum of digits equal to 22 are
there?  Explain your answer.
You may express your answer in terms of numbers $a,b,c$ which
are the answers to the previous parts of this problem.
 \examspace[3in]

\begin{solution}
\[
a - \binom{4}{1} b + \binom{4}{2} c
\]
The same as the number of solutions to the equation $x_1+x_2+x_3+x_4=22$
such that $x_1, x_2, x_3, x_4$ are smaller than or equal to 9.
Let $S$ be the set of these solutions. Let $A$ be the set of solutions
without constraints and $D$ the set of solutions in which at least one
number is greater than 9. Then, $S = A \setminus D$ and $|S| = |A| - |D|$.

To obtain $|D|$, we use the Inclusion-Exclusion principle.
Let $B_i$ be the set of solutions in which $x_i > 9$ holds.
%Note that $|B| = b$.
Let $C_{i,j}$ be the set of solutions in which both $x_i > 9$ and $x_j > 9$ hold.
%Note that $|C_{i,j}| = c$.
Note also that there are no solutions in which three or more variable are grater than 9.
Then,
\[
D = \union_{i} B_i \, ,
\]
and
\[
|D| = \sum_{i} |B_i| - \sum_{i \neq j} |C_{i,j}| = \binom{4}{1} b - \binom{4}{2} c \, .
\]
Finally,
\[
|S| = |A| - |D| = a - \binom{4}{1} b + \binom{4}{2} c \, .
\]
\end{solution}

\eparts

\iffalse

How many 4-digit numbers with sum of digits equal to 22 are there?

\hint{Inclusion-Exclusion.}

\begin{solution}
There is 

Let $x_1$, $x_2$, $x_3$ and $x_4$ be the digits of a 4-digit number $a$. Let
\begin{align*}
A & = \set{(x_1, x_2, x_3, x_4) \in \integers^4 \suchthat 0 \leq x_1, x_2, x_3, x_4 \leq 9 \QAND x_1+x_2+x_3+x_4 = 22} \\
S & = \set{(x_1, x_2, x_3, x_4) \in \integers^4 \suchthat x_1, x_2, x_3, x_4 \geq 0 \QAND x_1+x_2+x_3+x_4 = 22} \\
S_i & = \set{(x_1, x_2, x_3, x_4) \in \integers^4 \suchthat x_i \geq 10 \QAND x_j \geq 0, j \neq i \QAND x_1+x_2+x_3+x_4 = 22} \\
S_{i,j} & = \set{(x_1, x_2, x_3, x_4) \in \integers^4 \suchthat x_i, x_j \geq 10 \QAND x_k \geq 0, k \neq i,j \QAND x_1+x_2+x_3+x_4 = 22} \, .
\end{align*}
Then, $A = S \setminus (\union_i S_i)$ and
\begin{align*}
|A| & = |S| - \sum_{i=1}^4 |S_i| + \sum_{1\leq i < j \leq 4} |S_{i,j}| \\
     & = {22+4-1 \choose 4-1} - {4 \choose 1} {12+4-1 \choose 4-1} + {4 \choose 2} {2+4-1 \choose 4-1} \\
     & = {25 \choose 3} - 4 {15 \choose 3} + 6 {5 \choose 3} \\
     & = 2300 - 4 \cdot 455 + 6 \cdot 10 = 540 \, .
\end{align*}
{\color{red} TODO: Textual explanation would be better.}
\end{solution}
\fi

\end{problem}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Problem ends here
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\endinput
