...
\begin{solution}
\begin{staffnotes}
\TBA{Very wordy, consider reducing it by writing down some of the
  arguments used multiple times as lemmas}
\end{staffnotes}

Since $yy = yy$ then we must have that $\lnth{u} \le \lnth{y}$.
Therefore, since the string $uy$ starts with $u$ then the string $yu$
must also start with $u$, which implies that $y$ starts with $u$.

Denote by $s^t$ a string that is composed of the concatenation of $t$
times the string $s$.  Suppose that $y$ does not consist of
repetitions of the string $u$.  Let
\[
y = u^kp,
\]
where $k \ge 1$ and $p$ is a string that does not start with $u$.  Then
$uy = yu$ reduces to $up = pu$.  Notice that if $\lnth{p} \ge
\lnth{u}$ then, by the same argument as above, we would have that $p$
starts with $u$, which would contradict our assumption.  Hence,
$\lnth{p} < \lnth{u}$.

Now notice that starting from the string $u^kp$, we can sequentially
replace the string $up$ with $pu$ and obtain
\[
u^kp = pu^k.
\]
Concatenating $p$ on the right of both sides of this
equality gives
\[
yp = (u^kp)p = (pu^k)p = p(u^kp) = py.
\]
Since $u$ is the shortest positive length string such that $uy=yu$, it
follows that $\lnth{p}$ must be zero.  That is, $p = \emptystring$ and
so $y = u^k \in u^*$.

Since $xy = yx$ then we must have $\lnth{x} \ge \lnth{u}$.  Our
original equation can now be written as $xu^k = u^kx$ which implies
that $x$ starts with $u$. Suppose that $x$ does not consist of
repetitions of the string $u$, that is, $x = u^rq$, with $r \ge 1$ and
$q$ is a string that does not start with $u$.  Then our equation
reduces to $qu^k = u^kq$.  Since $q$ does not start with $u$, we must
have that $\lnth{q} < \lnth{u}$.  However, since $u^k = y$ the last
equation is equivalent to $qy = yq$, contradicting the minimality of
$u$.  Hence, $x$ also consists of repetitions of the string $u$.
\end{solution}
