...
Argue that for the state machine model described above, the parity of
a state is preserved by transitions.

\begin{solution}
This part follows directly from the argument in the next part showing
that $t$ is weakly decreasing, namely, $t$ changes by 0 or $-2$, so
...
into $b,c,a$.  Now $a,b,c \movesto b,c,a$ removes one out-of-order
pair $a,b$.  Moreover, if $a < c$, this move introduces the
out-of-order pair $c,a$ for a net change of zero in $t(A)$.  If $c <
...
\problempart Starting with $S \eqdef (2014, 2013, 2012,\ldots, 2 ,1)$ there is
only one sequence at which the procedure could terminate, that is,
from which no further transition is possible.  Describe this sequence
(no explanation is required).

\begin{solution}
$(1, 2, \dots, 2012, 2014, 2013)$.

Suppose $n=2013$. The reason is that the smallest element $k<n-1$ that is not in the
...
2014*2013/2$ is odd, while $t((1, 2, \dots, n-2, n-1, n)) = 0$ and $t((1, 2, \dots,
