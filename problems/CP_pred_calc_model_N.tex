...
\forall x.\, \prevsym(x) = x \QIMPLIES x = \numerl{0}.
\end{equation}
\end{solution}
\eparts
...
$\ltsym$ and make it act like a less-than relation.  For this
purpose, it's suggestive to write $x\ltsym y$ instead of $\textbf{less}(x,y)$.
Then one requirement will be that for all $n,m \in \naturals$, if $n <
m$, then $\numerl{n}\ltsym \numerl{m}$ must be true.  But we also want
$\ltsym$ to define a relation that has the ``less-than'' properties on all
