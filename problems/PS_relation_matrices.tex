...
$\QAND$.  Namely, suppose $R: B \to C$ is a binary relation with $C =
...
\lefteqn{[M_S \otimes M_R](i,j) =1}\\
 & \quad\QIFF\quad \text{\large OR}_{k=1}^m [M_S(i,k) \QAND M_R(k,j)] =1
     & \text{(by~\eqref{MSboolMR})}\\
& \quad\QIFF\quad [M_S(i,k) \QAND M_R(k,j)]=1 \text{ for some } k,
    1\leq k\leq m & \text{(def. of $\QOR$)}\\
& \quad\QIFF\quad [M_S(i,k) =1] \QAND [M_R(k,j)]=1] \text{ for some }
  k, 1\leq k\leq m & \text{(def. of $\QAND$)}\\
& \quad\QIFF\quad i \mrel{S} k\ \QAND\ k \mrel{R} j \text{ for some }
  k, 1\leq k\leq m & \text{(def. of $M_R, M_S$)}\\
& \quad\QIFF\quad i (R \compose S) j & \text{(def. of $R \compose
   S$)}.
\end{align*}
