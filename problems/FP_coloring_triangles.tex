...
What is $\expect{I_T}$? \brule{0.5in}
What is $\variance{I_T}$?  \brule{0.5in}
...
\ppart Let $T$ and $T'$ be any two different triangles, and suppose
that $r = 1/2$.  Show that the random variables $I_T$ and $I_{T'}$ are
independent.

\examspace[3.5in]
\begin{solution}
 Monochronicity of a triangle is obviously
independent of the edge colors of any nonoverlapping triangle, so we only
consider the case when $T$ and $T'$ share an edge.

So assume $T$ and $T'$ share an edge.  It is enough to show that
\begin{eqnarray*}
\pr{I_T = 1 \cap I_{T'} = 1} & = & \pr{I_T = 1} \cdot \pr{I_{T'} = 1}.
\end{eqnarray*}
