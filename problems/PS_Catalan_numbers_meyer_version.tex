...
\ppart The \term{enclosure} of a string, $s$, is the string
$\mtt{(}s\mtt{)}$ that starts with a left parenthesis followed by the
characters of $s$, and then ends with a right parenthesis.  Explain why
the generating function for the enclosures of strings with a good count is
$xC(x)$.

\hint The enclosure mapping that takes a string, $s$, to its enclosure,
$\mtt{(}s\mtt{)}$, is a bijection.

\begin{solution}
  The mapping which takes a string $s$ to its enclosure $\mtt{(}s\mtt{)}$
  is obviously a bijection from the set of strings to the set of enclosed
  strings.  But if $s$ has a good count, then so does $\mtt{(}s\mtt{)}$,
  so this mapping also defines a bijection from strings with good counts
  that have exactly $n$ left parentheses to their enclosures, which are
  strings with good counts that have exactly $n+1$ left parentheses.  So
  letting $e_{n+1}$ be the number strings with $n+1$ left parentheses that
  are enclosures of strings with a good count, we have $e_{n+1} = c_n$.
  Therefore,
\begin{align*}
xC(x) & = 0+ c_0x +c_1x^2 +\cdots+ c_nx^{n+1} +\cdots\\
      & = e_0 + e_1x +e_2x^2 +\cdots+ e_{n+1}x^{n+1} +\cdots,
\end{align*}
which shows that $xC(x)$ is the generating function for the enclosures of
...
that are enclosures of strings with good counts and $s = s_1\cdots s_k$.
aFor example, the string $r \eqdef \mtt{(())()(()())} \in \GC$ equals
$s_1s_2s_3$ where $s_1= \mtt{(())}, s_2 = \mtt{()}, s_3 = \mtt{(()())}$,
and this is the only way to express $r$ as a sequence of enclosed strings
with good counts.

\begin{solution}
  Having a good count by definition means the left-to-right parenthesis
  count starts and ends with 0 and never goes negative.  This implies that
  the enclosure of a string with good count start and ends with 0 and
  remains positive everywhere else.  So let $s_i$ be the substring that
  starts where the count is 0 for the $i$th time and ends when it is 0 for
  the $i+1$st time.
\end{solution}
...
\begin{solution}
By~\eqref{Cpmsqrt}, $2xC(x) = $1 \pm \sqrt{1 -4x}$, so
