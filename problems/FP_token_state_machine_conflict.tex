...
into state a $(n'_b,n'_w)$ equal to $(n_b - 1, n_w + 2)$ or to $(n_b +
2, n_w - 1)$.  But since
\[
...
The bases cases will be all the state such that $n_b$ and $n_w$ are
both $\leq 1$.

Verify the \inductioncase{base case}:

\examspace[2.0in]

\begin{solution}
There are four states with
both $n_b \leq 1$ and $n_w \leq 1 2$, namely
\[
(0,0), (0,1), (1,0), (1,1)
\]
Of these, only the start state $n_b=1,n_w=0$ satisfies $T(n_b,n_w)$.
But $(1,0)$ is the start state, and so is reachable.


Now complete the \inductioncase{Induction Step}:
