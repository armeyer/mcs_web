...
\begin{pcomments}
  \pcomment{CP_system_component_failure_with_independence}
  \pcomment{same as CP_system_component_failure with extra part on independence}
\end{pcomments}
...
\ppart Describe probabilities for the outcomes so that the component
failures are mutually independent.  Verify that $\pr{F_i} = p$.  What
is $\pr{F}$?
\begin{solution}
For $s \in \sspace$,
\[
\pr{s} \eqdef p^{\card{s}}(1-p)^{n-\card{s}}.
\]

\TBA{lots of binomial sums}
\end{solution}
\eparts
\end{problem}
