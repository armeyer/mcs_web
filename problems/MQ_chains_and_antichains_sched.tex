...
\begin{solution}
One largest chain is $\set{1,6,7,11,14}$. Greedily computing vertex depths show that all of the nodes $1,2,3,4,5,8,9,10,12,13$ have depth $1$, while nodes $6,7,11,14$ have depths $2,3,4,5$ respectively, so the largest chain has size $5$.
\end{solution}

\examspace[0.4in]

\ppart What is the largest antichain? (Again, give only one if you find
there are more than one). Prove there isn't a larger antichain.

\begin{solution}
One largest antichain is $\set{1,2,3,4,5,8,9,10,12,13}$. To see that there isn't a larger one, observe that only one node can be chosen from the chain $\{1,6,7,11,14\}$, so any antichain must exclude at least four vertices.
\end{solution}
...
\begin{solution}
With unlimited processors, we can take 5 seconds.  This is the length of the longest chain.
\end{solution}
...
With 5 processors, we can still finish everyting in 5 seconds. A
  ...
  $\set{9,10,11,12,13}$, $\set{14}$.  We cannot do this with fewer
  than 5 processors because in order to make progress on the longest
  chain at every time step, we need to process all of $\set{1,2,3,4,5}$ in step
  1.
\end{solution}
