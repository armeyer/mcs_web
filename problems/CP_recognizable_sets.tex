...
\ppart Describe how a recognizer for the set of strings that are the
arabic (decimal) numeral for an odd number would work.  (Even better,
actually write a recognizer procedure in your favorite programming
language).
...
\begin{solution}
  ...
  but it wouldn't be hard to write such a procedure and then to use it as
  above.
...
actually the typed description of some string procedure, let's refer to that
procedure as $P_s$.
...
\begin{solution}
  ...
  programs are commonly intended to be analyzable in order to confirm that
...
\begin{itemize}
\item are exaggerating the power (if any) of their methods to make a sale,
  or

  \item are trying to keep things simple by not going into the technical
