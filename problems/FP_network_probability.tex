...
\ppart\label{I1I12I3} Let $I_0$, $I_1$, and $I_2$ be indicator variables for the
events that a packet originating at $in_0$, $in_1$, and $in_2$,
respectively, gets routed through the dashed edge in the figure.
Explain why $T=I_0+I_1+I_2$.

\begin{solution}
  The only routes through the dashed edge are ones that originate at
  input terminals 0--2, so the total number of packets routed through
  the dashed edge will be the same as the total number of packets
  starting from these terminals, namely, $I_0+I_1+I_2$.
\end{solution}
...
their corresponding input.)
  \begin{itemize}
    \item What is the probability that the packet at input 2 has one of output terminals
          3--8 as its destination? \hfill\examrule
...
\begin{solution}
\begin{align*}
 \pr{\text{packet 2 is destined for output 3--8}} & = 2/3\\
 \expect{T} & = 2\\
 \variance{T} & = 2/3.
\end{align*}
...
\[
\pr{\text{packet 2 is destined for output 3--8}} = 6/9.
\]
But packet 2 gets routed through the dashed edge iff it is destined
for one of the outputs 3--8, so $\pr{I_2=1} = 2/3$ and hence
$\expect{I_2}=2/3$ and $\variance{I_2} = 2/3(1- 2/3) = 2/9$.  The same
reasoning applies to $I_0$ and $I_1$.  By linearity of expectation,
\[
...
\begin{solution}
  ...
  $I_0=1$, then there are only five remaining destinations from 3--8,
  so $\prcond{I_1 = 1}{I_0=1} = 5/9 \neq \pr{I_1=1}$.  So linearity of
  variance is not guaranteed, and the reasoning of
  part~\eqref{edgepacketprob} no longer applies.
\end{solution}
