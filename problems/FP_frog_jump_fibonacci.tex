...
steps.  We're interested in counting the number of ways that Kermite
can climb a staircase of any given size.  For example, if the
staircase has three stairs, then there are three ways that Kermite
could climb to the top: 3 hops; a hop and a skip; a skip and a hop.

Let $w_n$ to be the number ways Kermit can climb a size $n$ staircase.
So we've just observed that $w_3 = 3$.
...
Kermit can climb size $n \geq 3$ staircases in terms of the number of
...
sequence.  However the bases cases here are different.
