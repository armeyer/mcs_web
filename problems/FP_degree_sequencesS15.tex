...
\begin{pcomments}
  \pcomment{FP_degree_sequencesS15}
  \pcomment{variation of FP_degree_sequences}
  \pcomment{CH Spring '14; edited ARM 4/1/14, 5/19/15}
\end{pcomments}
...
graph, then its degree sequence is (1,1,2).  On the other hand,
(0,0,2) is not a degree sequence, since in any graph with an edge,
there are at least two vertices of positive degree, namely, the
...
\begin{solution}
This can only be two disjoint edges, so the graph is not connected.
\end{solution}
