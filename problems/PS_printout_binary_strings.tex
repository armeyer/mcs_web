...
\begin{pcomments}
  ...
  \pcomment{from: S17.cp2f, S09.ps1}
...
starting with zero, then row number three has binary representation
\STR{011}, which corresponds to the labelling $(\false\ \true\
\true)$.  So we generate the $k$th row of the array for $n$ variables by
finding the length-$n$ binary representation of $k$ and converting
\STR{0}'s back to $\false$ and \STR{1}'s back to $\true$.
