...
\begin{pcomments}
  \pcomment{PS_infinite_ordinals_full}
  \pcomment{extends PS_infinite_ordinals}
  \pcomment{draft ARM 3/13/18}
\end{pcomments}
...
\begin{definition*}
A set $x$ is \emph{closed under membership} if every element of $x$ is also a
subset of $x$, that is
\[
...
closed under membership.  We need to prove that $\nextset{x}$ is closed under
membership.  To do this, suppose $y \in \nextset{x}$.  We need to show $y
\subset \nextset{x}$.  Since $x \subset \nextset{x}$ by~\eqref{insubset}, it is
enough to show that $y \subseteq x$.
...
\ppart Prove the converse: every set that is closed under membership is an
ordinal.
