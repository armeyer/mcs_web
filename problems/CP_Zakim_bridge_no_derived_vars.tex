...
\begin{pcomments}
  \pcomment{CP_Zakim_bridge_no_derived_vars}
  \pcomment{revised from derivef var version 2/18/11 by ARM}
...
the Zakim bridge.  Their consulting architect has warned that the
...
\problempart Let $D_0 \eqdef 2A_0 - 3B_0 - 6C_0$ and define
\[
...
\begin{solution}
To verify that $P$ is preserved, suppose state $(A,B,C)$ has a
transition to $(A',B',C')$, and $P(A,B,C)$ is true.  We verify that
$P(A',B',C')$ is true by considering the two kinds of transitions.
...
\[
6C' = 6(C - 1) = 6C - 6= 2A-3B-D0 - 6= 2A - 3(B+2) - D0 = 2A' - 3B' - D0.
\]
In addition, $C' < C \leq 1000$ so $C' \leq 1000$.
\end{solution}

\ppart  Conclude that the traffic won't cause the bridge to collapse.

\begin{solution}
Note that $P$ is true of the start state $(A_0,B_0,C_0)$ by definition
of $D_0$ and the fact that $C_0 \leq 1000$.  Also, $P$ won't be true in any
collapsed state because $C >1000$ in a collapsed state.
\end{solution}
