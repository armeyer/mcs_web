...
\begin{pcomments}
  \pcomment{CP_ant_on_grid}
  \pcomment{from: F01.ps6; spring00.ps4-5; S97.handout 28.ps9soln}
\end{pcomments}
...
``everything else''---for example, information about where things are
on the grid.  Work out the details of such a model state machine;
design the ant-memory part of the state machine so the ant will eat
all the crumbs on \emph{any} finite path at which it starts and then
signal when it is done.  Be sure to clearly describe the possible
states, transitions, and inputs and outputs (if any) in your model.
Briefly explain why your ant will eat all the crumbs.
...
However, we requested that the ant-machine stops and signals \emph{
...
the ant can signal \emph{done}.  Here is the state machine:
\begin{align*}
    AntMemory       &= \{0,1,2,3,4\} \\
    BoardLocation   &= \naturals \times \naturals \\
...
The transitions correspond to the three actions $\set{\text{``go straight''},
\text{``turn left''}, \text{``done''}}$.  We will use variables $q=(m,l,d,C)$ to
