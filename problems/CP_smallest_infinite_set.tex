...
AN issue that puzzles some students (when they ar challenged
about it) is why the third clause in the definition of $f$ is needed
since $f$ is already defined on all the $a_n$'s.  The answer is that
there may be elements left over in $A$, and to be a bijection, the
value of $f$ on each ``left-over'' element of $A$ has to be defined
somehow.  In fact, if $A$ is uncountable, there are guaranteed to be
such left-over elements.
...
follow directly from definition of $f$, so it shouldn't be much burden
