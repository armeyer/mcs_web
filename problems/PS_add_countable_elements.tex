...
First, $f$ is \textbf{total}, since every $n \in \mathbb N$ can be written as either $2i$ or $2i+1$ for an appropriate $i$, and the last clause covers those $a$ values that don't match some $a_i$.

Second, $f$ is \textbf{1-to-1}, since clearly all the righthand sides of $f$'s defining equations are distinct: we assumed that $B$ does not overlap $A$, as well as that the sequences of $a_i$ and $b_i$ contain no duplicates.  Also, by definition, the last clause fires only for an $a$ that matches no $a_i$.
