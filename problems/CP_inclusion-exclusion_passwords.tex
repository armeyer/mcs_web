...
\begin{problem}
A certain company wants to have security for their computer systems.  So
they have given everyone a name and password.  A length 10 word containing
each of the characters:
\begin{quote}
a, d, e, f, i, l, o, p, r, s,
\end{quote}
is called a \emph{cword}.  A password will be a cword which does not
contain any of the subwords "fails", "failed", or "drop".

For example, the following two words are passwords: \qquad adefiloprs, srpolifeda,

but the following three cwords are not: \qquad a\textbf{drop}eflis, \textbf{failedrop}s, \textbf{drop}e\textbf{fails}.

\bparts

\ppart How many cwords contain the subword ``drop''?
...
\ppart How many cwords contain both ``drop'' and ``fails''?
