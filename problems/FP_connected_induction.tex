...
of computers in the cluster.  \inbook{IS\&T wants to minimize the
  number of hubs used to build this cluster.}
...
for the minimum number of wires needed to connect all the computers
and hubs to each other so that there is a path along the wires from
each computer to all the hubs and other computers.
...
\ppart\label{ISTMmax} Suppose IS\&T uses $m$ hubs where each hub is
\emph{connected to at most five wires}.  Write a simple formula in
terms of $m$ and $n$ for the maximum number of wires that can be
attached to the hubs and computers without leaving dangling ends.
...
\ppart Compare the previous two quantities to prove that at
...
\frac{n-2}{3} & = m.
\end{align*}
ut $m$ must be an integer, so we get
\[
m = \ceil{\frac{n-2}{3}}.
\]
\end{solution}
...
Prove by induction that $\ceil{(n-2)/3}$ hubs are sufficient to
