...
\begin{pcomments}
  ...
  \pcomment{from F13.ps8, F07.rec7t, S06.cp5f, S04.cp5w}
...
\begin{solution}
First proof by citing the text:

\begin{proof}

According to the Gray Edge Lemma~\bref{lem:necessary-gray}, a minimum
weight edge among the gray edges of a solid coloring must be an edge
of \emph{every} MST.  So use any black/white coloring in which the
endpoints of $e$ are different colors; this make $e$ a gray edge.
Since $e$'s weight is smaller than all other weights, it must be a
min-weight gray edge in this solid coloring.

(Technically the Gray Edge Lemma only applies to graphs with no
same-weight edges, but as remarked in the text, we could apply it to
graphs with same-weight edges by assigning an arbitrary rank to
same-weight edges, and then treat higher rank vertices as having
larger weight among the edges with the same weight.

Since $e$ has min-weight with no ties, it will be in every MST for
each such ranking, and therefore in all MST's with same-weights
allowed.

\end{proof}

A Second proof mimics the proof of the Gray Edge Swap
Lemma~\bref{lem;gray-swap}:
