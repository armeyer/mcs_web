...
\begin{pcomments}
  ...
  \pcomment{names edited F11, S12, S13}
\end{pcomments}
...
\newcommand{\Eric}{\text{Giuliano}}
\newcommand{\Tom}{\text{Uddhav}}
\newcommand{\Albert}{\text{Albert}}
\newcommand{\Oshani}{\text{Sharon}}
\newcommand{\Edmond}{\text{Sebastian}}
\newcommand{\Florent}{\text{Nirvan}}
\newcommand{\Nick}{\text{Eka}}
...
$\exists x, y, z. \,
    (x \neq y \QAND
     x \neq z \QAND
     y \neq z \QAND
     \cabal(x) \QAND \cabal(y) \QAND \cabal(z))$

\begin{solution}
  A direct English paraphrase would be ``There exist people we'll call
  $x,y$, and $z$, who are all different, such that $x,y$ and $z$ are each
  in the cabal.''  A better version would use the fact that there's no
  need in this case to give names to the people.  Namely, a better
  paraphrase is ``There are at least 3 different people in the cabal.''
  Perhaps a simpler way to say this is: ``The cabal is of size at least
  3.''
\end{solution}

\ppart\label{nNC} $\QNOT(\cabal(\Nick) \QAND \cabal(\Oshani))$
...
\ppart\label{Fall} $\cabal(\Florent) \QIMPLIES \forall x.\, \cabal(x)$

\begin{solution}
If \Florent\ is in the cabal, then everyone is.
\end{solution}

\ppart\label{CN} $\cabal(\Oshani) \QIMPLIES \cabal(\Nick)$
...
\ppart\label{EAnT}
$(\cabal(\Edmond) \QOR \cabal(\Albert)) \QIMPLIES \QNOT(\cabal(\Tom))$

\begin{solution}
  If either of \Edmond\ or \Albert\ is in the cabal, then \Tom\ is not.
  Equivalently, if \Tom\ \emph{is} in the cabal, the neither \Albert\ nor
  \Edmond\ is.
\end{solution}

\ppart\label{ENnE}
$(\cabal(\Edmond) \QOR \cabal(\Nick)) \QIMPLIES \QNOT(\cabal(\Eric))$
...
\ppart  Now use these facts to explain exactly who is on the cabal and why.

\begin{staffnotes}
  If a team is stuck, tell them that the cabal consists of exactly \Nick,
  \Edmond, and \Albert\ and have them check that this set satisfies all
  the conditions.  (See the end of the solution.)  Then start them back on
  proving that this is the unique set that works.
\end{staffnotes}

\begin{solution}
So much for the translations.  We now argue that the only
...
or \Oshani---who is not in the cabal.  But if \Florent\ were in the cabal, then
...
Now suppose \Tom\ is in the cabal.  Then by~\eqref{EAnT}, \Edmond\ and
\Albert\ are not, and we already know \Florent\ and \Oshani\ are not, so
only three remain who could be in the cabal, namely, \Tom, \Nick, and
\Eric.  But by~\eqref{eee} the cabal must have at least three members, so
it follows that the cabal must consist of exactly these three.  This
proves:
\begin{lemma}[TNE]
\text{If \Tom\ is in the cabal, then \Nick\ and \Eric\ are in the cabal.}
\end{lemma}

But by~\eqref{ENnE}, if \Nick\ is in the cabal, then \Eric\ is not.  That is, 
\begin{lemma}[NnE]
\text{\Nick\ and \Eric\ cannot both be in the cabal.}
\end{lemma}
Now from Lemma~(NnE) we conclude that the conclusion of
Lemma~(TNE) is false.  So by contrapositive, the hypothesis of
Lemma~(TNE) must also be false, namely,
...
the cabal must consist of at most two people (\Albert\ and \Eric). This
...
\ppart $\card{A} = 3$, so $A$ satisfies~\eqref{eee}.
\ppart \Oshani\ is not in $A$, so $A$ satisfies~\eqref{nNC} and~\eqref{CN}.
\ppart \Florent\ is not in $A$, so the hypothesis of~\eqref{Fall} is false, which
means that $A$ satisfies~\eqref{Fall}.
\ppart Finally, \Tom\ and \Eric\ are not in $A$, so the conclusions of
...
\begin{proposition*}
$\set{\Albert, \Edmond, \Nick}$ is the \emph{unique} cabal
