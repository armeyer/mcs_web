...
\compose R$ is a bijection.  \iffalse It is easy to see that $R$ must
be total ($[\geq 1\ \text{out}]$) and $S$ must be a surjection ($[\ge
  1\ \text{in}]$).\fi

\bparts

\ppart Give an example of such $R,S$ with $\card{A} = 2$ where neither
$R$ nor $S$ is a bijection.  Indicate exactly which
properties---total, surjection, function ($[\leq 1\ \text{out}]$), and
injection ($[\leq 1\ \text{in}]$) your example of $R$ and your example
of $S$ fail to have.
...
out.  Let the fourth element of $B$ have two $R$-arrows in and no $S$-arrows out.

This example shows  that neither $R$ nor $S$ have to have any additional
``jection'' properties.  That is,
\begin{itemize}
