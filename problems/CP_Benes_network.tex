...
through each switch.  Let's work through an example.  \inhandout{A
  Bene\v{s} network of size $N = 8$ is attached.}  Within the
Bene\v{s} network \inbook{ of size $N = 8$ shown in Figure~\ref{fig:B_3}}, the
two subnetworks of size $N = 4$ are marked.  We'll refer to these as
the \emph{upper} and \emph{lower} subnetworks.
...
lower subnetwork.  Construct a graph with vertices 0, 1, \dots, 7 and
