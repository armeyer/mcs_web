...
\begin{problem} A simple graph is \emph{sevenish} when it has no simple
cycles of length less than seven.  A \term{map} is a simple graph all
of whose faces are simple cycles (no dongles or bridges).  Let $M$ be
...
\hint Similar to the proof that $e \leq 3v-6$.  Use
Problem~\bref{planar-structural}.

\begin{solution}

\begin{proof}
  Since the faces are simple cycles, the length of each face is at least 7.
...
the fact that subgraphs of $M$ may not be a sevenish, connected, maps.
...
NEEDS REVISION:
  ...
  {\bf Base cases:} ($v\leq 3$).  A graph with at most 3 vertices is
  trivially be 3-colorable.

  {\bf Inductive step:} By part~\eqref{sevenishd2}, $G$ has a vertex,
  ...
  removing an edge may remove some simple cycles, but won't create new
  ones, so $H$ is still sevenish.  BUT MAY NOT BE A MAP!!

  So by strong induction, $H$ is
  3-colorable.  Now to 3-color $G$, 3-color each of the connected
  components, $H$, obtained when $u$ was removed.  Except for $u$,
  adjacent vertices in $G$ are also adjacent in some connected
  component, $H$, and will therefore be assigned different colors.
  Finally, to complete the 3-coloring of $G$, assigning to $u$ a color
  that differs from those of the at most two vertices adjacent to $u$.
\end{proof}\fi
