...
an \emph{assignment} or a \emph{use}.  In particular, an appearance is
...
variable is on the right side of an equation or on the ``Outputs''
line.  The \term{lifetime of a variable} is the segment of code
extending from the initial assignment of the variable until the last
use.\footnote{This definition is for the case that each variable is
  assigned at most once (see part~\eqref{assignedmore}).}  There is an
edge between two variables if their lifetimes overlap.  This rule
