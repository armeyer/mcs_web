...
\begin{problem} \mbox{}
  ...
  \textbf{\large For each of the statements below in
    parts~\eqref{gcdtf} through~\eqref{simgrpart}, write ``V'' for
    Valid or ``N'' for Not Valid, and \emph{provide counterexamples}
    for those that are not valid.}
...
\begin{solution}
N, $a=2\cdot 3, b=3\cdot 5, c=5\cdot 7$
\end{solution}
...
\begin{itemize}
\item  $G$ has a spanning tree.\fill\examrule

\examspace[0.4in]

\begin{solution}
N.  Any disconnected graph is a counterexample.
\end{solution}

\item $\card{\vertices{G}} = O(\card{\edges{G}})$ for connected $G$.\fill\examrule
...
\item $\card{\edges{G}} = O(\card{\vertices{G}})$.\fill\examrule

\examspace[0.4in]

\begin{solution}
N.  $\card{\vertices{K_n}} = n = o(n^2)$, but
...
that are true of \textbf{stable distributions on random walk graphs}
