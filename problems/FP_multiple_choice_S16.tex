...
%%   \False} \examspace[0.4in]
%% \begin{solution}
%% \False.  Choose a subgraph with 2 vertices and no edges.
%% \end{solution}
...
%% \begin{solution}
%% \False.  This got ``edges'' and ``vertices'' reversed.  Every tree is
%% a counterexample.
%% \end{solution}
...
\item $\card{\vertices{G}} = O(\card{\edges{G}})$ for connected $G$.  \instatements{\hfill \True \qquad
  \False} \examspace[0.4in]
\begin{solution}
\True.  To be connected, there must be at least $\card{\vertices{G}}
-1$ edges.
\end{solution}
...
%%   chromatic number of $G$. \instatements{\hfill \True \qquad
%%     \False} \examspace[0.4in]
%% \begin{solution}
%% \False.   $\chi(K_n) = n$, but $\card{\edges{K_n}} = \Theta(n^2)$.
%% \end{solution}
