...
\ppart\label{indeppairscircle} \inbook{Indicate which of} \inhandout{\textbf{Circle}} the
event pairs below that are independent.  In each case, briefly explain
your answer.
\begin{enumerate}[i.]

\item\label{ExyW} $E(x,y)$ versus $W(x,y)$ for $x \neq y$.
...
\item\label{Wwxyz} $W(w,x)$ versus $W(y,z)$ for distinct vertices $w$, $x$, $y$ and $z$.
