...
positive even-length closed walk, but \emph{no} vertex is on a
positive even-length cycle.
...
\caption{A positive even-length  $v$ to $v$ walk, but no positive even-length $v$ to $v$ cycle.}
\label{fig:walkpath}
\end{figure}
...
\]
There is no positive even-length cycle from $v$ to $v$ from~$u$ to~$u$
