...
%  \newcommand{\beats}{\!\rightarrow\!}
  ...
  player, but may very well have lost their games against must lower
...
to~$u$ that goes through $v$.  \textcolor{red}{\emph{A common mistake
    ...
    it may not be (see Problem~\ref{PS_shortest_closed_walk})}} Since
...
Lemma~\ref{shortestclosedwalk_lem} is to observe that in a tournament
...
\ppart Prove that the the less-than relation, $>$, on the
rational numbers, $\rationals$ is a DAG and tournament graph
that has no ranking.
...
\begin{solution}
  The $>$-relation defines a tournament graph on $\rational$ since for
