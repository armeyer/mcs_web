...
\ppart Write the numbers of the predicates below that are
preserved invariants:\examrule[3in]
\begin{align}
...
n_b+n_w    & > 5
\end{align}
%\item $2 n_b - n \equiv 1 \pmod{3}$ 
%\item $n_b + n \equiv 2 \pmod{3}$
...
\ppart Suppose the game starts with one black token and no white tokens.
Write the numbers of the predicates above that are true for all reachable
states.  \examrule[3in]
...
\begin{solution}
 The first, third, and fifth predicates are false in the start state and
 so are not true of all reachable states.  The second and fourth
 predicates are true in the start state and are also preserved invariants,
 so by the invariant principle, they true for all reachable states.
\end{solution}
