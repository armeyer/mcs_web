...
\begin{pcomments}
  \pcomment{CP_gen_fcn_quotient_polynomials}
...
then
\[
F'(x) \eqdef f_1 + 2 f_2 x + 3 f_3 x + \cdots.
\]
For example,
\[
...
\]
is the generating function for the sequence of nonegative integers.
Therefore
\[
...
Letting $F(x)$ be the generating function with $[x^n]F(x) = f(n)$, it
follows that
\[
F(x) -axF(x) -bx^2F(x)-cx^3 F(x) - Q(x) = f(0)-Q(0) + (f(1)-af(0)-Q(1))x 
+ (f(2) - af(1) -bf(0) - Q(2))x^2,
\]
allowing a solution for $F(x)$ as a quotient of polynomials.  As noted in
the Appendix, this implies that there is a closed form for $f(n)$.  In
fact, $f(n)$ equals the sum of a finite number of terms of the form $d
\gamma^n n^k$ for constants $d,\gamma \in \complexes$ and $k \in
\naturals$.\end{solution}
