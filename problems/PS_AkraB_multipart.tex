...
\begin{enumerate}
\item $T(n) = 3T\left(\floor{n\over 3}\right) + n$.
\item $T(n) = 4T\left(\floor{n\over 3}\right) + n^2$.
\item $T(n) = 3T\left(\floor{n\over 4}\right) + n$.
\item $T(n)=T\left(\floor{n\over 4}\right)+T\left(\floor{n\over 3}\right)+n$.
\item $T(n)=T\left(\ceil{n\over 4}\right)+T\left(\floor{{3n}\over 4}\right)+n$.
...
$a_1 = 4$, $b_1 = 1/3$, $h_1(n) = \lfloor n/3 \rfloor - n/3$, $g(n) =
  n^2$, $p = \log_3 4$,
%
$$T(n) = \Theta(n^{\log_3 4}(1+ \int_1^n \frac{u^2}{u^{\log_3 4 + 1}}
  du)) = \Theta(n^{\log_3 4}(1+ \int_1^n u^{1 - \log_3 4} du)) =
  \Theta(n^2).$$

\item
$a_1 = 3$, $b_1 = 1/4$, $h_1(n) = \lfloor n/4 \rfloor - n/4$, $g(n) =
...
\item
$a_1 = 1$, $a_2 = 1$, $b_1 = 1/4$, $b_2 = 1/3$, $h_1(n) = \lfloor n/4
  \rfloor - n/4$, $h_2(n) = \lfloor n/3 \rfloor - n/3$, $g(n) = n$,
  $p = 0.5605$ (the exact value does not matter, it turns out that for
  this case, we only need to know that $p < 1$),
%
...
it is certainly $h(m) = O({ m \over (\log m)^2})$. 
