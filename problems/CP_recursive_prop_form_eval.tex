...
The truth value of a propositional formula $F$ \emph{depends only on
  the truth values of $\pvrbls{F}$} iff

\begin{itemize}
\item $V_1$ and $V_2$ are sets of variables,
\item $e_1$ and $e_2$ are environments for $V_1$ and $V_2$, respectively,
\item $V_1 \intersect V_2 \supseteq \pvrbls{F}$, and
\item $\forall P \in \pvrbls{F}.\, e_1(P) = e_2(P)$,
\end{itemize}
\[
...
To prove that the value of a $F$ depends only on the values of $\pvrbls{F}$, suppose
...
\ppart Now we can formally define $F$ being \emph{valid}.  Namely, $F$ is valid iff
\[
