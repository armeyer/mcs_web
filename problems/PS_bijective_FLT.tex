...
factor of $p$.  \emph{Since $p$ is prime,} $k$ must be $1$
or $p$.  But \emph{since the string consists of at least two
...
\ppart Conclude that $p \divides (a^p-a)$ and from this conclude
Fermat's Little Theorem.
...
\begin{equation}\label{apeamp}
a^p \equiv a\pmod{p}.
\end{equation}
If $p$ does not divide $a$, then $a$ can be cancelled from both sides
of~\eqref{apeamp} to yield the Fermat Theorem.
\end{solution}
