...
\begin{pcomments}
  \pcomment{CP_tournament_chain_induction}
  \pcomment{part of CP_tournament_chain}
  \pcomment{usable before DAGs}
\end{pcomments}
...
\begin{problem} %\textbf{Tournament Graphs}
  ...
  ranking, each player won the game against the next ranked player,
  but may very well have lost their games against players ranked
  later---whoever does the ranking may have a lot of room to play
  favorites.
...
to~$u$ that goes through $v$.  \textcolor{red}{\emph{A common mistake
    ...
    it may not be (see
    Problem~\bref{CP_shortest_directed_closed_walk}).}} Since $v \neq
...
where the proof depends on finiteness.
\end{staffnotes}
