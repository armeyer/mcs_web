...
Define $B \eqdef S \cross S \cross R_3$, where $R_3$ is the set of
size 3 subsets of $R$.  Then define a bijection from $A$ to $B$ by
mapping a roll in $A$ to the sequence in $B$ whose first element is
the number that came up three times, whose second element is the
number that came up four times, and whose third element is the set of
colors of the three matching dice.
