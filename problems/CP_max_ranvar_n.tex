\documentclass[problem]{mcs}

\begin{pcomments}
  \pcomment{MQ_max_ranvar_n}
  \pcomment{subsumes CP_max_ranvar}
  \pcomment{ARM 5/5/12}
\end{pcomments}

\pkeywords{
  random_variables
  mutually_independent
  density
  uniform
  maximum
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Problem starts here
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{problem}
Let $k$ be in the integer interval~$[1,n]$, and let $R_1,R_2,\dots,
R_m$, be mutually independent random variables with uniform
distribution on~$[1,n]$.
\begin{staffnotes}
That is,
\[
\pr{R_i=v} = \frac{1}{n}  \qquad\text{ for } v \in [1,n].
\]
\end{staffnotes}
Let $M \eqdef \max\set{R_i \suchthat i \in [1,m]\, }$.

\bparts

\ppart Write a formula for $\pdf_M(1)$.
\begin{center}
\exambox{0.5in}{0.5in}{0in}
\end{center}
\examspace[0.2in]

\begin{solution}

\[
\paren{\frac{1}{n}}^m .
\]
\end{solution}

\ppart Write a formula for $\pdf_M(k)$ in terms of $\pr{M \leq j}$ for
some suitable $j\text{'s}$ that are $\leq k$.

\begin{center}
\exambox{2.0in}{0.5in}{0in}
\end{center}
\examspace[0.2in]

\begin{solution}
\[
\pr{M \leq k} - \pr{M \leq k-1}.
\]

This follows because, by the Disjoint Sum Rule,
\[
\pr{M \leq k} = \pr{M=k} + \pr{M \leq k-1}\ .
\]

\end{solution}

\ppart
Write a formula for $\pr{M \leq k}$.

\begin{center}
\exambox{0.5in}{0.5in}{0in}
\end{center}
\examspace[0.2in]

\begin{solution}
\[
\paren{\frac{k}{n}}^m .
\]

This follows $M \leq k$ iff $\forall i \in [1,n].\ R_i \leq k$.  Since
$R_i$ is uniform, $\pr{R_i} \leq k = k/n$, so by mutual independence,
\[
\pr{M \leq k} = \prod_{i=1}^m\pr{R_i \leq k} = \paren{\frac{k}{n}}^m .
\]
\end{solution}

\eparts

\end{problem}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Problem ends here
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\endinput
