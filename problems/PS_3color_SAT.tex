...
propositional connectiive can be expressed in terms of $\QOR$ and
$\QNOT$.  The only issue is that an output may need to fan out, but
this is easy to accomplish using length two ``double negation'' paths.
Namely, to fan out the truth value of an output vertex, $v$, to two
new output vertices $v_1$, $v_2$, attach new disjoint length-two paths
from $v$ to $v_1$ and $v$ to $v_2$, with all vertices on the paths
constrained to be $T,F$-valued.
\end{solution}
...
3-color-\textit{f}-gate and the $F$-colored vertex, so the output
vertex is constrained to be $T$ is any 3-coloring of the
...
vertex color $T$, which is possible iff $E$ is satisfiable.  Moreover,
the whole construction of $G$ can essentially be done in time
proportional to the size of $E$, so an efficient procedure to test $G$
for 3-colorability would yield an efficient procedure to test $E$ for
satisfiability.
