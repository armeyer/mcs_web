...
\ppart Use part~\eqref{kteamn} to conclude \emph{Pascal's Triangle
  Identity}:
\begin{equation}
...
\hint Equation~\eqref{eq:pascalid}.

\begin{solution}
\begin{proof}

Let the induction hypothesis $P(n)$ be the statement
\eqref{eq:hockeystick}.

\inductioncase{Base case}: ($n=2$).  The left hand side of
\eqref{eq:hockeystick} is $\binom{2}{2} = 1$ and the right hand is
$\binom{3}{3} = 1$.
