...
\ppart In fact, prove that it requires at most $2\ceil{\log_2 b}$
...
More precisely, the derived variable, $\ceil{\log_2 z}$ is
$\naturals$-valued and decreases by 1 at every transition from any state
with $\ceil{\log_2 z} > 1$.  So there are at most $\ceil{\log_2 b}-1$
decreases until $\ceil{\log_2 z} \leq 1$.  But that means $z \leq 2$,
which implies that $z$ will be set to 0 after at most two more
multiplications.  Since each of the first $\ceil{\log_2 b}-1$ transitions
involves at most two multiplications, the total number of multiplications
until $z=0$ is at most $2(\ceil{\log_2 z}-1)+2 = 2\ceil{\log_2 b}$.
\end{solution}
