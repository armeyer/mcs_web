...
\[
\exists a \exists b \exists c.\,  M(a,m,j) \QAND M(b,k,k) \QAND A(c,a,b) \QAND A(n,i,c).
\]
...
\forall x, y, z, n.\,
    (
    (\QNOT(\text{Zero}(x)) > 0 \QAND \QNOT(\text{Zero}(y)) \QAND \QNOT(\text{Zero}(z)) \QAND \QNOT(\text{Zero}(n)) \QAND \QNOT(\text{One}(x)))
    \QIMPLIES
    \QNOT(x^n + y^n = z^n)
    )
\]
where the expression $x^n + y^n = z^n$ is expressed in terms of $M$ and $X$ and
additional variables as in part~\eqref{avoidterms}.

\begin{staffnotes}
Nicer if the expression $x^n + y^n = z^n$ was actually replaced.
\end{staffnotes}
...
\begin{solution}
\[
\exists t.\, \text{Two}(t)\ \QAND
    \forall n \exists p \exists q.\,
         \text{Prime}(p) \QAND \text{Prime}(q)  \QAND \text{Greater}(p,n) \QAND A(q,p,t).
\]
\end{solution}
