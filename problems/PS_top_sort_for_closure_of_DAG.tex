...
\item\label{del-cycle} Delete an edge that is in a directed cycle.

\item\label{del-uncover} Delete edge $\diredge{u}{v}$ if there is a
  directed path from vertex $u$ to vertex $v$ that does not include
  $\diredge{u}{v}$.
...
\hint Let $s$ be the number of cycles, $e$ be the number of
