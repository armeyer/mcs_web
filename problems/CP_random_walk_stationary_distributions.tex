...
In general, a starting distribution $(p, 1-p)$ will oscillate between
itself and $(1-p, p)$, so it will converge to a single distribution
iff $p = 1/2$.  That is, the stationary distribution is the
\emph{only} initial distribution that converges to a stationary
distribution.
\end{solution}
...
\begin{solution}
There are uncountably many: any distribution with $d(b)=d(c)=0$, and
$d(a) = p$ and $d(d) = 1-p$ is stable for any real number $p$ in the
unit interval $[0,1]$.
\end{solution}
