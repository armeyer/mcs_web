...
\begin{problem}  Let $p$ be a prime number.

\bparts

\ppart Explain why the multinomial coefficient
...
\begin{solution}
The multinomial coefficient~\eqref{pk1k2} is an integer equal to the
quotient of $p!$ divided by the product $k_1 !, k_2 !, \dots, k_n !$.
If all the $k_i$'s are less than $p$, then none of the denominator
terms divides the numerator, $p$, and so the multinomial coefficient
is divisible by $p$.
\end{solution}
