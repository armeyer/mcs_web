...
\inhandout{A set of numbers is \term{well ordered} when each of its
  nonempty subsets has a minimum element.  The Well Ordering principle
  says, of course, that the set of nonnegative integers is well
  ordered, but so are lots of other sets, for example, the set
  \emph{Halves} equal to all the nonnegative integers along with all
  numbers of the form $n + 1/2$ for $n \in \naturals$.}
...
rational number, $q$, which by definition is less than $r$ and also
$\geq$ ---in fact greater than ---$\sqrt{2}$.
\end{solution}
...
it.  (For example, the set Halves above.)
...
\begin{solution}
$H$ has minimum element 0.  There are only finitely many elements in
  $H$ below any real number less than 1, and so $H$ is well ordered by
  part~\eqref{fin-below}.  In fact, there are only $n$ elements in $H$
  that are less than the element $n/(n+1)$.
\end{solution}
...
\begin{solution}
$H$ has minimum element 0.  Any decreasing sequence of elements $H$
  must be finite, so $H$ is well ordered by
  Problem~\bref{CP_well_order_decreasing}.

To see why such a decreasing sequence must be finite.  If it starts
with an nonnegative integer $n$, it can only contain $n$ more
integers, after which there must be an element of $H$ of the form
$m/(m+1)$, which will have only finitely $m$ more elements below it.
\end{solution}
