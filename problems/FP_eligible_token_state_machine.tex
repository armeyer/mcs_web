...
\begin{pcomments}
  ...
  \pcomment{S16.mid2; with students told to skip part(f)}
  \pcomment{ARM 3/10/16}
  \pcomment{part(g) not on mid2, suggested by Richard Spence 3/13/16}
\end{pcomments}
...
\begin{solution}
\begin{proof}
To verify $P(n+1)$, we must show that for any $(b,w)$ with $b+w =
n+1$, if $(b,w)$ is eligible, then $(b,w)$ is reachable.
...
\ppart Prove that $(4^7 +1 , 4^5 +2)$ is \emph{not} reachable.
...
$(4^7 + 1 , 4^5 + 2)$:
\[
\rem{(4^7 + 1) - (4^5 +2)}{4} = \rem{(4^7 - 4^5) - 4 + 3)}{4} = 3.
\]
which means $(4^7 + 1 , 4^5 + 2)$ is not reachable.
\end{solution}

\begin{solution}
Extra part (maybe use as seperate question): Verify that $V \eqdef
\rem{3b-w}{8}$ is a derived variable that is constant.  Conclude that
no state is reachable from both start states.

Ans: Start state $(5,4)$ has $V = 3$ while state $(4,3)$ has $V = 1$.
\end{solution}
