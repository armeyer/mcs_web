...
\begin{pcomments}
  \pcomment{CP_recursively_defined_sets}
...
\item The set $S \eqdef \set{ 2^k 3^m 5^n \in \mathbb N \suchthat k,m,n \in
\nngint}$.
...
\item  The set $T \eqdef \set{2^k 3^{2k+m} 5^{m+n} \in \mathbb N \suchthat k,m,n \in
\nngint}$.
...
\item The set $L \eqdef \set{ (a, b) \in \integers^2 \suchthat (a-b)
  \text{ is a multiple of } 3}$.
...
\ppart (\emph{Optional}) See if you can give an \emph{unambiguous}
