...
\begin{enumerate}
\item If an edge $\edge{u}{v}$ of $G$ is on a cycle, then
  delete $\edge{u}{v}$.
...
number of cycles.  For each of the derived variables below, indicate
...
\begin{solution}
  If a value (a \emph{\idx{derived variable}}) associated with a
  process state is nonnegative integer-valued and decreases at each
  step, then the process terminates after at most as many steps as the
  initial value of the quantity.  So we need only identify such a
  derived variable.  There are two in the list above, namely (vi) and
  (vii).
