  ...
  \begin{problemparts}
    ...
    \begin{solution}
     ...
     w_n & = \prcond{\text{win starting with $\$n$}}{\text{won first bet}}
         ...
         & \quad + \prcond{\text{win starting with $\$n$}}{\text{lost first bet}}
                \cdot\pr{\text{lost first bet}}\\
         & = \pr{\text{win starting with $\$n+3$}} \cdot \pr{\text{won first bet}}\\
         &  \quad +\pr{\text{win starting with $\$n-1$}} \cdot \pr{\text{lost first bet}},
\end{align*}
so
    \[
...
\end{editingnotes}
  ...
  ends.  Write a linear recurrence for $e_n$; you need \emph{not}
  solve the recurrence.

  \begin{solution}
    By the Law of Total Expectation, Theorem~\bref{total_expect},
    \begin{align*}
      e_n &= \paren{1+\expcond{\text{\#bets with $\$n$ start}}{\text{won first bet}}}
      \cdot \pr{\text{won first bet}}\\
      & \quad + \paren{1+\expcond{\text{\#bets with $\$n$ start}}{\text{lost first bet}}}
      \cdot \pr{\text{lost first bet}}\\
      &= \paren{1+\expect{\text{\#bets with $\$n+3$ start}}}
      \cdot \pr{\text{won first bet}}\\
